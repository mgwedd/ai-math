...
export function attachInteraction(controller, opts = {}) {
  ...
  return {
    ...
    refresh() { discover(); },
    /**
     * Capstone attempt boundary (CONTRACT v1.3 §2) — the mandated trio:
     * controller.newAttempt(seed) (param reroll + gate close) →
     * goals.resetAttempt() (clear in-flight hold/visited progress, re-arm
     * baseline) → baseline re-evaluation. Handles are re-scanned last (an
     * active drag, if any, ends cleanly via handle.remove()).
     * @param {number} [seed] deterministic reroll seed
     * @returns the applied param snapshot
     */
    newAttempt(seed) {
      const snap = controller.newAttempt(seed);
      if (goals && typeof goals.resetAttempt === 'function') goals.resetAttempt();
      feed();                                           // baseline re-eval (§7)
      discover();
      return snap;
    },
