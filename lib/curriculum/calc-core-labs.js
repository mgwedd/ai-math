...
INTERACTIVES.limTable = function(stage){
  ...
  function draw(){
    ...
    // dot marks the LAST listed row (di = shown-1) so the point and the table agree
    const di = shown - 1;
    if(di >= 0) P.dot(XS[di], f(XS[di]), 6, '#00d4ff');
...
INTERACTIVES.limitsOneSided = function(stage, api){
  ...
  function draw(){
    ...
    // removable case has a hole at (0,2) — punch the open circle like the other limit graphs
    if(fi === 2){
      L.ctx.strokeStyle = '#7c5cff'; L.ctx.lineWidth = 2.5; L.ctx.fillStyle = '#11152a';
      L.ctx.beginPath(); L.ctx.arc(P.sx(0), P.sy(2), 6, 0, 7); L.ctx.fill(); L.ctx.stroke();
    }
    ...
    const fmtV = v => v > 1e4 ? '+∞' : (v < -1e4 ? '−∞' : fmt2(v));
    const blowUp = fL > 1e4 || fR > 1e4 || fL < -1e4 || fR < -1e4;
    const verdict = blowUp
      ? '<span style="color:#ffc94d">both sides blow up → no finite limit (DNE)</span>'
      : (Math.abs(fL - fR) < 0.05 ? '<span style="color:#2dd4a0">sides agree → limit exists</span>' : '<span style="color:#ff5c7a">sides disagree → DNE</span>');
    L.readout.innerHTML = 'ε = ' + eps.toFixed(3) + '<br>left  f(−ε) = ' + fmtV(fL) + '<br>right f(+ε) = ' + fmtV(fR) + '<br>' + verdict;
    m.update({fi, eps, fL, fR});
  }
  let epsSl;
  const row = chips(L.ctrl, 'FUNCTION', FN.map(s=>s.name), (i, btn, r)=>{
    fi = i; eps = 1; if(epsSl) epsSl.set(1);   // reset the approach on switch so missions can't fire on a bare chip click
    [...r.children].forEach(el=>el.classList.remove('on')); btn.classList.add('on'); draw();
  });
  row.children[0].classList.add('on');
  epsSl = slider(L.ctrl, 'ε (approach distance)', 0.001, 1, 0.001, 1, x=>x.toFixed(3), x=>{eps=x; draw();});
