...
/* ================================================================
   EXPOSITION FIGURES for la-det ("see it first" gallery).
   ================================================================ */

/* detArea — animate the unit square morphing into a parallelogram under a
   fixed target matrix, so area = |det| reads as a continuous stretch. */
INTERACTIVES.detArea = function(stage){
  const F = figCanvas(stage, 640, 320);
  const ctx = F.ctx, W = F.W, H = F.H;
  const P = plane(ctx, W, H, 58, 170, 210);   // origin low-left so the first-quadrant shape fills the frame
  const I = [1, 0, 0, 1], TGT = [2, 0.8, 0.5, 1.5];
  let t = 0, raf = 0;
  function draw(){
    clearBg(ctx, W, H); P.grid();
    const u = 0.5 + 0.5 * Math.sin(t);
    const M = I.map((v, i) => v + (TGT[i] - v) * u);
    const a = M[0], b = M[1], c = M[2], d = M[3], det = a * d - b * c;
    // faint identity unit-square outline
    ctx.strokeStyle = 'rgba(139,147,184,.4)'; ctx.lineWidth = 1.5; ctx.setLineDash([5, 5]);
    ctx.beginPath();
    [[0,0],[1,0],[1,1],[0,1]].forEach((p,i)=>{const sx=P.sx(p[0]),sy=P.sy(p[1]); i?ctx.lineTo(sx,sy):ctx.moveTo(sx,sy);});
    ...
    ctx.closePath(); ctx.fillStyle = 'rgba(45,212,160,.22)'; ctx.fill();
    ctx.strokeStyle = '#2dd4a0'; ctx.lineWidth = 2.5; ctx.stroke();
    P.arrow(0,0,a,c,'#2dd4a0',3,'col 1'); P.arrow(0,0,b,d,'#00d4ff',3,'col 2');
    ctx.fillStyle = '#cdd4f0'; ctx.font = '700 14px ' + FONT(); ctx.textAlign = 'left';
    ctx.fillText('area = |ad − bc| = ' + Math.abs(det).toFixed(2), 16, 26);
  }
  draw();                                          // synchronous first paint
  function loop(){ t += 0.02; draw(); raf = requestAnimationFrame(loop); }
  raf = requestAnimationFrame(loop);
...
/* detSign — sweep M=[[1,0],[0,s]] through s=0 on the matmul "F" polygon:
   collapses flat at det=0, then returns mirror-reversed for det<0. */
INTERACTIVES.detSign = function(stage){
  const F = figCanvas(stage, 640, 320);
  const ctx = F.ctx, W = F.W, H = F.H;
  const P = plane(ctx, W, H, 58, 250, 175);   // origin left-of-centre so the F fills the frame
  const Fpoly = [[0,0],[0,2],[1,2],[1,1.6],[0.4,1.6],[0.4,1.1],[0.9,1.1],[0.9,0.7],[0.4,0.7],[0.4,0]];
  function T(x, y, M){ return [M[0]*x+M[1]*y, M[2]*x+M[3]*y]; }
  let t = 0, raf = 0;
  function draw(){
    clearBg(ctx, W, H); P.grid();
    const s = Math.sin(t);
    const M = [1, 0, 0, s];
    const det = s;
    const col = det > 0.05 ? '#2dd4a0' : det < -0.05 ? '#ff5c7a' : '#ffc94d';
    const fillCol = det > 0.05 ? 'rgba(45,212,160,.22)' : det < -0.05 ? 'rgba(255,92,122,.22)' : 'rgba(255,201,77,.22)';
    const pts = Fpoly.map(p => T(p[0], p[1], M));
    ctx.beginPath();
    pts.forEach((p,i)=>{const sx=P.sx(p[0]),sy=P.sy(p[1]); i?ctx.lineTo(sx,sy):ctx.moveTo(sx,sy);});
    ctx.closePath(); ctx.fillStyle = fillCol; ctx.fill();
    ctx.strokeStyle = col; ctx.lineWidth = 2.5; ctx.stroke();
    ctx.fillStyle = col; ctx.font = '700 14px ' + FONT(); ctx.textAlign = 'left';
    ctx.fillText('det = ' + det.toFixed(2) + (det < -0.05 ? '  (mirrored)' : det > 0.05 ? '  (upright)' : '  (collapsed!)'), 16, 26);
  }
  draw();                                          // synchronous first paint
  function loop(){ t += 0.02; draw(); raf = requestAnimationFrame(loop); }
  raf = requestAnimationFrame(loop);
...
/* detProductFig — three static panels A, B, B·A showing their parallelograms
   side by side: det(B·A) = det(A) × det(B). Static, no cleanup returned. */
INTERACTIVES.detProductFig = function(stage){
  const F = figCanvas(stage, 640, 320);
  const ctx = F.ctx, W = F.W, H = F.H;
  function mul(B, A){ return [B[0]*A[0]+B[1]*A[2], B[0]*A[1]+B[1]*A[3], B[2]*A[0]+B[3]*A[2], B[2]*A[1]+B[3]*A[3]]; }
  const detOf = M => M[0]*M[3] - M[1]*M[2];
  const A = [2, 0, 0, 1], B = [1, 0.5, 0, 1.5], BA = mul(B, A);
  clearBg(ctx, W, H);
  function panel(ox, M, title){
    const P = plane(ctx, W, H, 40, ox, 200);
    ctx.save(); ctx.beginPath(); ctx.rect(ox - 95, 0, 190, H); ctx.clip();   // keep each panel's grid in its own third
    P.grid();
    const pts = [[0,0],[M[0],M[2]],[M[0]+M[1],M[2]+M[3]],[M[1],M[3]]];
    ctx.beginPath();
    pts.forEach((p,i)=>{const sx=P.sx(p[0]),sy=P.sy(p[1]); i?ctx.lineTo(sx,sy):ctx.moveTo(sx,sy);});
    ctx.closePath(); ctx.fillStyle = 'rgba(45,212,160,.22)'; ctx.fill();
    ctx.strokeStyle = '#2dd4a0'; ctx.lineWidth = 2.5; ctx.stroke();
    ...
    ctx.fillText(title + '  |det| = ' + Math.abs(detOf(M)).toFixed(1), ox, 40);
  }
  panel(150, A, 'A'); panel(345, B, 'B'); panel(540, BA, 'B·A');
  ctx.fillStyle = '#cdd4f0'; ctx.font = '700 13px ' + FONT(); ctx.textAlign = 'center';
  ctx.fillText('det(B·A) = det(A) × det(B):  ' + detOf(BA).toFixed(1) + ' = ' + detOf(A).toFixed(1) + ' × ' + detOf(B).toFixed(1), W / 2, H - 16);
};
...
/* ================== la-det — Lab 3: det(B·A) = det(A) · det(B) ================
   Concept: composing two transforms multiplies their area factors. Pick A and
   B from a small transform bank and watch det(B·A) track det(A) × det(B). */
INTERACTIVES.detProduct = function(stage, api){
  const L = makeLab(stage);
  function mul(B, A){ return [B[0]*A[0]+B[1]*A[2], B[0]*A[1]+B[1]*A[3], B[2]*A[0]+B[3]*A[2], B[2]*A[1]+B[3]*A[3]]; }
  const detOf = M => M[0]*M[3] - M[1]*M[2];
  const TS = [
    ...
    {name:'Squash ½',  M:[0.5, 0, 0, 0.5]},
    {name:'Flip x',    M:[-1, 0, 0, 1]},
  ];
  let ai = 1, bi = 1;

  api.predict({
    prompt: 'A scales area by 2 (\\(\\det A = 2\\)) and B scales area by 3 (\\(\\det B = 3\\)). Predict \\(\\det(B\\cdot A)\\).',
    input: true, placeholder: 'det(B·A)', answer: 6, tol: 0.1, unit: '',
    reveal: 'Determinants multiply along a composition: \\(\\det(B\\cdot A) = \\det(A)\\,\\det(B) = 2\\times 3 = 6\\).',
  });

  const m = api.missions([
    {text:'Build an area-preserving chain: det(B·A) = 1 using two transforms that each change area', xp:20,
      check:s => Math.abs(s.detBA - 1) < 0.05 && s.ai !== s.bi && Math.abs(s.detA - 1) > 0.1},
    {text:'Flip orientation: make det(B·A) &lt; 0', xp:20,
      check:s => s.detBA < -0.5},
    {text:'Biggest blow-up: make det(B·A) = 16', xp:25,
      check:s => Math.abs(s.detBA - 16) < 0.1},
  ]);
  ...
  function draw(){
    ...
    const BA = mul(B, A);
    const detA = detOf(A), detB = detOf(B), detBA = detOf(BA);
    // faint unit square
    L.ctx.strokeStyle = 'rgba(139,147,184,.4)'; L.ctx.lineWidth = 1.5; L.ctx.setLineDash([5,5]);
    L.ctx.beginPath();
    [[0,0],[1,0],[1,1],[0,1]].forEach((p,i)=>{const sx=P.sx(p[0]),sy=P.sy(p[1]); i?L.ctx.lineTo(sx,sy):L.ctx.moveTo(sx,sy);});
    L.ctx.closePath(); L.ctx.stroke(); L.ctx.setLineDash([]);
    // parallelogram of B·A, colored by sign
    const col = detBA < 0 ? 'rgba(255,92,122,' : 'rgba(45,212,160,';
    const pts = [[0,0],[BA[0],BA[2]],[BA[0]+BA[1],BA[2]+BA[3]],[BA[1],BA[3]]];
    L.ctx.beginPath();
    pts.forEach((p,i)=>{const sx=P.sx(p[0]),sy=P.sy(p[1]); i?L.ctx.lineTo(sx,sy):L.ctx.moveTo(sx,sy);});
    L.ctx.closePath(); L.ctx.fillStyle = col+'.22)'; L.ctx.fill();
    L.ctx.strokeStyle = col+'.9)'; L.ctx.lineWidth = 2.5; L.ctx.stroke();
    P.arrow(0,0,BA[0],BA[2],'#2dd4a0',3.5,'î→');   // fixed column colours (sign is carried by the fill)
    P.arrow(0,0,BA[1],BA[3],'#ff5c7a',3.5,'ĵ→');
    L.readout.innerHTML = 'det(A) = ' + detA.toFixed(2) + '   det(B) = ' + detB.toFixed(2) +
      '<br>det(A) · det(B) = ' + (detA*detB).toFixed(2) +
      '<br>det(B·A) = <span style="color:' + (detBA<0?'#ff5c7a':'#2dd4a0') + '">' + detBA.toFixed(2) + '</span>';
    m.update({ai, bi, detA, detB, detBA});
  }
  const row1 = chips(L.ctrl, 'FIRST — A', TS.map(t=>t.name), (i,btn,row)=>{
    ai = i; [...row.children].forEach(b=>b.classList.remove('on')); btn.classList.add('on'); draw(); });
  const row2 = chips(L.ctrl, 'SECOND — B', TS.map(t=>t.name), (i,btn,row)=>{
    bi = i; [...row.children].forEach(b=>b.classList.remove('on')); btn.classList.add('on'); draw(); });
  row1.children[1].classList.add('on'); row2.children[1].classList.add('on');
  note(L.ctrl, 'Green = the composed transform B·A; its area factor is det(B·A). Watch the readout: det(A) × det(B) always equals det(B·A) — determinants multiply along a composition.');
  draw();
};

/* ================== la-det — Lab 4: shear preserves area ======================
   Concept: shearing a column along the other leaves the parallelogram's base
   and height unchanged, so det is invariant — unlike scaling a column. */
INTERACTIVES.detShear = function(stage, api){
  const L = makeLab(stage);
  let s = 1, k = 0;

  api.predict({
    prompt: 'You <b>shear</b> a parallelogram — slide its top edge sideways without changing its base or height. Does its area change?',
    choices: ['No — base × height is unchanged', 'Yes — it gets larger', 'Yes — it gets smaller'],
    answer: 0,
    reveal: 'Shearing slides one column along the other; base and height are unchanged, so the area — and \\(\\det\\) — stay the same. Columns behave just like rows here (\\(\\det M = \\det M^{\\top}\\)), which is why adding a multiple of one row to another during elimination never changes \\(\\det\\).',
  });

  const m = api.missions([
    {text:'Shear hard without changing the area: set k ≥ 1.5 while the area stays at 2 (det = 2)', xp:20,
      check:st => st.k >= 1.5 && Math.abs(st.det - 2) < 0.1},
    {text:'Double the area by scaling a column: make det = 4', xp:20,
      check:st => Math.abs(st.det - 4) < 0.1},
    {text:'Collapse it: scale column 1 to zero so det = 0 (not invertible)', xp:25,
      check:st => Math.abs(st.det) < 0.05},
  ]);

  const P = plane(L.ctx, L.W, L.H, 40, 200, L.H / 2);
  function draw(){
    clearBg(L.ctx, L.W, L.H); P.grid();
    const c1 = {x:2*s, y:0}, c2 = {x:2*s*k, y:1};
    const det = c1.x*c2.y - c2.x*c1.y;
    const col = Math.abs(det) < 0.05 ? 'rgba(255,201,77,' : det < 0 ? 'rgba(255,92,122,' : 'rgba(45,212,160,';
    const pts = [[0,0],[c1.x,c1.y],[c1.x+c2.x,c1.y+c2.y],[c2.x,c2.y]];
    L.ctx.beginPath();
    pts.forEach((p,i)=>{const sx=P.sx(p[0]),sy=P.sy(p[1]); i?L.ctx.lineTo(sx,sy):L.ctx.moveTo(sx,sy);});
    L.ctx.closePath(); L.ctx.fillStyle = col+'.22)'; L.ctx.fill();
    L.ctx.strokeStyle = col+'.9)'; L.ctx.lineWidth = 2.5; L.ctx.stroke();
    // faint base line along column 1
    L.ctx.strokeStyle = 'rgba(45,212,160,.25)'; L.ctx.lineWidth = 1; L.ctx.setLineDash([5,5]);
    L.ctx.beginPath(); L.ctx.moveTo(P.sx(-8), P.sy(0)); L.ctx.lineTo(P.sx(8), P.sy(0)); L.ctx.stroke();
    // dashed height indicator
    L.ctx.strokeStyle = 'rgba(0,212,255,.35)';
    L.ctx.beginPath(); L.ctx.moveTo(P.sx(c2.x), P.sy(0)); L.ctx.lineTo(P.sx(c2.x), P.sy(c2.y)); L.ctx.stroke();
    L.ctx.setLineDash([]);
    P.arrow(0,0,c1.x,c1.y,'#2dd4a0',3,'col 1'); P.arrow(0,0,c2.x,c2.y,'#00d4ff',3,'col 2');
    L.readout.innerHTML = 'col 1 = [' + fmt2(c1.x) + ', ' + fmt2(c1.y) + ']   col 2 = [' + fmt2(c2.x) + ', ' + fmt2(c2.y) + ']<br>' +
      'det = <span style="color:' + (Math.abs(det)<0.05?'#ffc94d':det<0?'#ff5c7a':'#2dd4a0') + '">' + det.toFixed(2) + '</span>' +
      '   area = ' + Math.abs(det).toFixed(2);
    m.update({s, k, det});
  }
  slider(L.ctrl, 'scale column 1 (s)', 0, 3, 0.1, 1, x=>x.toFixed(1), x=>{ s=x; draw(); });
  slider(L.ctrl, 'shear k (slide column 2 along column 1)', -1.5, 1.5, 0.1, 0, x=>x.toFixed(1), x=>{ k=x; draw(); });
  note(L.ctrl, 'Two moves: <b>shear k</b> slides column 2 along column 1 — the parallelogram slants but base × height (the area) stays fixed, so det never moves. <b>Scale s</b> stretches column 1 and changes det = 2s directly. Shearing is the row operation that leaves a determinant untouched.');
  draw();
};
