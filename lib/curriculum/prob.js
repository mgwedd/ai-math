/* ================================================================
   WORLD 2.5 — PROBABILITY & STATISTICS: the language models speak.
   ...
   World 3 (ml-gpt's "probability distribution over tokens") spends.
   ================================================================ */
...
LESSONS.push({
  ...
  learn:`<p>A <strong>random variable</strong> is a number assigned to a random outcome: the face a die shows, tomorrow's clicks, the next token's score. Before the roll it has no value — it has a <strong>distribution</strong>: each possible value with its probability.</p>
  ...
  <p>The <strong>law of large numbers</strong> makes the promise concrete: as you roll more, the running sample mean converges to E[X]. Chance is noisy per-event and lawful in bulk.</p>`,
  ml:`Every loss you'll ever minimize is an expectation: <code>E[loss(model(x), y)]</code> over the data distribution. You can't compute it exactly — you <b>estimate it from samples</b> (a batch!), trusting the law of large numbers. "Training loss" is literally a running sample mean of a random variable.`,
  deeper:[
...
WRONG_WHY['prob-rv']=[
 {1:'3 is the median face ⌊3.5⌋ — close, but expectation weights ALL faces: 21/6 = 3.5.',2:'21 is the SUM of the faces — you skipped the ÷6 weighting.',3:'6 is just the max. Expectation averages, it doesn\'t take the best case.'},
 {1:'Time isn\'t required — one die roll is a perfectly good random variable.',2:'An unknown isn\'t random unless outcomes carry probabilities.',3:'A probability is a number in [0,1] attached to an event; the RV is the VALUE you observe.'},
 {1:'The definition never promises E[X] is attainable — it\'s a weighted average over outcomes.',2:'Fair dice genuinely have E = 3.5; unfairness would MOVE it, not explain it.',3:'No rounding involved: 21/6 is exactly 3.5.'},
 {1:'At n = 10,000 the standard error is ~0.017 — a 0.96 deviation is ~50σ. That\'s not luck.',2:'Sample means are bounded by the faces (1–6), but 4.46 is perfectly possible — for a LOADED die.',3:'Sample size is exactly what makes it sayable: more rolls = tighter pin on E[X].'}];
...
LESSONS.push({
  ...
  <p><strong>Bernoulli(p)</strong> — one yes/no trial: 1 with probability p, else 0. The atom of probability.</p>
  <p><strong>Binomial(n, p)</strong> — count of successes in n independent Bernoulli trials:</p>
  <div class="formula">P(k) = C(n, k) · pᵏ(1−p)ⁿ⁻ᵏ</div>
  <p><strong>Gaussian / Normal(μ, σ)</strong> — the continuous bell: μ sets the center, σ the spread. It appears whenever many small independent effects add up (next lessons make that precise).</p>
  <p>Watch in the lab: crank a binomial's n and the bars <em>become</em> a bell. That's not coincidence — it's a theorem you'll meet in lesson 5.</p>`,
...
WRONG_WHY['prob-dist']=[
 {1:'Bernoulli covers ONE flip. A hundred of them, counted, is binomial.',2:'Gaussian is continuous — head-counts are whole numbers. (Though the binomial\'s SHAPE approaches a bell…)',3:'Uniform would mean 0 heads is as likely as 50. Try flipping 100 coins and getting 0.'},
 {1:'Shifting is μ\'s job — σ only stretches around the center.',2:'Backwards: bigger σ spreads the same area wider, so the peak DROPS.',3:'A Gaussian is symmetric for every μ, σ. No knob skews it.'},
 {1:'Flat would mean all head-counts equally likely — but extreme counts need every flip to agree: vanishingly rare.',2:'Exponential decay is monotone; the binomial rises to a hump at np then falls.',3:'Two peaks would need two distinct clusters of outcomes; coin-flip counts pile around one center, np.'},
 {1:'The 0.83 describes THIS email\'s label uncertainty, not a fraction of its content.',2:'Accuracy is measured across many emails; the sigmoid speaks about one.',3:'Two classes doesn\'t mean 50/50 — the whole point of the model is to move p away from ignorance.'}];
...
LESSONS.push({
  ...
  <div class="formula">P(H | E) = P(E | H) · P(H) / P(E)</div>
...
WRONG_WHY['prob-bayes']=[
 {1:'99% is the test\'s sensitivity — P(positive|sick), not P(sick|positive). Flipping these is THE classic error this lesson exists to kill.',2:'95% is specificity — a property of the test on healthy people, not your posterior.',3:'1% was your prior, BEFORE the test. The positive result does move you — from 1% up to ~17%.'},
 {1:'Dividing by the prior would make rare hypotheses MORE believable on weak evidence — backwards.',2:'Probabilities multiply through Bayes; nothing is subtracted.',3:'Evidence P(E) sits in the DENOMINATOR as a normalizer — and the posterior is the output, not an input.'},
 {1:'Test chemistry doesn\'t know prevalence — the numbers (sens/spec) are unchanged. What changes is the POPULATION the test runs on.',2:'Sensitivity is fixed by the test, not the disease rate. The arithmetic of the healthy majority does all the damage.',3:'It does matter — run the 10,000-person count and watch 17% fall out of a "99% accurate" test.'},
 {1:'Overfitting is a model-fitting failure (memorizing noise). Ignoring priors is a reasoning failure.',2:'The law of large numbers is about sample means converging — unrelated to dropping priors.',3:'Regularization is, if anything, the OPPOSITE: it injects a prior into training.'}];
...
LESSONS.push({
  ...
  <div class="formula">θ* = argmax<sub>θ</sub>  P(data | θ) = argmax<sub>θ</sub> Π p(xᵢ | θ)</div>
  ...
  <div class="formula">log L(θ) = Σ log p(xᵢ | θ)</div>
  <p>For a Gaussian, calculus gives a famous answer: the MLE of μ is the <strong>sample mean</strong>, and of σ² the sample variance. In the lab you'll find both by hand — drag the bell until the data stops being surprising.</p>
  ...
  ml:`This is where loss functions COME FROM. Minimizing <b>MSE</b> = MLE under Gaussian noise. Minimizing <b>cross-entropy</b> = MLE for a categorical model — which is exactly how GPT is trained: maximize the log-probability of the next token, summed over the internet. "Training" is this lab, scaled up.`,
  deeper:[
...
WRONG_WHY['prob-mle']=[
 {1:'P(parameters|data) is the POSTERIOR — maximizing it is MAP estimation and requires a prior. MLE flips the conditioning.',2:'Parameter count is architecture/model selection, a different question entirely.',3:'Fitting every point exactly is interpolation — usually catastrophic overfitting, and not what "most probable data" means.'},
 {1:'log of a number in (0,1) is NEGATIVE — it makes them smaller (and that\'s fine; we only compare).',2:'Likelihoods are products of densities — never negative. Log-likelihoods are routinely negative, harmlessly.',3:'Monotone transforms can\'t move an argmax. Same answer, friendlier arithmetic.'},
 {1:'The median is more robust to outliers but it is NOT the Gaussian MLE — the calculus delivers the mean.',2:'Min-max midpoint is the uniform distribution\'s flavor of estimate, not the Gaussian\'s.',3:'Only if your data happens to center at 0. The MLE follows the data, not the origin.'},
 {1:'K-means is clustering — no likelihood, no probability model over text.',2:'It has an exact probabilistic meaning — that\'s the beauty: the loss everyone uses IS a log-likelihood.',3:'Bayesian inference would maintain a distribution over weights; GPT training finds one point estimate by MLE.'}];
...
LESSONS.push({
  ...
  <div class="formula">x̄ₙ  ⟶  Normal( μ , σ/√n )</div>
...
WRONG_WHY['prob-clt']=[
 {1:'Only at n = 1. The whole theorem is that growing n ERASES the source\'s shape.',2:'Uniform is just another source that gets bell-ified like everything else.',3:'The SPREAD shrinks toward zero, but the distribution\'s shape goes Gaussian around μ — it doesn\'t collapse to the number 0.'},
 {1:'2 is the SOURCE σ — single draws. Averages of 100 are far steadier: divide by √100.',2:'0.02 divides by n = 100 instead of √n = 10. The square root is the entire point.',3:'20 multiplies by √n. Averaging REDUCES noise; more samples can\'t make the mean wilder.'},
 {1:'The bell needs averaging to emerge — one draw has nothing to average.',2:'Flat happens only if the SOURCE is flat (and n = 1 shows the source).',3:'A spike would mean every draw lands identically — only true if the source has zero variance.'},
 {1:'A quarter would need 16× the batch (1/√16). Tempting linear thinking; the √ says otherwise.',2:'Bigger batches genuinely average out more noise — that\'s why they\'re used.',3:'More averaging cannot AMPLIFY noise — direction is right only if you confuse n with 1/n.'}];
...
LESSONS.push({
  ...
  <div class="formula">H(P) = − Σ pᵢ log₂ pᵢ</div>
  ...
  <div class="formula">KL(P ‖ Q) = Σ pᵢ log₂ (pᵢ / qᵢ)</div>
...
WRONG_WHY['prob-entropy']=[
 {1:'All mass on one outcome is H = 0 — MINIMUM entropy. You\'ve confused certainty with uncertainty.',2:'Any asymmetry means some outcome is more predictable — average surprise drops below 2 bits.',3:'It does: log₂(k) bits for k outcomes, achieved exactly at uniform.'},
 {1:'Equally likely is MAXIMUM entropy (2 bits over 4 outcomes) — the far end of the scale from 0.',2:'Perfectly valid: P = (1,0,0,0) is a legitimate distribution. Boring, but legitimate.',3:'Probabilities are never negative anywhere in probability theory — and entropy\'s formula doesn\'t need them to hit 0.'},
 {1:'Independence is a property of JOINT distributions of two variables — KL here compares two beliefs about the SAME variable.',2:'Two uniforms over the same outcomes are equal, so KL = 0 — but that\'s because they\'re EQUAL, not because they\'re uniform.',3:'KL ≥ 0 always, but it DOES reach 0 — precisely at Q = P.'},
 {1:'H(data) is what training would minimize if it could — but no model choice changes the dataset\'s own entropy.',2:'They\'re not symmetric in the model\'s eyes: only KL contains θ.',3:'Cross-entropy is THE standard loss — the decomposition is exact, not an analogy.'}];
