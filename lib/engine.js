...
/* ---------- learner progress tools (available to everyone) ----------
   ...
   and resetting their own progress. Manual-unlock overrides live in S.admin
...
function mergeState(local, remote){
  ...
  m.admin = {
    unlockAll: !!((local.admin&&local.admin.unlockAll) || (remote.admin&&remote.admin.unlockAll)),
    unlocked: Object.assign({}, (remote.admin||{}).unlocked||{}, (local.admin||{}).unlocked||{}),
  };
...
function isUnlocked(lesson){
  if(S.admin?.unlockAll || S.admin?.unlocked?.[lesson.id]) return true;
...
function isManualUnlock(lesson){
  return !!(S.admin?.unlockAll || S.admin?.unlocked?.[lesson.id]) && !isNaturallyUnlocked(lesson);
}
const WORLD_META = {
...
function onAdminClick(e){
  ...
  const btn = e.target.closest('.admin-dots'); if(!btn) return;
...
function renderHome(){
  ...
  for(const w of WORLD_ORDER){
    ...
    h+='<section class="world'+(collapsed?' collapsed':'')+'"><div class="world-head">'+
       ...
       '<button class="admin-dots" data-world="'+w+'" title="World tools" aria-label="World tools">⋮</button>'+
       '</div>';
    ...
    for(const l of list){
      ...
      h+='<div class="lcard'+(unlocked?'':' locked')+(done_?' done-card':'')+'" '+
         ...
         '<button class="admin-dots admin-dots-card" data-lesson="'+l.id+'" title="Lesson tools" aria-label="Lesson tools">⋮</button>'+
...
export function mount(opts){
  ...
  view.addEventListener('click', onAdminClick, true);
