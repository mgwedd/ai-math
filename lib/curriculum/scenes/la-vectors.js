...
registerScene({
  ...
  goals: [
    goal('Stretch v out to length 5 — a 3-4-5 right triangle, any direction',
    ...
    goal('Now reach length 13 — the 5-12-13 triangle — with a NEGATIVE x-component',
  ...
  caption: 'Drag the tip. The dashed lines show the two components; the readout shows how they combine into the length ‖v‖ — that combination is Pythagoras.',
});
...
registerScene({
  ...
  goals: [
    goal('Place the tip exactly on the first target ★₁ = [4, −3]',
    ...
    goal('Now hit the second target ★₂ = [−3, 4] — a different quadrant, both signs flipped',
  ...
  caption: 'The two dashed drop-lines are the components read straight off the axes. Land the tip on each star by matching both numbers exactly.',
});
...
registerScene({
  ...
  goals: [
    goal('Shrink or stretch v to length exactly 1 — landing it on the circle',
    ...
    goal('Now find a DIFFERENT point on the circle, with a negative x-coordinate',
  ...
  caption: 'The faint ring is the unit circle — every vector of length 1. Drag v onto it, then find a second point on the ring in a different direction.',
});
...
registerScene({
  ...
  goals: [
    goal('Dial r and θ so v lands on [3, 4] (r = 5 — a familiar triangle)',
    ...
    goal('Now dial r down to exactly 1, with θ past 90° from straight right',
  ...
  caption: 'Two sliders, one vector: length r and angle θ. Watch the readout — the components are always [r cosθ, r sinθ], however you got there.',
});
...
registerScene({
  ...
  goals: [
    goal('Raise the total length of this 5-number vector to 7',
    ...
    goal('Now bring it back down near 4.2 — as close to the 2-number length (√13 ≈ 3.6) as these extra numbers allow',
  ...
  caption: 'This vector secretly has 5 numbers, not 2 — the bars show all of them. Slide k1, k2, k3 and watch the SAME magnitude formula still apply.',
});
...
registerScene({
  ...
  goals: [
    goal('Drag the probe so MOUSE is clearly its nearest neighbor',
    ...
    goal('Now make HORSE the nearest neighbor instead',
  ...
  caption: 'Each animal is a data point [speed, size] — an arrow from the origin. Drag the probe ● and watch which one is nearest: that IS similarity search.',
});
...
registerScene({
  ...
  goals: [
    goal('Send v to the marked target ★ (magnitude AND direction) and hold',
    ...
    goal('Now land v exactly on the marked point ☆ on the unit circle and hold',
    ...
    goal('Extend v with the hidden number k until the FULL 3-number length hits the target',
  ...
  caption: 'No hints now. Send v to ★, then onto ☆ on the unit circle, then dial k until the extended length matches its target — holding each steady. This is the exam.',
});
