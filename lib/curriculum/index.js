...
registerLesson({
  ...
  expositions:[
    {key:'sectan', title:'Secant tips into tangent as h → 0', figure:'derSecantTangent', caption:'The secant slope \\(\\frac{f(a+h)-f(a)}{h}\\) uses two points. Shrink \\(h\\) toward 0 and the secant rotates into the <b>tangent</b> — its slope is the derivative \\(f\'(a)=\\lim_{h\\to 0}\\frac{f(a+h)-f(a)}{h}\\).'},
    {key:'newfn', title:'The derivative is a new function', figure:'derSlopeFn', caption:'For \\(f(x)=\\tfrac12 x^2\\), the tangent <b>slope</b> on the top curve equals the <b>height</b> on the bottom curve — that bottom curve is \\(f\'(x)=x\\), a whole new function reporting the slope everywhere.'},
    {key:'local', title:'Zoom in — every smooth curve is locally a line', figure:'derLocalLinear', caption:'Zoom into a point on a curve and it flattens toward a straight line. That line is the tangent, and its slope is \\(f\'(a)\\) — local linearity is <b>why</b> the derivative exists.'},
  ],
  labs:[
    ...
    {key:'trace', title:'Trace out the derivative function', interactive:'derivTrace',
     intro:'<p>Read the <strong>slope</strong> of \\(f\\) at several \\(x\\)-values and stamp each as a point on the graph below — you are drawing \\(f\'(x)\\) by hand, proving the derivative is a function in its own right.</p>'},
    {key:'sign', title:'Sign of f′ ↔ shape of f', interactive:'derivSign',
     intro:'<p>Where is \\(f\\) rising, falling, or flat? Connect each to \\(f\'>0\\), \\(f\'<0\\), \\(f\'=0\\) — the reading that powers all of optimization.</p>'},
  ],
  quiz:[
