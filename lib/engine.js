...
function renderLesson(id){
  ...
  function showLearn(){
    ...
    const expHTML = exps.length
      ? '<div class="panel expo-panel"><h2>🎬 See it first</h2><div class="expo-set">'+
        exps.map((e,i)=>'<figure class="expo">'+
          (e.title?'<figcaption class="expo-title">'+e.title+'</figcaption>':'')+
          '<div class="expo-stage" id="expo-'+i+'"></div>'+
    ...
    exps.forEach((e,i)=>{
      const stage=b.querySelector('#expo-'+i);
      const fn=INTERACTIVES[e.figure];
      if(typeof fn==='function'){ const cleanup=fn(stage); if(typeof cleanup==='function') registerCleanup(cleanup); }
      else stage.innerHTML='<div class="lab-intro">Missing figure: '+e.figure+'</div>';
    });
