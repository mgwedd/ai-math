/* ================================================================
   ...
   WEAK-AREA TAG MIGRATION — the old quiz's four q.tag/q.focus pairs:
     quiz 'inverse meaning'     → scene 1 (both), scene 2 g1
                                  + capstone goal 1
     quiz 'computing inverses'  → scene 2 g2, scene 3 (both goals — g2's
                                  diag(2,2) target is deliberately NOT
                                  the identity, so it drills per-axis
                                  computation, not "B undoes A"; review
                                  finding R-CONTENT wave K)
                                  + capstone goal 2
...
registerScene({
  ...
  goals: [
    goal('Solve Ax = b: drag the input x until Ax lands on the target b (within 0.15), to see how running A backwards is a genuinely different problem from running it forwards — computing Ax is one multiplication, but hunting the x that lands on b is the inverse question',
...
registerScene({
  ...
  goals: [
    ...
    goal('Now make the round trip scale uniformly ×2 instead: B·A = diag(2, 2), to see how the axes are independent knobs — matching them takes DIFFERENT slider values, not one shared “reciprocal of A”',
      ...
      { xp: 20, hold: 400, tag: 'computing inverses',
        focus: 'Landing diag(2,2) needs 2·s1 = 2 but 4·s2 = 2 — the sliders land on DIFFERENT values because each axis carries its own factor; no single shared knob works.' }),
  ],
...
registerScene({
  ...
  goals: [
    goal('Solve the system: drag x until Ax lands on the first target b and hold, to see how an invertible transform hides no information — the target b pins down its input completely, and finding it is applying A⁻¹',
