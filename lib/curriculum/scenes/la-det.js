...
/* Micro-idea: composing two transforms MULTIPLIES their area factors. This
   is exactly the mechanism behind vanishing/exploding activations in a
   deep stack: each layer's local determinant (how much it scales volume)
   compounds multiplicatively across the chain, and a SINGLE collapsed
   (det≈0) layer collapses the whole composition, however healthy the
   rest of the chain is. Owns all three tags: the multiply rule itself
   (det formula), a sign flip through composition (negative det), and a
   chain-wide collapse from one bad factor (det = 0 collapse). */
registerScene({
  id: 'det.product',
  lesson: LESSON,
  space: 'plane2',
  ...
  goals: [
    goal('Build a chain that distorts area at each step but preserves it overall: det(A) and det(B) both far from 1, yet det(B·A) ≈ 1',
      (s) => Math.abs(det2(s.aCol1, s.aCol2) - 1) > 0.3 && Math.abs(det2(s.bCol1, s.bCol2) - 1) > 0.3
    ...
    goal('Now flip orientation through the chain: with both factors real, drive det(B·A) below −0.5',
    ...
    goal('Collapse the WHOLE chain by collapsing just ONE factor — keep the other healthy (|det| > 1) — and watch det(B·A) go to 0 too',
      (s) => mag(s.aCol1) > MIN_MAG && mag(s.aCol2) > MIN_MAG && mag(s.bCol1) > MIN_MAG && mag(s.bCol2) > MIN_MAG
...
/* Micro-idea: shearing column 2 ALONG column 1 (adding a multiple of one
   ...
   purpose: a shear costs zero determinant, hence zero information loss,
   while a scaling step does cost it. col1 = [2s, 0], col2 = [2sk, 1], so
...
registerScene({
  ...
  goals: [
    goal('Shear hard (|k| ≥ 1.5) while holding the area fixed at det ≈ 2 — because shear costs nothing',
      ...
      { xp: 20, hold: 400, tag: 'det formula',
        focus: 'Shear slides column 2 along column 1 without ever moving det = 2s. This is the volume-preserving move real invertible-network layers use on purpose — it costs zero determinant, hence zero information loss.' }),
    goal('Now scale column 1 instead: double the area to det = 4',
      (s) => Math.abs(2 * s.s - 4) < 0.15,
      { xp: 20, hold: 400, tag: 'det formula',
        focus: 'Unlike shearing, scaling column 1 changes det = 2s directly — this IS the move that costs information: a layer that scales activations changes how much variance survives, unlike a pure shear.' }),
