...
registerLesson({
  ...
  expositions:[
    {key:'add',   title:'Addition is tip-to-tail',        figure:'vecopsAddAnim', caption:'To add \\(a + b\\), walk along a, then continue along b. The green arrow from the origin to where you land is the sum — and you arrive at the same place whichever order you walk.'},
    {key:'scale', title:'Scaling slides along one line',  figure:'vecopsScale',   caption:'Multiplying by a scalar c stretches (\\(|c|>1\\)), shrinks (\\(|c|<1\\)), or flips (\\(c<0\\)) a vector <b>along its own line</b>. A scalar can never rotate it off that line.'},
    {key:'span',  title:'Combinations fill a plane — or collapse to a line', figure:'vecopsSpanFig', caption:'Every \\(c_1 a + c_2 b\\) lies in the <b>span</b> of a and b — the set of all such combinations. Independent vectors reach the whole plane; when one is a multiple of the other, the span collapses to a single line — the first sight of <b>linear dependence</b>.'},
  ],
  deeper:[
  ...
  labs:[
    ...
    {key:'span', title:'The span: cover the plane, then collapse it', interactive:'vecopsSpan',
     intro:'<p>Drag a and b and watch the lattice of all combinations \\(c_1 a + c_2 b\\). Spread them out to fill the plane, then line them up until the whole span crushes to a single line — that collapse is <strong>linear dependence</strong>.</p>'},
    {key:'diff', title:'The difference vector — the walk from a to b', interactive:'vecopsDiff',
     intro:'<p>The vector \\(b - a\\) is the arrow that takes you <em>from point a to point b</em>. Its length is the distance between them — the same magnitude idea from the last lesson, now measuring a gap.</p>'},
  ],
  quiz:[
