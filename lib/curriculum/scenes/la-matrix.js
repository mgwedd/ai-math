  ...
  goals: [
    goal('Separate the classes: transform space so every A lands at output x > 0.5 and every B at x < −0.5, to see how a single linear layer can pull two classes apart',
      ...
      { xp: 25, hold: 500, tag: 'reading transformations',
        focus: 'A matrix moves every point at once. A quarter-turn sends the "up" cloud rightward and the "down" cloud leftward — a linear map that separates them, exactly what a linear layer’s weight matrix learns.' }),
    ...
    goal('Separate them while keeping space full-dimensional (a separation with |det| > 0.5), to see how a layer can separate classes without collapsing dimensions',
      ...
      { xp: 25, hold: 500, tag: 'reading transformations',
        focus: 'You could squash everything onto the x-axis (det ≈ 0) to separate — but keep det away from 0 and the map stays invertible, preserving information the way a linear layer must.' }),
  ],
  caption: 'Drag the columns to warp space so the two clouds land on opposite sides of x = 0, to see how a linear layer’s weight matrix can separate data that’s already linearly separable — real depth in a network comes from stacking this with a nonlinearity.',
});
