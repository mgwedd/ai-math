...
/* ---------- persistent state ---------- */
const SAVE_KEY = 'gradient-ascent-v1';
...
/* ---------- server sync (postgres backend, optional) ----------
   When served over http(s) (docker compose up → localhost:3000) progress
   syncs to the API with a debounce; localStorage remains the offline
   fallback and the only store when opened as a file:// page.            */
const API = (typeof location !== 'undefined' && /^https?:$/.test(location.protocol)) ? '/api' : null;
const USER_RE = /^[a-z0-9_-]{1,32}$/;
let USER = null, syncTimer = null, eventQueue = [], flushing = false;
function setSync(st){
...
async function pushState(){
  try{
    const r = await fetch(API+'/state/'+USER, {method:'PUT',
      headers:{'Content-Type':'application/json'}, body:JSON.stringify({state:S})});
    if(!r.ok) throw new Error(r.status);
    flushEvents(); setSync('synced');
  }catch(e){ setSync('offline'); }
}
function logEvent(ev){ if(!API || !USER) return; eventQueue.push(ev); flushEvents(); }
async function flushEvents(){
  if(flushing || !eventQueue.length || !API || !USER) return;
  flushing = true;
  const batch = eventQueue.splice(0);
  try{
    const r = await fetch(API+'/events/'+USER, {method:'POST',
...
// in-app modal (window.prompt is blocked in embedded browsers like the
// VS Code preview, so never rely on it)
function askUsername(current, cb){
  const back = document.createElement('div'); back.className='modal-back';
  back.innerHTML =
    '<div class="modal"><h2>👤 Who\'s learning?</h2>'+
    '<p>Pick a username — your XP, lessons and quiz history save to Postgres under it. '+
    'Lowercase letters, numbers, <code>-</code> and <code>_</code> only.</p>'+
    '<input id="uname" maxlength="32" placeholder="e.g. michael" value="'+(current||'')+'">'+
    '<button class="btn" id="ugo">Start learning →</button></div>';
  document.body.appendChild(back);
  const input = back.querySelector('#uname'), btn = back.querySelector('#ugo');
  const clean = v => v.toLowerCase().trim().replace(/[^a-z0-9_-]/g,'').slice(0,32);
  const submit = ()=>{
    const u = clean(input.value);
    if(!USER_RE.test(u)){ input.style.borderColor='var(--bad)'; input.focus(); return; }
    back.remove(); cb(u);
  };
  btn.onclick = submit;
  input.addEventListener('keydown', e=>{ if(e.key==='Enter') submit(); });
  if(current && current!==''){ // allow dismissing when just switching users
...
function adoptUser(u){
  USER = u;
  try{ localStorage.setItem('ga-user', USER); }catch(e){}
  const pill = document.getElementById('user-pill');
  pill.style.display = 'flex';
  document.getElementById('user-name').textContent = USER;
  pill.onclick = ()=> askUsername(USER, nu=>{ if(nu && nu!==USER) adoptUser(nu); });
  initialFetch();
}
function initSync(){
  try{
    ...
    let u = '';
    try{ u = localStorage.getItem('ga-user') || ''; }catch(e){}
    if(USER_RE.test(u)) adoptUser(u);
    else askUsername('', adoptUser);
  }catch(e){ console.error('sync init failed', e); setSync('offline'); }
}
async function initialFetch(){
  setSync('saving');
  try{
    const r = await fetch(API+'/state/'+USER);
    if(r.ok){
