...
registerLesson({
  ...
  <div class="formula">$$\\text{softmax}(z)_i = \\dfrac{\\exp(z_i)}{\\sum_j \\exp(z_j)}$$</div>
  ...
  <div class="formula">$$\\text{softmax}(z)_i = \\dfrac{\\exp(z_i - c)}{\\sum_j \\exp(z_j - c)}$$</div>
  ...
  <div class="formula">$$\\text{CE} = -z_k + \\log \\sum_j \\exp(z_j) = -z_k + \\text{LSE}(z)$$</div>
...
INTERACTIVES['num-softmax-overflow'] = function(stage, api){
  ...
  function draw() {
    ...
    // draw bars
    const barW = 60, gap = 40, y0 = L.H - 80;
...
registerLesson({
  ...
  <div class="formula">$$\\dfrac{\\|\\Delta x\\|}{\\|x\\|} \\leq \\kappa(A) \\cdot \\dfrac{\\|\\Delta b\\|}{\\|b\\|}$$</div>
...
INTERACTIVES['num-solve-vs-invert'] = function(stage, api){
  ...
  function drawWithMissions() {
    ...
    L.readout.innerHTML = 'κ(A) = '+kappa.toExponential(2)+'  &nbsp; σ<sub>min</sub> = '+sMin.toExponential(2)
      + '<br>Direct solve error: <b style="color:#2dd4a0">'+errSolve.toExponential(2)+'</b>  &nbsp; residual ‖Ax−b‖ = '+resSolve.toExponential(2)
      + '<br>Invert+multiply error: <b style="color:#ff9db1">'+errInvert.toExponential(2)+'</b>  &nbsp; residual ‖Ax−b‖ = '+resInvert.toExponential(2)
      + '<br>More accurate: <b>'+ better +'</b>'
      + (errInvert > errSolve * 10 ? ' &nbsp; <b style="color:#ff9db1">Invert-then-multiply is measurably less accurate at this κ</b>' : ' &nbsp; Both similar — both need κ · machine ε headroom to differ');
