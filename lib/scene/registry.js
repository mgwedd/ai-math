...
/**
 * Frame-routing problems for a single EVALUATED entity (CONTRACT Amendment
 * v1.6 §1). Shared by validateScenes (registered scenes) AND mountScene's
 * inline-spec guard, so both paths reject the identical authoring mistakes
 * (R-KIT frozen: inline objects get the same validation as registered ones).
 * This MUST run on the production inline path: at draw time the backend silently
 * falls back to `main` for a bad/undeclared frame, so validation is the ONLY
 * place these surface. Checks: unknown frame; frame:`inset` in a scene with no
 * `inset` declared (nowhere to route); a `handle` on an inset entity (the inset
 * is READ-ONLY in v1.6); and a frame:`inset` at:`readout` label (inert — the
 * readout is a DOM strip, not the inset sub-space).
 * @param {Object} e an evaluated entity descriptor
 * @param {boolean} hasInset whether the scene declares an `inset` sub-space
 * @returns {string[]}
 */
export function entityFrameProblems(e, hasInset){
  const out = [];
  const frame = (e && e.frame == null) ? 'main' : (e && e.frame);
  if(!FRAMES.has(frame)){
    out.push('invalid frame `' + frame + '` (must be `main` or `inset`)');
  }else if(frame === 'inset'){
    if(!hasInset) out.push('uses frame:`inset` but the scene declares no `inset` sub-space');
    if(e.handle != null) out.push('a frame:`inset` entity cannot carry a `handle` (the inset is read-only in v1.6)');
    if(e.kind === 'label' && e.at === 'readout') out.push('a frame:`inset` label with at:`readout` is inert — the readout is a DOM strip, not the inset; give it an at:<vec> in inset coords');
  }
  return out;
}
...
export function validateScenes(){
  ...
  for(const spec of SCENES){
    ...
    list.forEach((e, i) => {
      ...
      // FRAME routing (v1.6): shared with mountScene's inline guard.
      entityFrameProblems(e, spec.inset != null).forEach(m => problems.push(tag + 'entities[' + i + '] ' + m));
    });
  }
  return problems;
}
