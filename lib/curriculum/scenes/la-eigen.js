...
registerScene({
  ...
  goals: [
    goal('Push k up until the FAST vector locks onto the dashed dominant direction (v_k = normalize(Mᵏv₀)), to see how repeatedly applying a matrix and renormalizing converges onto its dominant eigenvector — the exact trick PageRank uses to rank an entire web graph without ever solving for an eigenvector directly',
...
registerScene({
  ...
  goals: [
    ...
    goal('With the eigenvalues kept clearly distinct, rotate the live eigen-axis to φ ≈ 2.0 rad, to see how the eigen-axis ANGLE is an independent knob from the eigenvalues themselves — exactly like a PCA principal axis that can rotate while the variance it captures stays fixed',
