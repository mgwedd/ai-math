...
/* ---- 2×2 inverse toolkit (la-inverse) ------------------------------
   A⁻¹ = (1/det)·[[d,−b],[−c,a]] for A = [[a,b],[c,d]] — the closed form
   the whole la-inverse lesson orbits (det in the denominator IS the
   invertibility gate). Columns in, columns out, matching matApply/matMul:
   the input matrix is col1 = [a,c], col2 = [b,d]; the returned {c1,c2}
   are the INVERSE's columns, so matApply(inv.c1, inv.c2, b) solves
   Ax = b directly. Returns null when |det| < 1e-9 (singular — no
   inverse exists); callers must check truthiness before reading entries,
   so Infinity/NaN never leaks into a goal predicate or readout. */
export function inv2(col1, col2) {
  const d = det2(col1, col2);
  if (Math.abs(d) < 1e-9) return null;
  return {
    c1: { x: col2.y / d, y: -col1.y / d },
    c2: { x: -col2.x / d, y: col1.x / d },
  };
}
