...
registerScene({
  ...
  goals: [
    goal('Stretch a until its shadow reaches past the tip of b, to see how length and alignment both drive up a dot-product score',
    ...
    goal('Fold a down so its shadow shrinks to nothing (a ⟂ b), to see how zero dot product marks "no shared signal"',
    ...
    goal('Swing a the other way so its shadow points against b (negative), to see how a negative dot product signals opposition',
      (s) => proj(s.a, s.b) < -0.1,
      { xp: 20, tag: 'sign vs angle',
        focus: 'A backward shadow is a negative dot product — the angle has opened past 90°.' }),
  ],
  caption: 'Drag the arrows to see how the shadow reveals a·b: it’s the projection length times ‖b‖. This same arithmetic scores similarity between embeddings inside every attention layer.',
});
...
registerScene({
  ...
  goals: [
    goal('Sweep b until it aligns with a (cos θ > 0.98), to see how cos θ → 1 marks two embeddings pointing the same way',
      (s) => cos(A2, s.b) > 0.98,
      { xp: 20, tag: 'cosine similarity',
        focus: 'cos = +1 is the "same direction" anchor — the arrows overlap.' }),
    goal('Park b square to a (cos θ ≈ 0), to see how cos θ = 0 marks "unrelated" in a similarity score',
    ...
    goal('Sweep b to point directly against a (cos θ < −0.98), to see how cos θ → −1 marks directly opposed meaning',
      (s) => cos(A2, s.b) < -0.98,
      { xp: 20, tag: 'cosine similarity',
        focus: 'cos = −1 is the "opposite" anchor — b points 180° from a. Length never entered any of this.' }),
  ],
  caption: 'Sweep b around the circle — its length is locked, so only the angle changes, to see how cosine similarity depends purely on direction. This is why embeddings are compared by angle, not raw length.',
});
...
registerScene({
  ...
  goals: [
    goal('Make the vectors orthogonal (cos θ ≈ 0), to see how a zero dot product marks two directions sharing no signal',
    ...
    goal('Make them nearly identical in direction (cos θ > 0.97), to see how a near-1 cosine flags two vectors carrying almost the same meaning',
      (s) => cos(s.a, s.b) > 0.97,
      { xp: 20, hold: 500, tag: 'sign vs angle',
        focus: 'A large positive dot product means a small angle — the arrows point the same way.' }),
    goal('Make them oppose each other (cos θ < −0.9), to see how a negative dot product flags directly conflicting signals',
      (s) => cos(s.a, s.b) < -0.9,
      { xp: 20, hold: 500, tag: 'sign vs angle',
        focus: 'A negative dot product means an obtuse angle — the arrows point against each other.' }),
  ],
  caption: 'Drag both tips to see how the dot product’s sign tracks the angle: positive under 90°, zero at exactly 90°, negative beyond — the same three-way read a similarity score gives you.',
});
...
registerScene({
  ...
  goals: [
    goal('Lengthen a until a·b > 8 while staying aligned (cos θ > 0.8), to see how a dot product scales with magnitude even when direction is fixed',
    ...
    goal('Flip the sign: make a·b < 0 (stretching can’t — turn b past 90°), to see how only rotation, never scaling, flips a dot product’s sign',
      (s) => dot(s.a, s.b) < 0,
      { xp: 20, tag: 'sign vs angle',
        focus: 'Length never changes a sign. Only rotating past a right angle makes the dot product negative.' }),
    goal('Strongly aligned yet small: cos θ > 0.9 AND a·b < 1.5, to see how cosine similarity ignores magnitude entirely',
  ...
  caption: 'Slide a along its dashed line — only its length changes, to see how magnitude moves a·b while cosine similarity stays locked on direction alone. To flip the sign you must turn b.',
});
...
registerScene({
  ...
  goals: [
    goal('Aim the query so document 2 is the top match, to see how retrieval always returns the highest-cosine document',
      (s) => argmaxDoc(s.q) === 1,
      { xp: 20, tag: 'cosine similarity',
        focus: 'Retrieval returns the highest-cosine document. Point the query toward d2’s direction.' }),
    goal('Find a query where d2 and d3 are the top two, to see how top-k retrieval ranks by cosine, not just picks a single winner',
    ...
    goal('Make d4 the least similar while keeping d1 positive, to see how a negative-cosine document falls out of relevance while a positive one stays in play',
  ...
  caption: 'Rotate the query to see how the bars re-rank by cosine similarity — this is exactly how semantic search finds the closest match to an embedding query.',
});
...
registerScene({
  ...
  goals: [
    goal('Concentrate attention: push one key above 0.8, to see how aligning the query with one key sharpens softmax attention onto it',
    ...
    goal('Flatten attention: raise the temperature until every key sits under 0.4, to see how temperature controls how peaked or spread attention becomes',
      (s) => {
    ...
    goal('Focus on key 3 specifically: its weight above 0.9, to see how one dominant dot product can pull nearly all the attention weight to it',
  ...
  caption: 'Watch each key’s dot product with the query become an attention weight via softmax, to see how cooling the temperature sharpens focus onto a single key.',
});
...
registerScene({
  ...
  goals: [
    goal('Hit the dot-product target a·b = T (±0.4) and hold, to see how balancing length and angle together sets the score',
    ...
    goal('Hit the cosine target cos θ = C (±0.03) and hold, to see how cosine similarity depends on angle alone, never length',
    ...
    goal('Hit the angle target θ = A° (±5°) and hold, to see how the angle alone decides a dot product’s sign',
  ...
  caption: 'No hints now: hit each target — the dot product, the cosine, and the angle that all describe how these vectors relate — and hold steady. This is the exam.',
});
