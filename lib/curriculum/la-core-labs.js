...
INTERACTIVES.eigSweep = function(stage){
  ...
  const ctx = F.ctx, W = F.W, H = F.H;
  const P = plane(ctx, W, H, 40, W / 2, H / 2);
  const M = [2, 1, 1, 2];
  let t = 0, raf = 0;
  function draw(){
    clearBg(ctx, W, H); P.grid();
...
INTERACTIVES.eigPowerLab = function(stage, api){
  ...
  const STARTS = [[-0.7, 0.95], [0.9, -0.5], [-1, -0.2]];
  let mi = 0, si = 0, n = 0, trail = [], timer = null, counted = false, convergedRuns = 0;
  function apply(M, v){ return { x: M[0] * v.x + M[1] * v.y, y: M[2] * v.x + M[3] * v.y }; }
  function norm(v){ const k = Math.hypot(v.x, v.y) || 1; return { x: v.x / k, y: v.y / k }; }
  ...
  const m = api.missions([
    ...
    {text:'Prove it is start-independent: press New start and converge a SECOND time', xp:25, check:s=>s.convergedRuns>=2},
  ...
  function draw(){
    ...
    L.readout.innerHTML = 'step ' + n + '<br>angle to [1,1] = <span style="color:' + (angDom < 2 ? '#2dd4a0' : '#cdd4f0') + '">' + angDom.toFixed(1) + '°</span>' + (angDom < 2 ? '  ✓ CONVERGED' : '') + '<br>runs converged: ' + convergedRuns;
    m.update({ angDom, n, mi, convergedRuns });
  }
  function step(){
  ...
  newBtn.onclick = () => { si = (si + 1) % STARTS.length; u = normStart(STARTS[si]); n = 0; trail = []; counted = false; draw(); };
...
INTERACTIVES.eigTraceDet = function(stage, api){
  ...
  api.predict({
    prompt: 'For a 2×2, \\(\\lambda_1+\\lambda_2=\\operatorname{trace}\\) and \\(\\lambda_1\\lambda_2=\\det\\). A matrix has trace 5 and det 6. Predict its <b>larger</b> eigenvalue.',
    input: true, placeholder: 'larger λ', answer: 3, tol: 0.1, unit: '',
    reveal: 'The eigenvalues are the roots of \\(\\lambda^2 - 5\\lambda + 6 = (\\lambda-2)(\\lambda-3) = 0\\): λ = 3 and 2. The larger is <b>3</b>.',
  });

  const m = api.missions([
    {text:'Make the eigenvalues sum to 6: set the trace a + d = 6', xp:20, check:s=>Math.abs(s.T-6)<0.1},
    {text:'Make them multiply to 8 with two DISTINCT eigenvalues: det = 8, discriminant &gt; 0', xp:25, check:s=>Math.abs(s.D-8)<0.15 && s.disc>0.05},
  ...
  function draw(){
    ...
    // faint eigen-axis lines (symmetric ⇒ real perpendicular axes when disc>0)
    L.ctx.save();
    L.ctx.setLineDash([6, 6]); L.ctx.lineWidth = 1.2; L.ctx.strokeStyle = 'rgba(255,201,77,.18)';
    L.ctx.beginPath(); L.ctx.moveTo(P.sx(-5), P.sy(-5)); L.ctx.lineTo(P.sx(5), P.sy(5)); L.ctx.stroke();
    L.ctx.beginPath(); L.ctx.moveTo(P.sx(-5), P.sy(5)); L.ctx.lineTo(P.sx(5), P.sy(-5)); L.ctx.stroke();
    L.ctx.restore();
    ...
    L.readout.innerHTML = 'trace = a + d = ' + T.toFixed(2) + '<br>det = ad − b² = ' + D.toFixed(2) + '<br>λ₁ = ' + l1.toFixed(2) + '   λ₂ = ' + l2.toFixed(2);
    m.update({ T, D, disc, l1, l2 });
  }
