...
import {
  ...
  grid, vector, point, segment, label, angleArc,
  goal,
} from '../../scene/index.js';
import {
  angleDeg, dot, norm, scale, matApply,
  eigSym2, eigenMatrix, isAligned, eigRatio,
  minMagConstraint, circleConstraint, covOf, projectedVariance,
} from './vec-math.js';
...
registerScene({
  ...
  goals: [
    goal('Find the STRETCH eigen-direction: drag v so Mv points the same way, ×3 as long',
    ...
    goal('Find the OTHER eigen-direction: Mv points the same way, but UNCHANGED in length (λ=1)',
    ...
    goal('Now break it: pick a direction where M rotates v by more than 15°',
...
registerScene({
  ...
  goals: [
    goal('Reproduce the textbook matrix: set a = 4 and d = 3 (b stays free) — read the eigenvalues off the diagonal',
    ...
    goal('Confirm it: with a ≈ 4, align v with î — that direction is ALWAYS an eigenvector of a triangular matrix',
    ...
    goal('Now push b past 3 while a stays at 4 and d at 3 — the shear grows but NEITHER eigenvalue moves',
...
registerScene({
  ...
  goals: [
    goal('Find the STRETCH direction (positive λ ≈ 3.30): Mv points the SAME way as v',
    ...
    goal('Find the FLIP direction (negative λ ≈ −0.30): Mv points OPPOSITE to v',
    ...
    goal('Break alignment: pick a direction where M rotates v by more than 30°',
...
registerScene({
  ...
  goals: [
    goal('Send v1 to the STRETCH eigen-direction (ratio ≈ 3)',
    ...
    goal('Send v2 to the OTHER eigen-direction (ratio ≈ 1)',
    ...
    goal('With both aligned, confirm they sit at 90° — a symmetric matrix’s eigenvectors are always perpendicular',
...
registerScene({
  ...
  goals: [
    ...
    goal('Find an in-between angle where the variance equals the AVERAGE of the two extremes',
...
registerScene({
  ...
  goals: [
    goal('Find the FIRST eigen-direction (dashed target 1): align v with it and hold',
    ...
    goal('Find the SECOND eigen-direction (dashed target 2): align v with it and hold',
