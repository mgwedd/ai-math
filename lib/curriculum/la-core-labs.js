...
INTERACTIVES.matColumns = function(stage){
  ...
  function draw(){
    ...
    ctx.fillStyle='#cdd4f0'; ctx.font='600 12px '+FONT();
...
INTERACTIVES.matLinearity = function(stage){
  ...
  function draw(){
    ...
    ctx.textAlign='left'; ctx.fillStyle='#cdd4f0'; ctx.font='700 12px '+FONT();
    ctx.fillText('Mv = x·col1 + y·col2', 16, 24);
  }
  draw();
  function loop(){ t+=0.02; draw(); raf=requestAnimationFrame(loop); }
  raf=requestAnimationFrame(loop);
...
INTERACTIVES.matGallery = function(stage){
  ...
  function panel(ox,oy,M,name){
    ...
    ctx.font='600 11px '+FONT(); ctx.fillStyle='#8b93b8';
...
INTERACTIVES.matrixReadoff = function(stage, api){
  ...
  api.predict({
    ...
    choices: ['90° rotation counterclockwise', 'reflection over the x-axis', 'horizontal shear'],
    answer: 0,
    reveal: 'Columns are landing spots: \\(\\hat\\imath\\to[0,1]\\) (right becomes up) and \\(\\hat\\jmath\\to[-1,0]\\) (up becomes left) — a quarter-turn counterclockwise, \\(M=\\begin{bmatrix}0&-1\\\\1&0\\end{bmatrix}\\).',
  });
