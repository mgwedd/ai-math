...
registerLesson({
  ...
  <p>• <strong>Forward pass</strong> (World 1): the input \\([x_1, x_2]\\) is a vector; each layer is a <em>matrix multiply</em> then a nonlinearity. \\(h = \\tanh(W_1 \\cdot x + b_1)\\), then \\(\\hat{y} = \\sigma(W_2 \\cdot h + b_2)\\).<br>
  • <strong>Loss</strong> (World 3): binary cross-entropy — how surprised the model is by the true label.<br>
  • <strong>Backprop</strong> (World 2): the chain rule, run backwards through the net, hands every weight its gradient \\(\\dfrac{\\partial \\text{loss}}{\\partial w}\\).<br>
  • <strong>Update</strong>: \\(w \\leftarrow w - \\eta \\cdot \\nabla w\\). Gradient <em>descent</em>. Repeat for E epochs.</p>
...
INTERACTIVES.mlcapstone = function(stage, api){
  ...
  chips(L.ctrl, 'TRAIN', ['▶ Train', '⏹ Stop', '↺ New data'], (i, _btn, _r)=>{
