...
export const pool =
  ...
  (g.__aimathPool = new Pool({
    ...
    // TLS, strictest available option first:
    // 1. DATABASE_SSL_CA = PEM of the provider's CA (e.g. Supabase's
    //    prod CA cert) -> full chain verification against that CA.
    // 2. DATABASE_SSL=no-verify -> encrypted but unverified (pragmatic
    //    fallback for providers with self-signed chains).
    // 3. neither -> whatever the connection string's sslmode says.
