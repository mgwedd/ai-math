...
INTERACTIVES.limitsChallenge = function(stage, api){
  ...
  api.predict({
    prompt: 'Plug x = 3 into <code>(x²−9)/(x−3)</code> and you get 0/0. Factor it and predict the limit <b>lim<sub>x→3</sub></b>.',
...
INTERACTIVES.derivEstimate = function(stage, api){
  ...
  api.predict({
    prompt: 'For <code>f(x) = x³/3</code> the derivative is <code>f′(x) = x²</code>. Predict <b>f′(2)</b>.',
...
INTERACTIVES.rulesChallenge = function(stage, api){
  ...
  api.predict({
    prompt: 'Differentiate <code>3x²</code> with the power rule, then evaluate at x = 2. Predict <b>the value of the derivative there</b>.',
...
INTERACTIVES.chainChallenge = function(stage, api){
  ...
  api.predict({
    prompt: 'A 2-stage chain has local slopes <code>du/dw = 2</code> and <code>dL/du = 3</code>. The chain rule says dL/dw is their <b>product</b>. Predict <b>dL/dw</b>.',
...
INTERACTIVES.optimClassify = function(stage, api){
  ...
  api.predict({
    prompt: 'For <code>f(x) = x⁴/4 − x²</code>, f′(x) = x³ − 2x = x(x²−2). How many <b>critical points</b> (values where f′ = 0) does it have?',
...
INTERACTIVES.graddescChallenge = function(stage, api){
  ...
  api.predict({
    prompt: 'On the bowl <code>f(x) = x²/2</code> the gradient-descent update is <code>x ← (1 − lr)·x</code>. With <b>lr = 2.1</b>, does x settle to the minimum or blow up?',
  ...
  note(L.ctrl, 'Start is fixed at x = 3. Pick a learning rate, then Step or Auto-run. Watch <code>x ← (1−lr)·x</code>: below 1 it crawls in, exactly 1 snaps to the bottom, between 1 and 2 it overshoots but still shrinks, at 2 it bounces forever, and beyond 2 it explodes.');
  reset();
  return () => stopAuto();
};
...
INTERACTIVES.integralsChallenge = function(stage, api){
  ...
  api.predict({
    prompt: 'The area under <code>f(x) = x</code> from 0 to 2 is a triangle. Predict <b>∫₀² x dx</b>.',
