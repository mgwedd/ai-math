...
INTERACTIVES.derSecantTangent = function(stage){
  ...
  function draw(){
    ...
    P.fn(f, '#7c5cff', 3);
    const h = 0.05 + 1.2 * (0.5 + 0.5 * Math.cos(t));   // cap h so the moving partner point stays on-canvas
...
INTERACTIVES.derSlopeFn = function(stage){
  ...
  function draw(){
    clearBg(ctx, W, H);
    const a = 1.6 * Math.sin(t * 0.6);   // amplitude keeps both dots inside their panels
    const s = fp(a);
    // top panel — clipped to its strip so its grid/curve/tangent can't bleed into the bottom
    ctx.save(); ctx.beginPath(); ctx.rect(0, 0, W, 205); ctx.clip();
    ...
    ctx.restore();
    // bottom panel — clipped to its strip
    ctx.save(); ctx.beginPath(); ctx.rect(0, 205, W, H - 205); ctx.clip();
    PB.grid(); PB.fn(fp, '#00d4ff', 3);
    PB.dot(a, s, 6, '#00d4ff');
    ctx.restore();
    // dashed vertical link (unclipped): slope up top → height down below
...
INTERACTIVES.derLocalLinear = function(stage){
  ...
  function draw(){
    clearBg(ctx, W, H);
    const zoom = 1 + 15 * (0.5 + 0.5 * Math.sin(t));   // up to ×16 so the arc unmistakably flattens onto its tangent
...
INTERACTIVES.derivTrace = function(stage, api){
  ...
  api.predict({
    prompt:'You\'ll read the tangent slope of \\(f(x)=\\tfrac12 x^2\\) at several \\(x\\)-values. If you plot each slope reading as a point, what shape will the points form?',
  ...
  const m = api.missions([
    ...
    {text:'Reveal f′ and see every stamp land right on it', xp:20, check:s=>s.revealed && s.allX && s.onCurve},
  ]);
  ...
  note(L.ctrl, 'Move to an x-value, read the tangent slope f′(x), and press Stamp to drop it as a point at height f′(x) on the same axes. Stamp x = −2, −1, 0, 1, 2 and the dots trace out f′(x) = x — a straight line. The derivative is its own function.');
  draw();
};
