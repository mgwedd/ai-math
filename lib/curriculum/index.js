...
registerLesson({
  ...
  expositions:[
    {key:'area', title:'The determinant is an area factor', figure:'detArea', caption:'Feed the unit square through the matrix and it becomes a parallelogram. Its area is \\(|\\det| = |ad-bc|\\) — how much the transformation stretches every area.'},
    {key:'sign', title:'The sign is orientation', figure:'detSign', caption:'As \\(\\det\\) falls through 0 the shape flattens onto a line (no inverse); past 0 it returns <b>mirror-reversed</b>. \\(\\det>0\\) keeps orientation, \\(\\det<0\\) flips it, \\(\\det=0\\) destroys it.'},
    {key:'product', title:'Determinants multiply', figure:'detProductFig', caption:'Stack two transforms and their area factors multiply: \\(\\det(B\\cdot A) = \\det(A)\\,\\det(B)\\).'},
  ],
  labs:[
    ...
    {key:'multiply', title:'det(B·A) = det(A) · det(B)', interactive:'detProduct',
     intro:'<p>Compose two transforms and watch the area factors <strong>multiply</strong>: \\(\\det(B\\cdot A) = \\det(A)\\,\\det(B)\\). Build an area-preserving chain, an orientation flip, and the biggest blow-up you can.</p>'},
    {key:'shear', title:'Shear preserves area', interactive:'detShear',
     intro:'<p>Two moves on a parallelogram: <strong>shearing</strong> (sliding one column along the other) leaves the area — and \\(\\det\\) — unchanged, while <strong>scaling</strong> a column changes it directly. This is exactly why elimination never changes a determinant.</p>'},
  ],
  quiz:[
