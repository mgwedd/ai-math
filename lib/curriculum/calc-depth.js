...
LESSONS.push({
  ...
  <p>• A convex function has <strong>no local minima other than the global one</strong>. Every flat spot is THE bottom.<br>
  • So gradient descent — from <em>any</em> start — is guaranteed to find the global optimum.<br>
  • Convexity is preserved by friendly operations: a <strong>sum of convex functions is convex</strong>, and so is a max of convex functions (that is why the SVM hinge loss is convex).</p>
  <p>Real neural-network losses are wildly <strong>non-convex</strong> — ridges, saddles, many valleys — which is exactly why initialization, momentum, restarts, and learning-rate schedules matter. Convexity is the dividing line between optimization that "just works" and optimization as an art.</p>`,
  ml:`The reliable, classical corner of ML lives here: <b>linear/ridge regression</b> (convex MSE), <b>logistic regression</b> and <b>softmax cross-entropy</b> (convex in their weights), and <b>SVMs</b> (convex hinge loss) all have a single global optimum you can reach every time. Deep nets give that up — but they are trained with the same downhill machinery, trusting that in very high dimensions the many local minima are empirically about equally good. Knowing whether your loss is convex tells you whether to expect one answer or many.`,
  deeper:[
...
LESSONS.push({
  ...
  <p>On a true quadratic (like squared-error loss) the parabola IS the function, so Newton lands on the exact minimum in <strong>one step</strong>. Near any minimum it converges <em>quadratically</em> — the error roughly squares each step. The catch: it needs f″. In many dimensions that is the <strong>Hessian</strong> matrix of all second partials, and its eigenvalues (World 1!) classify the point: all positive → bowl (min), all negative → dome (max), mixed signs → <strong>saddle</strong>.</p>`,
  ml:`Full Newton is too costly for deep nets — the Hessian of a billion-parameter model has 10¹⁸ entries — but its spirit is everywhere. <b>Adam</b> rescales each parameter by an estimate of its own curvature; <b>L-BFGS</b> builds a cheap low-rank Hessian approximation; <b>natural gradient</b> and <b>K-FAC</b> precondition with the Fisher matrix. Curvature also explains GD’s pain: when the Hessian is <b>ill-conditioned</b> (one eigenvalue ≫ another) gradient descent zig-zags down a narrow ravine, which is exactly what momentum and warmup are fighting.`,
  deeper:[
