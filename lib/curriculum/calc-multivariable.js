...
registerLesson({
  ...
  learn:`<p>World 2's chain rule was \\((f \\circ g)'(x) = f'(g(x)) \\cdot g'(x)\\) — multiply the slopes along a chain. When each stage maps <em>vectors to vectors</em>, "the slope" becomes a matrix: the <strong>Jacobian</strong>, the grid of all partials of every output with respect to every input.</p>
  ...
  <p>That is not an analogy for backprop — it is backprop. A neural net is a deep composition \\(x_n = f_n(\\cdots f_2(f_1(x)))\\); the gradient of the scalar loss w.r.t. the inputs is one long product of layer Jacobians. Two facts make it practical:</p>
...
INTERACTIVES['mv-lagsolve'] = function(stage, api){
  ...
  const f  =(x,y)=> x + 2*y;
  const gf =(_x,_y)=> [1, 2];              // ∇f (constant field — nice teaching case)
