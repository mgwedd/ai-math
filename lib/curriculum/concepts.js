...
export const TAG_ALIASES = {
  // Proof-literacy micro-course (proofs.js: proof-read, proof-contra,
  // proof-induction, proof-counterexample)
  'statement anatomy': 'direct-proof',
  'hypothesis and conclusion': 'direct-proof',
  'definition unpacking': 'direct-proof',
  'direct proof structure': 'direct-proof',
  'quantifier order': 'direct-proof',
  'universal quantifier': 'direct-proof',
  'existential witness': 'direct-proof',
  'proving for all': 'direct-proof',
  'contrapositive equivalence': 'proof-by-contraposition',
  'contrapositive when to use': 'proof-by-contraposition',
  'converse is not equivalent': 'proof-by-contraposition',
  'negation of implication': 'proof-by-contraposition',
  'proof by contradiction setup': 'proof-by-contraposition',
  'irrationality of root two': 'proof-by-contraposition',
  'induction base case': 'mathematical-induction',
  'inductive step': 'mathematical-induction',
  'induction needs base and step': 'mathematical-induction',
  'broken induction horses': 'mathematical-induction',
  'strong induction': 'mathematical-induction',
  'gauss triangular sum': 'mathematical-induction',
  'geometric series sum': 'mathematical-induction',
  'one counterexample refutes': 'counterexamples',
  'examples never prove': 'counterexamples',
  'edge cases': 'counterexamples',
  'conjecture repair': 'counterexamples',
  'double descent': 'counterexamples',
  'correlation zero independence': 'counterexamples',
  ...
  // Unsupervised module (ml-unsupervised.js: ml-kmeans, ml-gmm, ml-pca)
  'what k-means optimizes': 'k-means',
  'assignment step': 'k-means',
  'centroid update': 'k-means',
  'lloyd iteration order': 'k-means',
  'local minima': 'k-means',
  'k-means++ init': 'k-means',
  'computing a centroid': 'k-means',
  'choosing k elbow': 'k-means',
  'convergence guarantee': 'k-means',
  'mixture model purpose': 'gaussian-mixture-models',
  'responsibilities': 'gaussian-mixture-models',
  'soft vs hard assignment': 'gaussian-mixture-models',
  'computing a responsibility': 'gaussian-mixture-models',
  'EM step order': 'gaussian-mixture-models',
  'EM monotonic likelihood': 'gaussian-mixture-models',
  'M-step updates': 'gaussian-mixture-models',
  'EM local optima': 'gaussian-mixture-models',
  'kmeans as EM limit': 'gaussian-mixture-models',
  'variance maximization': 'principal-component-analysis',
  'principal component eigenvector': 'principal-component-analysis',
  'variance decomposition': 'principal-component-analysis',
  'variance explained ratio': 'principal-component-analysis',
  'computing variance explained': 'principal-component-analysis',
  'pca via svd': 'principal-component-analysis',
  'centering the data': 'principal-component-analysis',
  'pca scale sensitivity': 'principal-component-analysis',
  'pca procedure order': 'principal-component-analysis',
  'reconstruction error': 'principal-component-analysis',
  // rl-mdp's three migrated quiz/capstone tags (lib/curriculum/rl.js,
  // lib/curriculum/scenes/rl-mdp.js) — mirrors the capstone's g.tag strings.
...
/* --- Proof-literacy micro-course (lib/curriculum/proofs.js) --- */
registerConcept({
  id: 'direct-proof', world: 'pre', title: 'Reading & writing direct proofs',
  wikipedia: 'Direct_proof',
  prereqs: [],
});
registerConcept({
  id: 'proof-by-contraposition', world: 'pre', title: 'Contrapositive & proof by contradiction',
  wikipedia: 'Proof_by_contrapositive',
  prereqs: ['direct-proof'],
});
registerConcept({
  id: 'mathematical-induction', world: 'pre', title: 'Mathematical induction',
  wikipedia: 'Mathematical_induction',
  prereqs: ['direct-proof', 'summation-notation'],
});
registerConcept({
  id: 'counterexamples', world: 'pre', title: 'Counterexamples & conjecture repair',
...
/* --- Unsupervised module (lib/curriculum/ml-unsupervised.js:
       ml-kmeans, ml-gmm, ml-pca) --- */
registerConcept({
  id: 'k-means', world: 'ml', title: 'Clustering: k-means & Lloyd\'s algorithm',
  wikipedia: 'K-means_clustering',
  prereqs: ['vectors', 'vector-operations'],
});
registerConcept({
  id: 'gaussian-mixture-models', world: 'ml', title: 'Gaussian mixtures & expectation-maximization',
  wikipedia: 'Mixture_model',
  prereqs: ['probability-distributions', 'maximum-likelihood-estimation', 'k-means'],
});
registerConcept({
  id: 'principal-component-analysis', world: 'ml', title: 'Principal component analysis',
  wikipedia: 'Principal_component_analysis',
  prereqs: ['eigenvectors-eigenvalues', 'covariance-correlation', 'vector-projection', 'svd'],
});
...
/* --- RL world, lesson 1 (lib/curriculum/rl.js: rl-mdp). NOTE: this
       registry's WORLDS enum tracks the four KB tracks (pre/la/calc/prob/
       ml), separate from the app's per-lesson `world` string — rl-mdp's
       lesson.world is 'rl', a new app-level track (see WORLD_META in
       lib/engine.js); its concepts still file under world:'ml' here, same
       as every other ML-adjacent concept. Only rl-mdp's own three quiz/
       capstone tags get a concept; later RL lessons (Q-learning, policy
       gradient) add their own when they land. */
registerConcept({
  id: 'markov-decision-process', world: 'ml', title: 'Markov decision processes',
  wikipedia: 'Markov_decision_process',
  prereqs: ['markov-chains', 'expectation-random-variables'],
});
registerConcept({
  id: 'bellman-equation', world: 'ml', title: 'Value functions & the Bellman equation',
  wikipedia: 'Bellman_equation',
  prereqs: ['markov-decision-process'],
});
registerConcept({
  id: 'discount-factor', world: 'ml', title: 'The discount factor (γ) & effective horizon',
  wikipedia: 'Bellman_equation',
  prereqs: ['markov-decision-process'],
});
registerConcept({
  id: 'value-iteration', world: 'ml', title: 'Value iteration',
  wikipedia: 'Dynamic_programming',
  prereqs: ['bellman-equation'],
});

/* --- RL world, lesson 2 (lib/curriculum/rl.js: rl-qlearning, P2 wave K).
       Same world:'ml' filing convention as rl-mdp's three concepts above —
       this lesson's own three migrated quiz/capstone tags. */
registerConcept({
  id: 'temporal-difference-learning', world: 'ml', title: 'Temporal-difference learning & the TD error',
  wikipedia: 'Temporal_difference_learning',
  prereqs: ['bellman-equation'],
});
registerConcept({
  id: 'q-learning', world: 'ml', title: 'Q-learning (model-free, off-policy TD control)',
  wikipedia: 'Q-learning',
  prereqs: ['temporal-difference-learning', 'value-iteration'],
});
registerConcept({
  id: 'exploration-exploitation', world: 'ml', title: 'The exploration-exploitation trade-off',
  wikipedia: 'Multi-armed_bandit',
  prereqs: ['markov-decision-process'],
});
