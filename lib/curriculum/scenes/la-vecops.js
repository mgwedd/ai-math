...
/* Micro-idea: b − a is the arrow FROM point a TO point b; its length is the
   ...
   ratio a shrunk vector could fake. */
registerScene({
  ...
  goals: [
    ...
    goal('Aim the residual straight up: b−a has x ≈ 0 and length ≥ 1.5',
      (s) => Math.abs(sub(s.b, s.a).x) < 0.15 && sub(s.b, s.a).y >= 1.5,
      { xp: 20, hold: 400, tag: 'vector addition',
        focus: 'A purely-vertical residual means only ONE coordinate is off — isolating one axis is what it looks like when a model has learned every feature except one.' }),
    goal('Build a 3-4-5 walk: make the distance ‖b−a‖ exactly 5',
