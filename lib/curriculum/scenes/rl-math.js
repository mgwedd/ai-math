...
/* ================================================================
   P2 wave K ADDITIVE EXPORTS — model-free TD control (rl-qlearning).
   rl-mdp's exports above are UNTOUCHED; everything below is new and
   shared by lib/curriculum/scenes/rl-qlearning.js only.
   ================================================================ */
import { makeRng } from '../../scene/index.js';

/** The SAME discount factor rl-mdp.js uses (its own local GAMMA=0.9) —
 * named here so rl-qlearning's Q-learning runs against the identical MDP
 * (same grid, same gamma) as rl-mdp's value iteration, making the two
 * lessons' policies directly comparable. */
export const CANONICAL_GAMMA = 0.9;

function argmaxIndex(qs) {
  let best = 0, bestVal = -Infinity;
  for (let i = 0; i < qs.length; i++) if (qs[i] > bestVal + 1e-9) { bestVal = qs[i]; best = i; }
  return best;
}

/**
 * Tabular Q-learning: replay `episodes` epsilon-greedy episodes from START,
 * deterministically, driven by a seeded PRNG (CONTRACT determinism — never
 * Math.random/Date). Each step:
 *   delta = r + gamma * max_a' Q(s',a') - Q(s,a)     (max term is 0 if s' terminal —
 *                                                      absorbing, never bootstrapped)
 *   Q(s,a) <- Q(s,a) + alpha * delta
 * Action selection is epsilon-greedy: with probability eps, take a uniformly
 * random action (drawn from the SAME rng stream, so a full replay is a pure
 * function of (seed, alpha, eps, episodes) alone); otherwise the current
 * greedy (argmax) action, ties broken to the first action in ACTIONS order.
 * An episode ends on reaching either terminal, or after `maxSteps` (default
 * 60 — this grid's optimal path is 6 steps, so 60 is generous headroom and
 * never a binding cap at the fixed-alpha scenes' DEMO_ALPHA=0.5. The
 * capstone's low-alpha corner (e.g. alpha=0.05, eps=0) CAN hit the cap on an
 * early episode before the greedy tie-break has self-corrected — that is a
 * legitimate episode timeout, not a bug: every step up to the cap still
 * applies the correct TD update, the episode just moves on to the next one
 * without having reached a terminal yet).
 * @returns {number[][][]} Q, ROWS x COLS x 4 (ACTIONS order); wall cells stay
 *   all-zero (never read, mirrors valueIterate's convention).
 */
export function qLearn(seed, alpha, eps, episodes, maxSteps = 60) {
  ...
  for (let ep = 0; ep < n; ep++) {
    ...
    for (let t = 0; t < maxSteps; t++) {
      const aIdx = rng() < eps ? Math.floor(rng() * 4) : argmaxIndex(Q[s.r][s.c]);
      const a = ACTIONS[aIdx];
      const ns = succ(s.r, s.c, a);
      const term = isTerminal(ns.r, ns.c);
      const maxNext = term ? 0 : Math.max(...Q[ns.r][ns.c]);
      const delta = rewardOf(ns.r, ns.c) + CANONICAL_GAMMA * maxNext - Q[s.r][s.c][aIdx];
      Q[s.r][s.c][aIdx] += alpha * delta;
      if (term) break;
      s = ns;
    }
  }
...
/** The greedy action Q-learning has learned for (r,c): argmax_a Q(r,c,a),
 * ties broken to the first action — same tie-break convention as
 * greedyAction(V,r,c), so the two policies are directly comparable action-
 * for-action (rl-qlearning's "compare policies" scene). */
export function qGreedyAction(Q, r, c) {
  return ACTIONS[argmaxIndex(Q[r][c])];
}
