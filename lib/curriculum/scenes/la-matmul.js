...
registerScene({
  ...
  goals: [
    goal('B is fixed. Drag A so C = B∘A sends î to ★₁ (apply A, then B), to see how tracking a point through two applied-in-order matrices previews exactly how a signal is chased through stacked network layers',
    ...
    goal('Now send ĵ to ★₂ the same way — through A, then through the fixed B, to see how every column of a composed matrix runs through the identical two-step pipeline',
...
registerScene({
  ...
  goals: [
    goal('Drag v so applying Sh FIRST, then R, lands it on ★₁, to see how the order two transforms are applied in changes where a point ends up',
    ...
    goal('Now drag v so applying R FIRST, then Sh, lands the SAME v on ★₂ — a different spot for the same two matrices, to see how swapping the order sends the identical vector somewhere else — exactly why matrix multiplication doesn’t commute',
...
registerScene({
  ...
  goals: [
    goal('Drag A’s column 2 until entry (1,2) of B·A — row 1 of B dotted with column 2 of A — reads 3, to see how a single entry of a matrix product depends on just one row and one column, not the whole matrices',
    ...
    goal('Drag A’s column 1 until entry (2,1) of B·A reads −2, to see how that same entry-is-a-dot-product rule isolates a different single column',
...
registerScene({
  ...
  goals: [
    goal('Slide k so C = B∘A(k)’s second column lands on ★, to see how one scalar parameter inside a layer can steer the entire composed transformation',
    ...
    goal('Now slide k so entry (2,2) of B∘A reads −1.5 exactly, to see how a product’s entry can sometimes be read straight off a parameter instead of recomputed',
...
registerScene({
  ...
  goals: [
    goal('However you split the two layers, land W’s column 1 on ★₁, to see how two stacked linear layers always collapse into one matrix without a nonlinearity between them',
    ...
    goal('And land W’s column 2 on ★₂ — the same single-matrix rule, to see how any split of the work between two linear layers produces the identical combined matrix W',
...
/* Micro-idea: B·A ≠ A·B for most pairs — swapping two network LAYERS
   ...
   (not the insight), a near-zero matrix drags the gap toward 0 for free,
   and — the review-caught case — leaving EITHER matrix at the identity
   also trivially commutes with anything (I commutes with everything), so
   the commuting goal requires: both real (MIN_MAG floor), genuinely
   DIFFERENT from each other (col-distance floor), AND each individually
   far from the identity (its own col-distance-from-I floor) — not just
   equal, collapsed, or one of them left untouched. */
const gapOf = (s) => {
...
const distFromIdentity = (c1, c2) => mag(sub(c1, I1)) + mag(sub(c2, I2));
const bothNonIdentity = (s) => distFromIdentity(s.aCol1, s.aCol2) > 0.4
  && distFromIdentity(s.bCol1, s.bCol2) > 0.4;
registerScene({
  id: 'matmul.commute',
  lesson: LESSON,
  space: 'plane2',
  ...
  goals: [
    goal('You saw one fixed pair disagree in the last scene — now prove it’s the rule, not the exception: drag matrices of your own choosing to gap > 1.5',
    ...
    goal('Now find two DIFFERENT, real matrices that DO commute (gap < 0.05) — e.g. two rotations — the exception, not the rule',
      (s) => bothReal(s) && distinctOf(s) > 0.4 && bothNonIdentity(s) && gapOf(s) < 0.05,
  ...
  caption: 'Drag all four columns — two for A, two for B. The readout tracks gap = ‖B·A − A·B‖: watch it move as you pick different pairs, then hunt for the rare pair that drives it to zero.',
});
...
registerScene({
  ...
  goals: [
    goal('Place A (both columns) to match its target — this is what gets applied FIRST (±0.2) and hold, to see how the first-applied matrix in a composition fixes the first stage of the transformation',
    ...
    goal('Place B (both columns) to match its target — applied SECOND (±0.2) and hold, to see how the exam target locks in a genuinely order-dependent function — reordering two network layers is never a free rewrite',
    ...
    goal('With both placed, land C’s column 1 on ◆ (±0.2) and hold — that entry is B applied to A’s column 1, to see how the composed matrix’s column falls out for free once both factors are fixed',
