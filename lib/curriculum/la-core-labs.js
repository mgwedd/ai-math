...
/* ================================================================
   EXPOSITION FIGURES for la-vecops ("see it first" gallery).
   Same contract as the la-vectors figures above: build own canvas,
   draw once synchronously, return a cancel cleanup only if animated.
   ================================================================ */

/* vecopsAddAnim — tip-to-tail addition, animated: a translucent copy of b
   slides from the origin out to the tip of a while a green sum arrow fades
   in at the landing point. Shows a + b is order-independent by construction. */
INTERACTIVES.vecopsAddAnim = function(stage){
  const F = figCanvas(stage, 640, 300);
  const ctx = F.ctx, W = F.W, H = F.H;
  const P = plane(ctx, W, H, 46, 200, 188);   // origin low so the a+b tip label clears the top edge
  const a = { x: 3, y: 1 }, b = { x: 1, y: 2 };
  let t = 0, raf = 0;
  function draw(){
    clearBg(ctx, W, H); P.grid();
    const p = 0.5 + 0.5 * Math.sin(t);            // 0..1 walk phase
    P.arrow(0, 0, a.x, a.y, '#7c5cff', 4, 'a');
    P.arrow(0, 0, b.x, b.y, '#00d4ff', 4, 'b');
    // translucent copy of b, tail sliding along a as p goes 0 -> 1
    ctx.save();
    ctx.globalAlpha = 0.5;
    P.arrow(a.x * p, a.y * p, a.x * p + b.x, a.y * p + b.y, '#00d4ff', 3);
    ctx.restore();
    // the sum, fading in as the walk completes
    ctx.save();
    ctx.globalAlpha = p;
    P.arrow(0, 0, a.x + b.x, a.y + b.y, '#2dd4a0', 4, 'a+b');
    P.dot(a.x + b.x, a.y + b.y, 6, '#2dd4a0');
    ctx.restore();
    ctx.fillStyle = '#8b93b8'; ctx.font = '700 13px ' + FONT(); ctx.textAlign = 'left';
    ctx.fillText('walk a, then b → land at a + b', 20, H - 16);
  }
  draw();                                          // synchronous first paint
  function loop(){ t += 0.02; draw(); raf = requestAnimationFrame(loop); }
  raf = requestAnimationFrame(loop);
...
/* vecopsScale — a fixed vector a with a scalar c sweeping through -1..2,
   showing c·a can only stretch/shrink/flip ALONG a's own line, never rotate
   off it. Animated. */
INTERACTIVES.vecopsScale = function(stage){
  const F = figCanvas(stage, 640, 260);
  const ctx = F.ctx, W = F.W, H = F.H;
  ...
  function draw(){
    clearBg(ctx, W, H); P.grid();
    // faint full line through a — the line a scalar can never leave
    ctx.strokeStyle = 'rgba(124,92,255,.2)'; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(P.sx(-4 * a.x), P.sy(-4 * a.y)); ctx.lineTo(P.sx(4 * a.x), P.sy(4 * a.y)); ctx.stroke();
    const c = 0.5 + 1.5 * Math.sin(t);              // sweeps roughly -1..2
    const ca = { x: c * a.x, y: c * a.y };
    const col = c > 0 ? '#2dd4a0' : '#ff5c7a';
    P.arrow(0, 0, a.x, a.y, '#7c5cff', 3, 'a');
    P.arrow(0, 0, ca.x, ca.y, col, 4, 'c·a');
    P.dot(ca.x, ca.y, 6, col);
    ctx.fillStyle = col; ctx.font = '700 16px ' + FONT(); ctx.textAlign = 'left';
    ctx.fillText('c = ' + c.toFixed(2), 20, 30);
    ctx.fillStyle = '#8b93b8'; ctx.font = '700 12px ' + FONT();
    ctx.fillText('|c|>1 stretch · |c|<1 shrink · c<0 flip', 20, H - 16);
  }
  draw();                                          // synchronous first paint
  function loop(){ t += 0.02; draw(); raf = requestAnimationFrame(loop); }
  raf = requestAnimationFrame(loop);
...
/* vecopsSpanFig — two side-by-side lattices of c1*a + c2*b: independent
   vectors fill the whole plane; a dependent pair (b = 2a) collapses every
   combination onto a single line. Static. */
INTERACTIVES.vecopsSpanFig = function(stage){
  const F = figCanvas(stage, 640, 320);
  const ctx = F.ctx, W = F.W, H = F.H;
  clearBg(ctx, W, H);
  function lattice(P, a, b){
    for (let c1 = -3; c1 <= 3; c1++){
      for (let c2 = -3; c2 <= 3; c2++){
        P.dot(c1 * a.x + c2 * b.x, c1 * a.y + c2 * b.y, 2.5, 'rgba(0,212,255,.5)');
      }
    }
    P.arrow(0, 0, a.x, a.y, '#7c5cff', 3);
    P.arrow(0, 0, b.x, b.y, '#7fe7ff', 3);
  }
  // Each panel is clipped to its own half so its grid + lattice can't bleed
  // across the divider or off the canvas edge.
  function panel(x0, x1, ox, a, b, title){
    ctx.save();
    ctx.beginPath(); ctx.rect(x0, 0, x1 - x0, H); ctx.clip();
    const P = plane(ctx, W, H, 24, ox, H / 2);
    P.grid();
    lattice(P, a, b);
    ctx.restore();                                 // titles draw unclipped
    ctx.fillStyle = '#cdd4f0'; ctx.font = '700 13px ' + FONT(); ctx.textAlign = 'center';
    ctx.fillText(title, ox, 28);
  }
  // LEFT — independent a, b: the lattice fills the whole plane
  panel(0, 324, 165, { x: 1.4, y: 0.5 }, { x: 0.4, y: 1.4 }, 'independent → whole plane');
  // RIGHT — dependent a, b (b = 2a): the lattice collapses onto one line
  panel(326, W, 480, { x: 1, y: 0.6 }, { x: 2, y: 1.2 }, 'dependent (b = 2a) → a line');
  // divider between the two panels
  ctx.strokeStyle = 'rgba(255,255,255,.12)'; ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(325, 20); ctx.lineTo(325, H - 10); ctx.stroke();
};
...
/* ================== la-vecops — Lab 3: the span, filled and collapsed =========
   Concept: every c₁a + c₂b is the SPAN of a and b. Dragging b toward a's own
   line collapses the whole lattice of combinations from a 2-D fill down to a
   single line — the first hands-on encounter with linear dependence, keyed
   off the determinant of the two columns. */
INTERACTIVES.vecopsSpan = function(stage, api){
  const L = makeLab(stage);
  const a = {x:2, y:1}, b = {x:1, y:2};

  api.predict({
    prompt: 'You have \\(a = [2, 1]\\) and \\(b = [4, 2]\\). Can you reach the point \\([0, 3]\\) as a combination \\(c_1 a + c_2 b\\)?',
    choices: ['Yes — some c₁, c₂ reach it', 'No — it lies off their line'],
    answer: 1,
    reveal: 'b = 2a, so a and b are <b>dependent</b> — every c₁a + c₂b is just a multiple of a, landing somewhere on the single line through a. \\([0, 3]\\) is not on that line, so no c₁, c₂ can reach it.',
  });

  const m = api.missions([
    {text:'Collapse the span onto a line: make b parallel to a so |det| ≈ 0 (&lt; 0.15), both non-trivial (‖a‖, ‖b‖ &gt; 1)', xp:25,
      check:s => Math.abs(s.det) < 0.15 && s.na > 1 && s.nb > 1},
    {text:'Re-expand to a full 2-D lattice: push |det| ≥ 4', xp:20,
      check:s => Math.abs(s.det) >= 4},
    {text:'Tune the lattice cell to area exactly 2 (|det| = 2 ± 0.2)', xp:20,
      check:s => Math.abs(Math.abs(s.det) - 2) < 0.2},
  ]);

  const P = plane(L.ctx, L.W, L.H, 34);
  let dragging = null;
  function draw(){
    clearBg(L.ctx, L.W, L.H); P.grid();
    const det = a.x*b.y - a.y*b.x;
    const na = Math.hypot(a.x, a.y), nb = Math.hypot(b.x, b.y);
    const collapsed = Math.abs(det) < 0.15;
    const dotCol = collapsed ? '#ffc94d' : '#7fe7ff';
    for (let c1 = -2; c1 <= 2; c1 += 0.5){
      for (let c2 = -2; c2 <= 2; c2 += 0.5){
        P.dot(c1*a.x + c2*b.x, c1*a.y + c2*b.y, 2, dotCol);
      }
    }
    // faint parallelogram cell spanned by a and b
    L.ctx.beginPath();
    [[0,0], [a.x,a.y], [a.x+b.x,a.y+b.y], [b.x,b.y]].forEach((q,i) => {
      const sx = P.sx(q[0]), sy = P.sy(q[1]); i ? L.ctx.lineTo(sx,sy) : L.ctx.moveTo(sx,sy);
    });
    L.ctx.closePath(); L.ctx.strokeStyle = 'rgba(255,255,255,.18)'; L.ctx.lineWidth = 1; L.ctx.stroke();
    P.arrow(0,0, a.x, a.y, '#7c5cff', 4, 'a');
    P.arrow(0,0, b.x, b.y, '#00d4ff', 4, 'b');
    P.dot(a.x, a.y, 7, '#b9a8ff'); P.dot(b.x, b.y, 7, '#7fe7ff');
    const detCol = collapsed ? '#ffc94d' : '#2dd4a0';
    const spanTxt = collapsed ? 'a LINE (dependent)' : 'the whole plane';
    L.readout.innerHTML = 'a = [' + fmt2(a.x) + ', ' + fmt2(a.y) + ']  b = [' + fmt2(b.x) + ', ' + fmt2(b.y) + ']<br>' +
      'det = <span style="color:' + detCol + '">' + det.toFixed(2) + '</span><br>' +
      'span = ' + spanTxt;
    m.update({det, na, nb});
  }
  ...
  note(L.ctrl, 'Drag <b style="color:#b9a8ff">a</b> and <b style="color:#7fe7ff">b</b>. The dots are every reachable combination c₁a + c₂b. Slide b onto a\'s own line and the whole lattice flattens into one line — that collapse is <b>linear dependence</b>. The readout\'s <b>det = a₁b₂ − a₂b₁</b> is the area of the little cell the two vectors span; it hits 0 exactly when they line up.');
  draw();
};

/* ================== la-vecops — Lab 4: the difference vector ==================
   Concept: b − a is the arrow that walks FROM a TO b; its length is the
   distance between the two points — the same magnitude idea from the last
   lesson, now applied to a gap between two positions rather than one. */
INTERACTIVES.vecopsDiff = function(stage, api){
  const L = makeLab(stage);
  const a = {x:1, y:1}, b = {x:3, y:2};

  api.predict({
    prompt: 'Points \\(a = [1, 1]\\) and \\(b = [4, 5]\\). The walk from a to b is \\(b - a\\). Predict its length \\(\\|b - a\\|\\).',
    input: true, placeholder: '‖b − a‖', answer: 5, tol: 0.1, unit: '',
    reveal: '\\(b - a = [3, 4]\\), and \\(\\|[3, 4]\\| = 5\\) — a 3-4-5 triangle.',
  });

  const m = api.missions([
    {text:'Read the displacement: make the walk exactly \\(b - a = [2, 2]\\)', xp:20,
      check:s => Math.abs(s.dx - 2) < 0.15 && Math.abs(s.dy - 2) < 0.15},
    {text:'Make \\(b - a\\) point straight up: same x, with b at least 1.5 above a', xp:20,
      check:s => Math.abs(s.dx) < 0.15 && s.dy >= 1.5},
    {text:'A 3-4-5 walk: make the distance ‖b − a‖ = 5 (± 0.2)', xp:20,
      check:s => Math.abs(s.dist - 5) < 0.2},
  ]);
  ...
  function draw(){
    ...
    P.arrow(0,0, a.x, a.y, '#7c5cff', 3.5, 'a');
    P.arrow(0,0, b.x, b.y, '#00d4ff', 3.5, 'b');
    P.dot(a.x, a.y, 7, '#b9a8ff'); P.dot(b.x, b.y, 7, '#7fe7ff');
    P.arrow(a.x, a.y, b.x, b.y, '#2dd4a0', 4, 'b−a');
    L.ctx.save(); L.ctx.globalAlpha = .4;
    P.arrow(0, 0, d.x, d.y, '#2dd4a0', 2.5);
    L.ctx.restore();
    L.readout.innerHTML = 'a = [' + fmt2(a.x) + ', ' + fmt2(a.y) + ']  b = [' + fmt2(b.x) + ', ' + fmt2(b.y) + ']<br>' +
      'b − a = [' + fmt2(d.x) + ', ' + fmt2(d.y) + ']<br>' +
      '‖b − a‖ = ' + dist.toFixed(2) + '  <span style="color:#8b93b8">(distance a→b)</span>';
    m.update({dx:d.x, dy:d.y, dist});
  }
  function pick(e){ const p = L.toCanvas(e), wx = P.wx(p.x), wy = P.wy(p.y);
    return (Math.hypot(wx-a.x, wy-a.y) < Math.hypot(wx-b.x, wy-b.y)) ? a : b; }
  L.canvas.addEventListener('pointerdown', e=>{dragging=pick(e); L.canvas.setPointerCapture(e.pointerId); move(e);});
  ...
  function move(e){ if(!dragging) return; const p = L.toCanvas(e);
    dragging.x = snap(P.wx(p.x), 0.25); dragging.y = snap(P.wy(p.y), 0.25); draw(); }
  note(L.ctrl, 'Drag either dot. The <b style="color:#2dd4a0">green</b> arrow drawn from a to b is <b>b − a</b> — the walk that carries you from one point to the other. The faint ghost from the origin is that same vector, just relocated to start at 0; its length is the distance between a and b.');
  draw();
};
