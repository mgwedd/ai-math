/* ================================================================
   ...
   ================================================================ */
import { LESSONS, INTERACTIVES, WRONG_WHY, QMETA, SCORING, LEVELS } from './curriculum/registry.js';
...
// per-question concept metadata: inline (q.tag/q.focus) or QMETA registry
function metaOf(l, qi){
  const q=l.quiz[qi], m=(QMETA[l.id]&&QMETA[l.id][qi])||{};
  return { tag: q.tag || m.tag || l.title,
           focus: q.focus || m.focus || ('Re-read the lesson "'+l.title+'" — especially its Go-deeper notes.') };
}
...
const WORLD_META = {
  ...
  prob: {name:'🎲 World 2.5 — Probability & Statistics', desc:''},
  ml:   {name:'🤖 World 3 — Machine Learning', desc:''},
};
...
export { LESSONS, INTERACTIVES, WRONG_WHY, QMETA, SCORING };
