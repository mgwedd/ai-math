...
registerLesson({
  ...
  <div class="formula">$$z = w \\cdot x + b \\qquad p = \\sigma(z) = \\dfrac{1}{1 + e^{-z}}$$</div>
  ...
  <div class="formula">$$\\text{NLL} = -\\sum \\left[ y_i \\ln p_i + (1-y_i)\\ln(1-p_i) \\right]$$</div>
...
registerLesson({
  ...
  <div class="formula">$$\\text{minimize} \\quad \\text{NLL}(w) + \\lambda \\cdot \\text{penalty}(w)$$</div>
  ...
  <div class="formula">$$\\text{L2 penalty} = \\|w\\|^2 \\iff \\text{Gaussian prior} \\qquad \\text{L1 penalty} = \\|w\\|_1 \\iff \\text{Laplace prior}$$</div>
...
registerLesson({
  ...
  <div class="formula">$$\\text{odds} = \\dfrac{p}{1-p} \\qquad \\text{logit}(p) = \\ln\\!\\left(\\dfrac{p}{1-p}\\right) = z = w \\cdot x + b$$</div>
  ...
  <div class="formula">$$p = \\sigma(z) = \\dfrac{1}{1 + e^{-z}}$$</div>
  ...
  deeper:[
   ...
   {title:'🚀 Go deeper: why THIS link, not some other squashing curve', body:'The logit isn\'t picked because it looks nice — it falls out of the math. Write a Bernoulli outcome (y ∈ {0,1}) in <b>exponential-family</b> form and the "natural parameter" it\'s linear in is exactly ln(p/(1−p)), the log-odds. Pairing a linear predictor with the natural parameter of its distribution is called the <b>canonical link</b>, and it\'s special: the log-likelihood becomes concave in w (a single global maximum, no bad local optima), and the gradient of the negative log-likelihood collapses to the strikingly clean <b>(p − y)·x</b> — predicted-probability-minus-actual-label, scaled by the input. That is the exact gradient your optimizer uses to train logistic regression, and it is no coincidence that it looks just like the gradient of squared error in linear regression: canonical links are built precisely so this simplification happens. Recall from the card above that a weight w = ln(2) ≈ 0.69 multiplies the odds by e^w ≈ 2 — every worked number in this lesson is a symptom of the same underlying choice of link. (McCullagh &amp; Nelder, <em>Generalized Linear Models</em>, 2nd ed., ch. 2.)'}],
  interactive:'mllogit',
  quiz:[
