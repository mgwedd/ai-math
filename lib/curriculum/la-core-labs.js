...
INTERACTIVES.vectorsData = function(stage, api){
  ...
  api.predict({
    prompt: 'The probe sits at <code>[1, 1]</code> and the cat at <code>[2, 1]</code>. Predict the <b>distance</b> between them.',
  ...
  const m = api.missions([
    {text: 'Give the probe <b>speed 4, size 0.75</b> — land exactly on the 🦅 hawk', xp: 20,
      check: s => s.dHawk < 0.2},
    {text: 'Make <b>🐢 turtle</b> the nearest neighbor while staying <b>≥ 1.0</b> away from it', xp: 25,
...
INTERACTIVES.vectorsNorm = function(stage, api){
  ...
  api.predict({
    prompt: 'A vector <code>v = [6, 8]</code>. Predict its magnitude <b>‖v‖</b> before you build anything.',
...
INTERACTIVES.vecopsReach = function(stage, api){
  ...
  api.predict({
    prompt: 'With <code>a = [2, 1]</code> and <code>b = [1, 3]</code>, you want <code>c₁·a + c₂·b = [5, 5]</code>.<br>Solve the system in your head and predict <b>c₁</b> (how many copies of a).',
...
INTERACTIVES.dotTarget = function(stage, api){
  ...
  api.predict({
    prompt: 'Compute it by hand first: <code>a = [3, 4]</code>, <code>b = [4, 3]</code>. Predict <b>a · b</b>.',
...
INTERACTIVES.matrixAim = function(stage, api){
  ...
  api.predict({
    prompt: 'The matrix <code>M = [[2,0],[0,1]]</code> doubles x and leaves y. It sends <code>[3, 4]</code> to a point — predict that point\'s <b>x-coordinate</b>.',
...
INTERACTIVES.matmulReach = function(stage, api){
  ...
  api.predict({
    prompt: 'R = rotate 90° CCW, S = shear→ (<code>[[1,1],[0,1]]</code>). Apply both to a point, in the two orders R∘S and S∘R. Do the orders land the point in the <b>same</b> place?',
...
INTERACTIVES.detBuild = function(stage, api){
  ...
  api.predict({
    prompt: 'Column 1 is pinned at <code>[2, 1]</code>. You drag column 2 to <code>[4, 2]</code>. Predict the determinant <b>det = ad − bc</b>.',
...
INTERACTIVES.eigenHunt = function(stage, api){
  ...
  api.predict({
    prompt: '<code>M = [[3, 1], [0, 2]]</code> is <b>triangular</b> (a zero in the lower-left). For a triangular matrix the eigenvalues are just the diagonal entries. Predict the <b>larger</b> eigenvalue.',
