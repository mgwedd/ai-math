...
/* ================================================================
   EXPOSITION FIGURES for la-matmul ("see it first" gallery).
   Each takes ONLY a stage, builds its own canvas, defines its own
   mul/T/lerp/poly/F locally, draws once synchronously, and returns a
   cancel cleanup because all three animate. No Math.random.
   ================================================================ */

/* mmCompose — B·A = apply A first (shear), then B (rotate). One F morphs
   ID → A → BA in a loop over faint reference ghosts of each stage. */
INTERACTIVES.mmCompose = function(stage){
  const F0 = figCanvas(stage, 640, 340);
  const ctx = F0.ctx, W = F0.W, H = F0.H;
  const P = plane(ctx, W, H, 44, W/2, H/2);
  function mul(B,A){ return [B[0]*A[0]+B[1]*A[2], B[0]*A[1]+B[1]*A[3], B[2]*A[0]+B[3]*A[2], B[2]*A[1]+B[3]*A[3]]; }
  function T(M,x,y){ return [M[0]*x+M[1]*y, M[2]*x+M[3]*y]; }
  const lerp = (M0,M1,u) => M0.map((v,i)=>v+(M1[i]-v)*u);
  function poly(pts,fill,stroke){ ctx.beginPath();
    pts.forEach((p,i)=>{const sx=P.sx(p[0]),sy=P.sy(p[1]); i?ctx.lineTo(sx,sy):ctx.moveTo(sx,sy);});
  ...
  const A=[1,0.6,0,1];
  const B=[0.77,-0.64,0.64,0.77];
  const BA=mul(B,A);
  const ID=[1,0,0,1];
  let t=0,raf=0;
  function draw(){
    clearBg(ctx,W,H); ctx.globalAlpha=.3; P.grid(); ctx.globalAlpha=1;
    // faint reference ghosts: original, after A, after BA
    poly(F,'rgba(139,147,184,.10)','rgba(139,147,184,.4)');
    poly(F.map(p=>T(A,p[0],p[1])),'rgba(0,212,255,.06)','rgba(0,212,255,.3)');
    poly(F.map(p=>T(BA,p[0],p[1])),'rgba(45,212,160,.06)','rgba(45,212,160,.3)');
    // animated bold F: ID→A on the first half, A→BA on the second
    const u=1+Math.sin(t);
    const cur = u<1 ? lerp(ID,A,u) : lerp(A,BA,u-1);
    poly(F.map(p=>T(cur,p[0],p[1])),'rgba(45,212,160,.22)','#2dd4a0');
    ctx.fillStyle='#cdd4f0'; ctx.font='700 15px '+FONT(); ctx.textAlign='left';
    ctx.fillText(u<1?'apply A (shear)…':'…then B (rotate)', 20, 30);
  }
  draw();
  function loop(){ t+=0.02; draw(); raf=requestAnimationFrame(loop); }
  raf=requestAnimationFrame(loop);
...
/* mmOrder — two side-by-side panels: B·A (shear after rotate) vs A·B
   (rotate after shear). Each morphs ID → its product; the two land
   differently, showing non-commutativity. */
INTERACTIVES.mmOrder = function(stage){
  const F0 = figCanvas(stage, 640, 340);
  const ctx = F0.ctx, W = F0.W, H = F0.H;
  function mul(B,A){ return [B[0]*A[0]+B[1]*A[2], B[0]*A[1]+B[1]*A[3], B[2]*A[0]+B[3]*A[2], B[2]*A[1]+B[3]*A[3]]; }
  function T(M,x,y){ return [M[0]*x+M[1]*y, M[2]*x+M[3]*y]; }
  ...
  const A=[0,-1,1,0];
  const B=[1,1,0,1];
  const CL=mul(B,A), CR=mul(A,B);
  const ID=[1,0,0,1];
  const PL=plane(ctx,W,H,22,165,180);
  const PR=plane(ctx,W,H,22,480,180);
  let t=0,raf=0;
  function panel(P,C,title,color,x0){
    ctx.save();
    ctx.beginPath(); ctx.rect(x0,0,320,H); ctx.clip();   // keep each panel's grid in its own half
    ctx.globalAlpha=.3; P.grid(); ctx.globalAlpha=1;
    function poly(pts,fill,stroke){ ctx.beginPath();
      pts.forEach((p,i)=>{const sx=P.sx(p[0]),sy=P.sy(p[1]); i?ctx.lineTo(sx,sy):ctx.moveTo(sx,sy);});
      ctx.closePath(); ctx.fillStyle=fill; ctx.fill(); ctx.strokeStyle=stroke; ctx.lineWidth=2; ctx.stroke(); }
    poly(F,'rgba(139,147,184,.10)','rgba(139,147,184,.4)');
    const u=0.5+0.5*Math.sin(t);
    const cur=lerp(ID,C,u);
    poly(F.map(p=>T(cur,p[0],p[1])),color+'.22)',color+'1)');
    ctx.fillStyle='#cdd4f0'; ctx.font='700 13px '+FONT(); ctx.textAlign='center';
    ctx.fillText(title, P.sx(0), 24);
    ctx.restore();
  }
  function draw(){
    clearBg(ctx,W,H);
    panel(PL,CL,'B·A: shear after rotate','rgba(45,212,160,',0);
    panel(PR,CR,'A·B: rotate after shear','rgba(255,92,122,',320);
  }
  draw();
  function loop(){ t+=0.02; draw(); raf=requestAnimationFrame(loop); }
  raf=requestAnimationFrame(loop);
...
/* mmEntry — number grids, no plane: (B·A) built one entry at a time.
   Cycles through the four entries, highlighting the contributing row of
   B and column of A, showing the dot-product formula, revealing the cell. */
INTERACTIVES.mmEntry = function(stage){
  const F0 = figCanvas(stage, 640, 320);
  const ctx = F0.ctx, W = F0.W, H = F0.H;
  const B=[[2,0],[1,3]], A=[[1,4],[2,1]], Pr=[[2,8],[7,7]];
  let t=0,raf=0;
  // grid drawn at top-left (gx,gy): a 2×2 of numbers with cell size cs
  const cs=44;
  function cell(gx,gy,i,j){ return [gx+j*cs+cs/2, gy+i*cs+cs/2]; }
  function grid(gx,gy,M,hiRow,hiCol,greenIJ,muted){
    for(let i=0;i<2;i++) for(let j=0;j<2;j++){
      const [cx,cy]=cell(gx,gy,i,j);
      let col='#cdd4f0';
      if(greenIJ && greenIJ[0]===i && greenIJ[1]===j) col='#2dd4a0';
      else if(muted) col='#5b6488';
      ctx.fillStyle=col; ctx.font='700 20px '+FONT(); ctx.textAlign='center'; ctx.textBaseline='middle';
      ctx.fillText(String(M[i][j]), cx, cy);
    }
    // gold highlight box around a row or column
    ctx.strokeStyle='#ffc94d'; ctx.lineWidth=2.5;
    if(hiRow!=null) ctx.strokeRect(gx+2, gy+hiRow*cs+2, cs*2-4, cs-4);
    if(hiCol!=null) ctx.strokeRect(gx+hiCol*cs+2, gy+2, cs-4, cs*2-4);
    // bracket edges
    ctx.strokeStyle='rgba(139,147,184,.5)'; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.moveTo(gx-2,gy); ctx.lineTo(gx-2,gy+cs*2); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(gx+cs*2+2,gy); ctx.lineTo(gx+cs*2+2,gy+cs*2); ctx.stroke();
  }
  function draw(){
    clearBg(ctx,W,H);
    const k=Math.floor(t*0.7)%4, i=k>>1, j=k&1;
    const gy=90;
    const bx=60, ax=250, px=470;
    grid(bx,gy,B,i,null,null,false);
    grid(ax,gy,A,null,j,null,false);
    grid(px,gy,Pr,null,null,[i,j],true);
    ctx.fillStyle='#8b93b8'; ctx.font='700 24px '+FONT(); ctx.textAlign='center'; ctx.textBaseline='middle';
    ctx.fillText('·', (bx+cs*2+ax)/2, gy+cs);
    ctx.fillText('=', (ax+cs*2+px)/2, gy+cs);
    // labels
    ctx.fillStyle='#8b93b8'; ctx.font='700 13px '+FONT();
    ctx.fillText('B', bx+cs, gy-16); ctx.fillText('A', ax+cs, gy-16); ctx.fillText('B·A', px+cs, gy-16);
    // dot-product formula for the highlighted entry, under the grids
    ctx.fillStyle='#ffe6a8'; ctx.font='700 17px '+FONT(); ctx.textAlign='center';
    const line = 'entry ('+(i+1)+','+(j+1)+'):  '+B[i][0]+'·'+A[0][j]+' + '+B[i][1]+'·'+A[1][j]+' = '+Pr[i][j];
    ctx.fillText(line, W/2, gy+cs*2+44);
  }
  draw();
  function loop(){ t+=0.02; draw(); raf=requestAnimationFrame(loop); }
  raf=requestAnimationFrame(loop);
...
/* ================== la-matmul — Lab 3: Compute the product, entry by entry ====
   Concept: each product entry is a row·column dot product; order changes them.
   Fill B·A entry-by-entry, then A·B to see a different matrix. */
INTERACTIVES.matmulEntry = function(stage, api){
  const L=makeLab(stage, {h:280});
  const B=[2,0,1,3], A=[1,4,2,1];
  let mode='BA';
  let p={p00:0,p01:0,p10:0,p11:0};
  const TARGET={BA:[2,8,7,7], AB:[6,12,5,3]};

  api.predict({
    prompt:'Entry (row 1, column 2) of \\(B\\cdot A\\) is (row 1 of B) · (column 2 of A). With \\(B=\\begin{bmatrix}2&0\\\\1&3\\end{bmatrix}\\) and \\(A=\\begin{bmatrix}1&4\\\\2&1\\end{bmatrix}\\), predict it.',
    input:true, placeholder:'entry (1,2)', answer:8, tol:0.5, unit:'',
    reveal:'Row 1 of B is \\([2,0]\\); column 2 of A is \\([4,1]\\). Their dot product is \\(2\\cdot4 + 0\\cdot1 = 8\\). Every entry of a product is one row·column dot product.',
  });

  const m=api.missions([
    {text:'Fill the top row of B·A: entry (1,1) = 2 and (1,2) = 8', xp:20,
      check:s=>s.mode==='BA' && Math.abs(s.p00-2)<0.5 && Math.abs(s.p01-8)<0.5},
    {text:'Complete the full product B·A = [[2,8],[7,7]]', xp:25,
      check:s=>s.mode==='BA' && Math.abs(s.p00-2)<0.5 && Math.abs(s.p01-8)<0.5 && Math.abs(s.p10-7)<0.5 && Math.abs(s.p11-7)<0.5},
    {text:'Switch to A·B and complete it — a DIFFERENT matrix [[6,12],[5,3]]', xp:25,
      check:s=>s.mode==='AB' && Math.abs(s.p00-6)<0.5 && Math.abs(s.p01-12)<0.5 && Math.abs(s.p10-5)<0.5 && Math.abs(s.p11-3)<0.5},
  ]);

  const ctx=L.ctx;
  const cs=52, gy=80;
  const lx=[B[0],B[1],B[2],B[3]], rx=[A[0],A[1],A[2],A[3]];
  function cellCenter(gx,i,j){ return [gx+j*cs+cs/2, gy+i*cs+cs/2]; }
  function grid(gx,vals,greenMask){
    ctx.strokeStyle='rgba(139,147,184,.5)'; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.moveTo(gx-2,gy); ctx.lineTo(gx-2,gy+cs*2); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(gx+cs*2+2,gy); ctx.lineTo(gx+cs*2+2,gy+cs*2); ctx.stroke();
    for(let i=0;i<2;i++) for(let j=0;j<2;j++){
      const idx=i*2+j, [cx,cy]=cellCenter(gx,i,j);
      // product cells: green when correct, soft-red when wrong, neutral while still at the untouched 0
      ctx.fillStyle = greenMask ? (greenMask[idx]?'#2dd4a0':(vals[idx]===0?'#cdd4f0':'#ff9db0')) : '#cdd4f0';
      ctx.font='700 22px '+FONT(); ctx.textAlign='center'; ctx.textBaseline='middle';
      ctx.fillText(String(vals[idx]), cx, cy);
    }
  }
  function draw(){
    clearBg(ctx,L.W,L.H);
    const left = mode==='BA' ? B : A;
    const right = mode==='BA' ? A : B;
    const tgt = TARGET[mode];
    const pv=[p.p00,p.p01,p.p10,p.p11];
    const mask=pv.map((v,i)=>Math.abs(v-tgt[i])<0.5);
    const bx=40, ax=210, px=400;
    grid(bx,left,null);
    grid(ax,right,null);
    grid(px,pv,mask);
    ctx.fillStyle='#8b93b8'; ctx.font='700 26px '+FONT(); ctx.textAlign='center'; ctx.textBaseline='middle';
    ctx.fillText('·', (bx+cs*2+ax)/2, gy+cs);
    ctx.fillText('=', (ax+cs*2+px)/2, gy+cs);
    ctx.fillStyle='#8b93b8'; ctx.font='700 13px '+FONT();
    const lName = mode==='BA'?'B':'A', rName = mode==='BA'?'A':'B';
    ctx.fillText(lName, bx+cs, gy-14); ctx.fillText(rName, ax+cs, gy-14);
    ctx.fillText(mode==='BA'?'B·A':'A·B', px+cs, gy-14);
    const correct=mask.filter(Boolean).length;
    L.readout.innerHTML='Computing <b>'+(mode==='BA'?'B·A':'A·B')+'</b><br>'+
      '<span style="color:'+(correct===4?'#2dd4a0':'#ffc94d')+'">'+correct+' / 4</span> entries correct';
    m.update({mode, p00:p.p00, p01:p.p01, p10:p.p10, p11:p.p11});
  }
  slider(L.ctrl, 'entry (1,1)', -20, 20, 1, 0, x=>x.toFixed(0), x=>{p.p00=x; draw();});
  slider(L.ctrl, 'entry (1,2)', -20, 20, 1, 0, x=>x.toFixed(0), x=>{p.p01=x; draw();});
  slider(L.ctrl, 'entry (2,1)', -20, 20, 1, 0, x=>x.toFixed(0), x=>{p.p10=x; draw();});
  slider(L.ctrl, 'entry (2,2)', -20, 20, 1, 0, x=>x.toFixed(0), x=>{p.p11=x; draw();});
  const crow=chips(L.ctrl,'COMPUTE',['B·A','A·B'],(i,btn,row)=>{
    mode=i===0?'BA':'AB'; [...row.children].forEach(b=>b.classList.remove('on')); btn.classList.add('on'); draw();
  });
  crow.children[0].classList.add('on');
  note(L.ctrl,'Each product entry is a row·column dot product. Fill all four, then switch to <b>A·B</b> — the entries change, because order matters.');
  draw();
};

/* ================== la-matmul — Lab 4: Commuting vs non-commuting pairs =======
   Concept: some pairs commute (rotations, scalars), most don't. Overlay B·A
   (green) against A·B (red); when they coincide the pair commutes. */
INTERACTIVES.matmulCommute = function(stage, api){
  const L=makeLab(stage);
  const TS=[
    {name:'Rotate 90°',  M:[0,-1,1,0],  rot:true},
    {name:'Rotate 45°',  M:[0.71,-0.71,0.71,0.71], rot:true},
    {name:'Scale 2×',    M:[2,0,0,2],   scalar:true},
  ...
  api.predict({
    prompt:'Which pair of transformations is guaranteed to <b>commute</b> (\\(A\\cdot B = B\\cdot A\\))?',
    choices:['Two rotations','A rotation and a shear','No two transforms ever commute'],
    answer:0,
    reveal:'Rotations commute with each other (and any scalar multiple of the identity, like \\(2I\\), commutes with everything). A rotation and a shear generally do <b>not</b>: \\(R\\cdot S \\neq S\\cdot R\\). Non-commutativity is exactly why layer order matters in a network.',
  });

  const m=api.missions([
    {text:'Confirm a NON-commuting pair: make the gap large (&gt; 1.5)', xp:20,
      check:s=>s.gap>1.5},
    {text:'Find a COMMUTING pair of two DIFFERENT rotations (gap &lt; 0.05)', xp:25,
      check:s=>s.gap<0.05 && s.ai!==s.bi && s.aRot && s.bRot},
    {text:'Show a scalar (Scale 2× = 2I) commutes with a different transform (gap &lt; 0.05)', xp:20,
      check:s=>s.gap<0.05 && s.ai!==s.bi && (s.aName==='Scale 2×' || s.bName==='Scale 2×')},
  ]);

  const ctx=L.ctx;
  const P=plane(ctx,L.W,L.H,52);
  const F=[[0,0],[0,2],[1,2],[1,1.6],[0.4,1.6],[0.4,1.1],[0.9,1.1],[0.9,0.7],[0.4,0.7],[0.4,0]];
  function poly(pts,fill,stroke){ ctx.beginPath();
    pts.forEach((p,i)=>{const sx=P.sx(p[0]),sy=P.sy(p[1]); i?ctx.lineTo(sx,sy):ctx.moveTo(sx,sy);});
    ctx.closePath(); ctx.fillStyle=fill; ctx.fill(); ctx.strokeStyle=stroke; ctx.lineWidth=2; ctx.stroke(); }
  function draw(){
    clearBg(ctx,L.W,L.H); ctx.globalAlpha=.3; P.grid(); ctx.globalAlpha=1;
    const A=TS[ai].M, B=TS[bi].M;
    const C1=mul(B,A), C2=mul(A,B);
    const gap=C1.reduce((s,v,i)=>s+Math.abs(v-C2[i]),0);
    poly(F,'rgba(139,147,184,.12)','rgba(139,147,184,.5)');
    poly(F.map(p=>T(C1,p[0],p[1])),'rgba(45,212,160,.16)','#2dd4a0');
    poly(F.map(p=>T(C2,p[0],p[1])),'rgba(255,92,122,.16)','#ff5c7a');
    L.readout.innerHTML='B·A vs A·B<br>‖B·A − A·B‖ = <span style="color:'+(gap<0.05?'#2dd4a0':'#ff5c7a')+'">'+gap.toFixed(2)+'</span>'+(gap<0.05?'  ✓ they COMMUTE':'  ✗ order matters');
    m.update({ai, bi, aName:TS[ai].name, bName:TS[bi].name, aRot:!!TS[ai].rot, bRot:!!TS[bi].rot, gap});
  }
  const rowA=chips(L.ctrl,'A (applied first / right factor)',TS.map(t=>t.name),(i,btn,row)=>{
    ai=i; [...row.children].forEach(b=>b.classList.remove('on')); btn.classList.add('on'); draw();
  });
  rowA.children[0].classList.add('on');
  const rowB=chips(L.ctrl,'B (applied second / left factor)',TS.map(t=>t.name),(i,btn,row)=>{
    bi=i; [...row.children].forEach(b=>b.classList.remove('on')); btn.classList.add('on'); draw();
  });
  rowB.children[0].classList.add('on');
  note(L.ctrl,'Green = <b>B·A</b>, red = <b>A·B</b>. When they land on the same F the pair commutes; when they split apart, order matters.');
  draw();
};
