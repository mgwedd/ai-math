...
registerLesson({
  ...
  expositions:[
    {key:'compose', title:'Multiplication is composition', figure:'mmCompose', caption:'\\(B\\cdot A\\) means <b>apply A first, then B</b> — read right to left, like \\(B(A(x))\\). The shape flows through one warp, then the next.'},
    {key:'order', title:'Order matters', figure:'mmOrder', caption:'\\(B\\cdot A \\neq A\\cdot B\\) in general. Rotate-then-shear lands a shape somewhere different from shear-then-rotate: matrix multiplication is <b>not commutative</b>.'},
    {key:'entry', title:'Each entry is a dot product', figure:'mmEntry', caption:'Entry \\((i,j)\\) of a product is <b>row \\(i\\) of the left</b> dotted with <b>column \\(j\\) of the right</b>: \\((B\\cdot A)_{ij} = \\sum_k B_{ik}A_{kj}\\).'},
  ],
  labs:[
    ...
    {key:'entryfill', title:'Compute the product, entry by entry', interactive:'matmulEntry',
     intro:'<p>Build \\(B\\cdot A\\) one entry at a time — each is a <strong>row · column dot product</strong>. Predict one, then fill all four and watch them go green. Then switch to \\(A\\cdot B\\) and see a different matrix.</p>'},
    {key:'commute', title:'Commuting vs non-commuting pairs', interactive:'matmulCommute',
     intro:'<p>Pick two transforms and overlay \\(B\\cdot A\\) against \\(A\\cdot B\\). Find a pair that <strong>commutes</strong> (they coincide), then one that does not — the gap between the two images is non-commutativity.</p>'},
  ],
  quiz:[
