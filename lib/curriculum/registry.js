...
// Scene Kit registry (pure data, renderer-free) — needed by validateCurriculum
// for the CONTRACT §7.1 capstone-placement check on lessons that opt into
// `scenes:[...]`. Safe import: lib/scene/registry.js touches no DOM/GPU.
...
function lessonShapeProblems(l){
  ...
  // CONTRACT v1.3 §5: a scenes-only lesson is valid — `scenes:[...]` with >= 1
  // scene satisfies the interactive-content requirement in place of
  // labs/interactive (quiz rules unchanged). Deep scene checks (ids resolve,
  // capstone last, capstone has goals) stay in validateCurriculum below.
  const hasScenes = Array.isArray(l.scenes) && l.scenes.length > 0;
  if(!l.interactive && !hasLabs && !hasScenes) p.push('needs an `interactive` key, a non-empty `labs` array, or a non-empty `scenes` array');
...
export function validateCurriculum(){
  ...
  for(const l of LESSONS){
    ...
    // Scene-lesson checks (Scene Kit CONTRACT §7.1 — this is the curriculum
    // registry's court, NOT validateScenes(): scene ORDER is a lesson-level
    // property invisible to the SCENES registry). For every lesson that has
    // opted into `scenes:[...]` (VISUAL_FIRST §7 migration switch):
    //   - every entry resolves (a scene id registered in SCENES, or an inline
    //     scene object) — mirrors the interactive-key resolution above;
    //   - at most ONE resolved entry has capstone:true;
    //   - if present, the capstone is the FINAL entry. A mid-sequence capstone
    //     completes the lesson early and silently strands every later scene;
    //   - CONTRACT v1.3 §4: a capstone scene declares >= 1 goal — makeGoals([])
    //     is vacuously all-done at construction, so an empty-goals capstone
    //     would instantly complete the lesson. Non-capstone scenes may have
    //     zero goals (decorative).
    if(l.scenes !== undefined){
      if(!Array.isArray(l.scenes) || l.scenes.length === 0){
        errs.push('"'+l.id+'": `scenes` must be a non-empty array of scene ids');
      } else {
        const resolved = l.scenes.map((entry,si)=>{
          if(typeof entry === 'string'){
            const sc = SCENES.find(s => s.id === entry);
            if(!sc) errs.push('"'+l.id+'": scenes['+si+'] "'+entry+'" is not registered in SCENES');
            return sc;
          }
          if(entry && typeof entry === 'object') return entry;   // inline spec
          errs.push('"'+l.id+'": scenes['+si+'] must be a scene id or scene object');
          return undefined;
        });
        const capIdx = resolved.map((s,si)=>s && s.capstone === true ? si : -1).filter(si=>si>=0);
        if(capIdx.length > 1)
          errs.push('"'+l.id+'": has '+capIdx.length+' capstone scenes (at most 1 — the capstone is the exam)');
        else if(capIdx.length === 1 && capIdx[0] !== l.scenes.length - 1)
          errs.push('"'+l.id+'": capstone scene must be the LAST entry of `scenes` (found at index '+capIdx[0]+' of '+(l.scenes.length-1)+') — scenes after the capstone are unreachable');
        for(const si of capIdx){
          const g = resolved[si].goals;
          if(!Array.isArray(g) || g.length === 0)
            errs.push('"'+l.id+'": capstone scene '+(resolved[si].id ? '"'+resolved[si].id+'"' : 'scenes['+si+']')+' declares no goals — an empty-goals capstone is vacuously complete and would instantly finish the lesson (CONTRACT v1.3 §4)');
        }
      }
    }
