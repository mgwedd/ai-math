...
/* ---------- admin tools (email-allowlisted) ----------
   Only emails in ADMIN_EMAILS get any admin UI. Extend the list here.
   Admin override state lives in S.admin ({unlockAll, unlocked:{id:true}}) so
   it survives reload, but is kept OUT of the normal progress maps so a
   "reset" never touches it (and vice-versa). */
const ADMIN_EMAILS = ['michael.wedd@gmail.com'];
let isAdmin = false;
function adminState(){ if(!S.admin) S.admin = {unlockAll:false, unlocked:{}}; if(!S.admin.unlocked) S.admin.unlocked={}; return S.admin; }
...
function isUnlocked(lesson){
  if(isAdmin && (S.admin?.unlockAll || S.admin?.unlocked?.[lesson.id])) return true;
...
function adminUnlockAll(){ adminState().unlockAll = true; adminAfter(); }
function adminUnlockLesson(id){ adminState().unlocked[id] = true; adminAfter(); }
function resetLessonProgress(id){
  delete S.done[id]; delete S.missions[id]; delete S.firstTry[id];
  if(S.weak) delete S.weak[id];
  if(S.admin?.unlocked) delete S.admin.unlocked[id];
}
function adminResetLesson(id){
  if(!confirm('Reset all progress for this lesson? (XP is kept)')) return;
  resetLessonProgress(id); pruneAchievements(); adminAfter();
}
function adminResetWorld(w){
  ...
  if(S.admin) S.admin.unlockAll = false;
  list.forEach(l=>resetLessonProgress(l.id));
  pruneAchievements();
  adminAfter();
}

// Lightweight admin popover (vanilla DOM, mirrors .user-menu styling).
// items: [{label, danger?, run}]. Anchored under `btn`.
function openAdminMenu(btn, items){
  document.querySelectorAll('.admin-menu').forEach(m=>m.remove());
  const menu = document.createElement('div'); menu.className='user-menu admin-menu';
  menu.innerHTML = items.map((it,i)=>
    '<button class="um-item'+(it.danger?' um-out':'')+'" data-i="'+i+'">'+it.label+'</button>').join('');
  const close = ()=>{ menu.remove(); document.removeEventListener('pointerdown', onAway, true); };
...
// Delegated handler for admin ⋮ buttons rendered into home (avoids inline
// handlers so it plays nicely with the data-driven renderHome markup).
function onAdminClick(e){
  const btn = e.target.closest('.admin-dots'); if(!btn || !isAdmin) return;
  e.stopPropagation(); e.preventDefault();
  const world = btn.dataset.world, lesson = btn.dataset.lesson;
  if(world){
    openAdminMenu(btn, [
      {label:'🔓 Unlock all', run:adminUnlockAll},
      {label:'♻️ Reset world', danger:true, run:()=>adminResetWorld(world)},
    ]);
  } else if(lesson){
    openAdminMenu(btn, [
      {label:'🔓 Unlock lesson', run:()=>adminUnlockLesson(lesson)},
      {label:'♻️ Reset lesson', danger:true, run:()=>adminResetLesson(lesson)},
    ]);
  }
}
...
function renderHome(){
  ...
  for(const w of WORLD_ORDER){
    ...
    h+='<section class="world"><div class="world-head"><h2>'+WORLD_META[w].name+'</h2>'+
       ...
       (isAdmin?'<button class="admin-dots" data-world="'+w+'" title="Admin: world tools" aria-label="Admin world tools">⋮</button>':'')+
    ...
    for(const l of list){
      ...
      h+='<div class="lcard'+(unlocked?'':' locked')+(done_?' done-card':'')+'" '+
         ...
         (isAdmin?'<button class="admin-dots admin-dots-card" data-lesson="'+l.id+'" title="Admin: lesson tools" aria-label="Admin lesson tools">⋮</button>':'')+
...
export function mount(opts){
  ...
  isAdmin = ADMIN_EMAILS.includes((opts.userEmail || '').toLowerCase());
  ...
  // admin ⋮ menus: capture-phase delegation so the click never reaches the
  // lesson card's onclick=go('lesson', …) navigation.
  if (isAdmin) view.addEventListener('click', onAdminClick, true);
