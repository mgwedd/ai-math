...
import { makeLab, slider, chips, plane, clearBg, registerCleanup } from '../engine.js';
...
registerLesson({
  ...
  <div class="formula">$$f \\text{ convex} \\iff \\text{chord always} \\geq \\text{curve} \\iff f''(x) \\geq 0 \\text{ everywhere}$$</div>
  ...
  deeper:[
   ...
   {title:'🚀 Go deeper: the first-order condition (tangent-plane lower bound)', body:'There is a slope-only test that needs no second derivative. A differentiable f is convex exactly when it always sits <em>on or above its own tangent</em>:<div class="formula">$$f(y) \\geq f(x) + \\nabla f(x)^T (y - x) \\quad \\text{for all } x, y$$</div>Every tangent plane under-estimates the graph, so the linear model gradient descent trusts is never an over-estimate of the true loss — the picture behind convergence proofs. It also delivers the optimality certificate: where ∇f(x)=0 the bound reads f(y) ≥ f(x) for every y, so that point is the global minimum. (Boyd &amp; Vandenberghe, <em>Convex Optimization</em>, §3.1.3.)'},
...
registerLesson({
  ...
  <div class="formula">$$f(x) \\approx f(a) + f'(a)(x-a) + \\tfrac{1}{2} f''(a)(x-a)^2$$</div>
  ...
  <div class="formula">$$x \\leftarrow x - \\dfrac{f'(x)}{f''(x)}$$</div>
...
INTERACTIVES.newton = function(stage, api){
  const L=makeLab(stage);
  const fns=[
    {n:'quadratic', f:x=>0.5*(x-2)*(x-2), df:x=>(x-2), ddf:_x=>1, min:2},
  ...
  L.ctrl.appendChild(note);
  draw();
  // Predict-then-verify: commit BEFORE touching the sliders. The controls stay
  // locked until you lock in a guess — no sweeping your way to the answer.
  api.predict({
    prompt:'You start on the <b>quadratic bowl</b>. Gradient descent will crawl down in many small steps. How many steps does <b>Newton’s method</b> take to land on the minimum?',
    choices:['Exactly 1 step','About 5 steps','Roughly the same as gradient descent','It never lands exactly — only gets close'],
    answer:0,
    reveal:'A quadratic <em>is</em> its own second-order Taylor model, so Newton’s parabola matches the function perfectly and jumps straight to the vertex — <b>one step</b>, from any start. Now sweep the start and the GD learning rate to watch that hold while gradient descent grinds.'
  });
};
