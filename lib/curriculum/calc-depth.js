...
registerLesson({
  ...
  quiz:[
   ...
   {q:'Why does convexity matter so much for gradient descent?', opts:['Every local minimum is also the global minimum, so downhill always wins','It makes the gradient larger','It removes the need for a learning rate','It guarantees the loss is zero'], a:0, tag:'global guarantee', focus:'On a convex loss, the only flat spots are the global minimum — GD cannot get stuck.',
    why:'Convex functions have no spurious local minima, so following the slope downhill from any start lands at THE global optimum.', wrong:{1:'A bigger gradient is not the point — convexity is about the SHAPE guaranteeing no traps.',2:'You still need a learning rate; convexity just guarantees where you end up, not the step size.',3:'The minimum of a convex loss can be any value, not necessarily zero.'}},
