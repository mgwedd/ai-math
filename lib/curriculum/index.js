/* ================================================================
   ...
   Wrong-answer feedback: WRONG_WHY[lessonId][questionIdx][wrongOptIdx]
     explains why that option is wrong + the misconception behind it.
   ================================================================ */
...
LESSONS.push({
  ...
  learn:`<p>A <strong>vector</strong> is just an ordered list of numbers. In 2D, the vector <code>[3, 2]</code> means "go 3 right, 2 up" — drawn as an arrow from the origin.</p>
  ...
  interactive:'vectors',
  quiz:[
    {q:'The vector <code>[−4, 3]</code> has magnitude…', opts:['1','5','7','25'], a:1,
     why:'‖v‖ = √((−4)² + 3²) = √(16+9) = √25 = <b>5</b>. Classic 3-4-5 triangle.'},
    {q:'A vector <code>[0.2, −0.1, 0.7, …]</code> with 1,536 numbers in it is best described as…', opts:['An error — vectors are 2D or 3D','A point/direction in 1,536-dimensional space','A matrix','1,536 separate vectors'], a:1,
     why:'Dimensions beyond 3 work exactly like 2D — you just can\'t draw them. OpenAI\'s embedding vectors are literally this size.'},
    {q:'Which vector is a <b>unit</b> vector?', opts:['<code>[1, 1]</code>','<code>[0.6, 0.8]</code>','<code>[2, 0]</code>','<code>[0.5, 0.5]</code>'], a:1,
     why:'√(0.36 + 0.64) = √1 = 1. Note <code>[1,1]</code> has magnitude √2 ≈ 1.41 — a common trap.'},
  ]
});
...
LESSONS.push({
  ...
  <p><strong>Addition</strong> — add component-wise: <code>[1,2] + [3,−1] = [4,1]</code>. Geometrically: walk along the first arrow, then the second ("tip-to-tail").</p>
  <p><strong>Scalar multiplication</strong> — multiply every component by a number: <code>2·[3,1] = [6,2]</code>. It stretches (|c|&gt;1), shrinks (|c|&lt;1), or flips (c&lt;0) the arrow.</p>
  <div class="formula">c·a + b   ←  a "linear combination"</div>
  ...
  ml:`A neuron computes exactly this: <code>w₁x₁ + w₂x₂ + … + b</code> — a linear combination of its inputs plus a bias. Training a network = finding the right scalars. You're about to do by hand what backprop does a trillion times.`,
  interactive:'vecops',
  quiz:[
    {q:'<code>[2, −1] + [−2, 4]</code> = ?', opts:['<code>[0, 3]</code>','<code>[4, 5]</code>','<code>[0, −5]</code>','<code>[−4, −4]</code>'], a:0,
     why:'Component-wise: 2+(−2)=0 and −1+4=3.'},
    {q:'What does multiplying a vector by <b>−0.5</b> do to it?', opts:['Rotates it 90°','Halves its length and reverses its direction','Doubles its length','Nothing — scalars only affect magnitude'], a:1,
     why:'|−0.5| = 0.5 shrinks it to half length; the minus sign flips it 180°.'},
    {q:'Which expression is a linear combination of <code>a</code> and <code>b</code>?', opts:['<code>a · b</code>','<code>3a − 2b</code>','<code>a / b</code>','<code>‖a‖ + ‖b‖</code>'], a:1,
     why:'Scalars times vectors, summed: 3a + (−2)b. Dot product gives a number, division isn\'t defined, and magnitudes are just lengths.'},
  ]
});
...
LESSONS.push({
  ...
  <p>It also has a geometric identity: <code>a·b = ‖a‖‖b‖cos θ</code>, where θ is the angle between the arrows. So the dot product measures <strong>alignment</strong>:</p>
  ...
  ml:`This is the workhorse of modern AI. <b>Semantic search</b> ranks documents by cosine similarity of embeddings. <b>Attention</b> in transformers scores every query against every key with a dot product. When you hear "Q·Kᵀ" — it's this, billions of times per second.`,
  interactive:'dot',
  quiz:[
    {q:'<code>[2, 3] · [4, −1]</code> = ?', opts:['5','11','<code>[8, −3]</code>','−5'], a:0,
     why:'2·4 + 3·(−1) = 8 − 3 = <b>5</b>. The dot product is always a single number, never a vector.'},
    {q:'Two embedding vectors have cosine similarity <b>≈ 0</b>. The texts they represent are…', opts:['Opposites in meaning','Nearly identical in meaning','Unrelated in meaning','Both empty strings'], a:2,
     why:'cos θ = 0 means orthogonal — no alignment either way. Opposite meaning would push toward −1, identical toward +1.'},
    {q:'If <code>a·b &gt; 0</code>, the angle between a and b is…', opts:['Exactly 0°','Less than 90°','Exactly 90°','More than 90°'], a:1,
     why:'cos θ &gt; 0 exactly when θ &lt; 90°. Positive dot product = "same general direction".'},
  ]
});
...
LESSONS.push({
  ...
  <div class="formula">M = [ a  b ]      î ↦ [a, c]  (column 1)<br>      [ c  d ]      ĵ ↦ [b, d]  (column 2)</div>
  <p>Every other vector follows along, because transformation is linear: <code>M[x,y] = x·(Mî) + y·(Mĵ)</code>. The <strong>columns of the matrix tell you where the basis vectors land.</strong> That single sentence unlocks all of linear algebra.</p>
  ...
  interactive:'matrix',
  quiz:[
    {q:'A matrix has columns <code>[0,1]</code> and <code>[−1,0]</code> — i.e. î lands on [0,1], ĵ lands on [−1,0]. What does it do?', opts:['Rotates 90° counterclockwise','Reflects over the x-axis','Scales by 2','Shears horizontally'], a:0,
     why:'î (pointing right) ends up pointing up; ĵ (up) ends up pointing left. That\'s a quarter-turn counterclockwise.'},
    {q:'Where does <code>M = [[2,0],[0,1]]</code> send the vector <code>[3, 4]</code>?', opts:['<code>[6, 4]</code>','<code>[3, 8]</code>','<code>[6, 8]</code>','<code>[5, 5]</code>'], a:0,
     why:'It doubles x and leaves y alone: [2·3, 1·4] = [6, 4]. A pure horizontal stretch.'},
    {q:'How do you read off what a matrix does, instantly?', opts:['Add all entries','Its columns are where î and ĵ land','Its rows are where î and ĵ land','Compute the determinant first'], a:1,
     why:'Column 1 = image of î, column 2 = image of ĵ. Everything else follows by linearity.'},
  ]
});
...
LESSONS.push({
  ...
  learn:`<p>What does <code>B·A</code> mean? <strong>Apply A first, then B.</strong> Matrix multiplication is function composition — reading right to left, like <code>B(A(x))</code>.</p>
  <p>To compute an entry: row of the left matrix · column of the right matrix (a dot product!).</p>
  <div class="formula">(B·A)ᵢⱼ = (row i of B) · (column j of A)</div>
  ...
  ml:`A deep network is literally a chain of these: <code>output = W₃·σ(W₂·σ(W₁·x))</code>. "Deep" = many compositions. And because order matters, layer order matters — you can't shuffle a network's layers and expect the same function.`,
  interactive:'matmul',
  quiz:[
    {q:'The product <code>B·A</code> applied to a vector x means…', opts:['Apply B first, then A','Apply A first, then B','Apply both simultaneously','Average the two transformations'], a:1,
     why:'B·A·x evaluates inside-out: A hits x first, then B hits the result. Right-to-left, like nested function calls B(A(x)).'},
    {q:'R rotates 90° CCW, S shears horizontally. Which is true?', opts:['R·S = S·R always','R·S ≠ S·R in general','Both equal the identity','Products of transforms are undefined'], a:1,
     why:'You proved this in the lab — composition order changes the result. Matrix multiplication is associative but not commutative.'},
    {q:'Entry (1,2) of <code>B·A</code> (row 1, column 2) equals…', opts:['B\'s row 1 · A\'s column 2','B\'s column 1 · A\'s row 2','B₁₂ × A₁₂','The sum of B\'s row 1'], a:0,
     why:'Each entry of a product is a dot product: row i of the left matrix with column j of the right matrix.'},
  ]
});
...
LESSONS.push({
  ...
  <div class="formula">det [ a  b ] = ad − bc<br>     [ c  d ]</div>
  <p>• <code>|det| = 3</code> → every area gets 3× bigger<br>
  • <code>det &lt; 0</code> → space gets <strong>flipped</strong> (mirror image)<br>
  • <code>det = 0</code> → space is <strong>squashed onto a line</strong> — area becomes 0, and the transformation can't be undone. Information is destroyed.</p>
  <p>That last case is huge: a matrix is <strong>invertible</strong> exactly when det ≠ 0.</p>`,
  ml:`det = 0 means a layer collapses dimensions — different inputs map to the same output and can never be told apart again. Related ideas show up everywhere: singular matrices break solvers, and near-zero determinants mean numerically unstable training. Also: the "log-det" term appears in normalizing flows, a family of generative models.`,
  interactive:'det',
  quiz:[
    {q:'det of <code>[[3, 1], [2, 2]]</code> = ?', opts:['4','8','6','−4'], a:0,
     why:'ad − bc = 3·2 − 1·2 = 6 − 2 = <b>4</b>. Unit squares become parallelograms of area 4.'},
    {q:'A matrix has det = 0. What happened to 2D space?', opts:['Nothing — it\'s the identity','It rotated','It collapsed onto a line (or point) — not invertible','It flipped orientation'], a:2,
     why:'Zero area scaling means everything got flattened. Two different inputs now share an output, so there\'s no way back.'},
    {q:'det = −2 means the transformation…', opts:['Shrinks area by 2 and rotates','Doubles area and flips orientation','Is not invertible','Halves all lengths'], a:1,
     why:'|−2| = 2 → areas double. The negative sign → orientation flips, like a mirror.'},
  ]
});
...
LESSONS.push({
  ...
  <div class="formula">M·v = λ·v</div>
  <p><code>v</code> is an <strong>eigenvector</strong> ("eigen" = own/characteristic), and the stretch factor <code>λ</code> is its <strong>eigenvalue</strong>.</p>
  <p>The lab uses <code>M = [[2,1],[1,2]]</code>. It has two eigendirections: along <code>[1,1]</code> vectors get stretched ×3 (λ=3), and along <code>[1,−1]</code> they're untouched (λ=1). Every other vector gets rotated toward the dominant eigendirection.</p>
  <p>Eigenvectors reveal what a matrix <em>fundamentally does</em>, stripped of all the rotating.</p>`,
  ...
  quiz:[
    {q:'v is an eigenvector of M with λ = 3. Then M·v is…', opts:['v rotated by 3°','3 times longer than v, same direction','v reflected','The zero vector'], a:1,
     why:'M·v = λv = 3v: same direction, triple the length. No rotation — that\'s the defining property.'},
    {q:'For <code>M = [[2,0],[0,5]]</code> (diagonal), the eigenvalues are…', opts:['2 and 5','0 and 0','7 only','10 only'], a:0,
     why:'Diagonal matrices stretch along the axes: î stays on the x-axis (×2), ĵ stays on the y-axis (×5). The diagonal entries ARE the eigenvalues.'},
    {q:'PCA reduces dimensions by projecting data onto…', opts:['Random directions','The top eigenvectors of the covariance matrix','The smallest eigenvectors','The data\'s mean'], a:1,
     why:'The eigenvectors with the largest eigenvalues capture the most variance — keep those, drop the rest, and you lose the least information.'},
  ]
});
...
LESSONS.push({
  ...
  learn:`<p>Time to put it all together. A neural network layer computes <code>new_representation = W·x (+ bias, + nonlinearity)</code>. Why? Because the raw data is often arranged so that no simple rule separates the classes — but in the <strong>right transformed space</strong>, a trivial rule works.</p>
  ...
  <div class="formula">x' = W·x      then classify by:  x' &gt; 0 ?</div>
  ...
  quiz:[
    {q:'Why transform data before classifying it?', opts:['It makes files smaller','In the right space, a simple boundary separates classes that were tangled in the original space','Transformation always improves accuracy','It removes the need for labels'], a:1,
     why:'A linear cut that fails in raw space can succeed after a good change of representation — that\'s the entire point of hidden layers.'},
    {q:'In <code>y = W·x</code>, training a network adjusts…', opts:['The data x','The entries of matrices like W','The number of dimensions only','The labels'], a:1,
     why:'The data is fixed; the learnable parameters are the matrix entries (weights). You just hand-optimized one with sliders.'},
    {q:'If your W had det = 0, what risk did you run?', opts:['None — det doesn\'t matter','The data would explode to infinity','Different points could collapse together, becoming impossible to separate','The classes would swap'], a:2,
     why:'det = 0 flattens space onto a line. Points from both classes can land on top of each other — information needed to classify is gone.'},
  ]
});
...
LESSONS.push({
  ...
  learn:`<p>Consider <code>f(x) = (x² − 1)/(x − 1)</code>. Plug in x = 1 and you get 0/0 — undefined. There's a <strong>hole</strong> in the graph.</p>
  ...
  <div class="formula">lim<sub>x→1</sub> (x² − 1)/(x − 1) = 2</div>
  ...
  interactive:'limits',
  quiz:[
    {q:'<code>lim<sub>x→3</sub> (x² − 9)/(x − 3)</code> = ?', opts:['0','Undefined','6','9'], a:2,
     why:'Factor: x² − 9 = (x−3)(x+3). Away from the hole, f(x) = x + 3 → 6. The function never needs a value AT 3 for the limit to exist.'},
    {q:'A function approaches 5 from the left of x=2 but 7 from the right. The limit at x=2…', opts:['Is 6 (the average)','Is 5','Is 7','Does not exist'], a:3,
     why:'Left and right limits must agree. 5 ≠ 7 → no limit. (This is what a "jump" discontinuity looks like.)'},
    {q:'f(1) is undefined but lim<sub>x→1</sub> f(x) = 2. Which is true?', opts:['Impossible — limits need f defined there','Perfectly fine — limits care about the approach, not the destination','The limit must also be undefined','f must equal 2 somewhere else'], a:1,
     why:'That\'s the whole point of limits: they describe behavior NEAR a point, ignoring (even missing) values AT it.'},
  ]
});
...
LESSONS.push({
  ...
  <div class="formula">slope = ( f(a+h) − f(a) ) / h</div>
  <p>The trick: shrink the gap <code>h</code> toward 0 (a limit!). The secant line tips into the <strong>tangent</strong> line, and its slope is the <strong>derivative</strong> f′(a):</p>
  <div class="formula">f′(a) = lim<sub>h→0</sub> ( f(a+h) − f(a) ) / h</div>
  <p>For <code>f(x) = x²/2</code> in the lab, the derivative is <code>f′(x) = x</code> — at x=2 the curve climbs at slope 2, at x=0 it's flat. The derivative is a <em>new function</em> reporting the slope everywhere.</p>`,
  ...
  quiz:[
    {q:'f′(a) measures…', opts:['The height of f at a','The instantaneous rate of change of f at a','The area under f up to a','The average value of f'], a:1,
     why:'It\'s the slope of the tangent line — how fast f is rising or falling exactly at a.'},
    {q:'For <code>f(x) = x²/2</code> (lab curve), the slope at x = 3 is…', opts:['3','4.5','9','6'], a:0,
     why:'f′(x) = x, so f′(3) = 3. You verified this pattern by shrinking h in the lab.'},
    {q:'A function has f′(c) = 0 at some point c. The tangent line there is…', opts:['Vertical','Horizontal','Undefined','At 45°'], a:1,
     why:'Zero slope = flat. These flat spots are where minima and maxima hide — the key to optimization.'},
  ]
});
...
LESSONS.push({
  ...
  <div class="formula">f(x) = xⁿ   →   f′(x) = n·xⁿ⁻¹</div>
  <p>The exponent hops down front, then drops by one: <code>x³ → 3x²</code>, <code>x² → 2x</code>, <code>x → 1</code>, and constants → 0 (flat lines have no slope).</p>
  ...
  <p>So <code>(x³ + 4x² − 7x + 2)′ = 3x² + 8x − 7</code>. Mechanical. You'll verify the rule visually in the lab.</p>`,
  ...
  interactive:'rules',
  quiz:[
    {q:'d/dx of <code>x⁵</code> = ?', opts:['<code>5x⁴</code>','<code>x⁴</code>','<code>5x⁵</code>','<code>4x⁵</code>'], a:0,
     why:'Exponent down front, drop by one: 5x⁴.'},
    {q:'d/dx of <code>3x⁴ − 2x + 7</code> = ?', opts:['<code>12x³ − 2</code>','<code>12x³ − 2x</code>','<code>3x³ − 2</code>','<code>12x³ + 7</code>'], a:0,
     why:'Term by term: 3·4x³ = 12x³, then −2x → −2, and the constant 7 → 0.'},
    {q:'Why is the derivative of a constant zero?', opts:['Constants are negative','A horizontal line has zero slope everywhere','It\'s a convention with no meaning','Because n·xⁿ⁻¹ fails'], a:1,
     why:'f(x) = 7 never changes, so its rate of change is 0 at every point. (It also follows from the power rule: 7x⁰ → 0.)'},
  ]
});
...
LESSONS.push({
  ...
  learn:`<p>What's the derivative of a <em>pipeline</em>? If <code>L = f(g(w))</code> — w flows through g, then f — the <strong>chain rule</strong> says:</p>
  <div class="formula">dL/dw = dL/du · du/dw     (where u = g(w))</div>
  ...
  <p>The lab is a micro neural net: prediction <code>u = w·x</code>, loss <code>L = (u − target)²</code>. The chain gives:</p>
  <div class="formula">dL/dw = 2(u − target) · x</div>
  ...
  quiz:[
    {q:'L = f(g(w)), with du/dw = 3 and dL/du = 2 at the current point. dL/dw = ?', opts:['5','6','2/3','1.5'], a:1,
     why:'Chain rule: multiply the local slopes. 2 × 3 = 6.'},
    {q:'In the lab\'s net, the gradient dL/dw came out <b>negative</b>. To reduce the loss you should…', opts:['Decrease w','Increase w','Leave w alone','Change x instead'], a:1,
     why:'Negative slope means L decreases as w increases. Step w in the direction OPPOSITE the gradient\'s sign... which here means increasing w. (gradient descent: w ← w − lr·grad)'},
    {q:'A 10-layer net multiplies 10 local derivatives, each ≈ 0.5. The early-layer gradient is ≈…', opts:['5','0.5','0.001 (0.5¹⁰)','10'], a:2,
     why:'0.5¹⁰ ≈ 0.00098 — nearly zero. This is the vanishing gradient problem: early layers stop learning in deep, poorly-designed nets.'},
  ]
});
...
LESSONS.push({
  ...
  <div class="formula">slope goes − → 0 → +   ⇒  local MIN<br>slope goes + → 0 → −   ⇒  local MAX</div>
  <p>The lab curve is <code>f(x) = x³/3 − x</code>, so <code>f′(x) = x² − 1</code> — critical points at x = −1 and x = +1. One is a peak, one is a valley. Beware: a <em>local</em> minimum isn't necessarily the <em>global</em> one.</p>`,
  ml:`Training = minimizing a loss function with millions of dimensions instead of one. The "flat spot" condition becomes ∇L = 0. Loss landscapes are full of local minima and saddle points — understanding critical points is understanding why training sometimes gets stuck and why optimizers like Adam exist.`,
  interactive:'optim',
  quiz:[
    {q:'f′(c) = 0 and the slope goes from negative to positive through c. Then c is…', opts:['A local maximum','A local minimum','A saddle point','Not a critical point'], a:1,
     why:'Descending, flat, then climbing — the bottom of a valley. Local min.'},
    {q:'For <code>f(x) = x³/3 − x</code>, the critical points are at…', opts:['x = 0 only','x = ±1','x = ±3','There are none'], a:1,
     why:'f′(x) = x² − 1 = 0 → x² = 1 → x = ±1. You stood on both in the lab.'},
    {q:'Why can gradient-based training get "stuck"?', opts:['Computers run out of numbers','The optimizer may settle in a local minimum or flat region, not the global best','Losses can\'t be differentiated twice','Learning rates are always too small'], a:1,
     why:'The slope is zero at ANY flat spot, so a slope-following walker can\'t tell a local valley from the deepest one.'},
  ]
});
...
LESSONS.push({
  ...
  <div class="formula">x ← x − lr · f′(x)</div>
  ...
  <p>The learning rate <code>lr</code> is the most famous hyperparameter in ML:<br>
  ...
  <p>The lab landscape has <strong>two valleys</strong> — one deeper than the other. Where you start determines where you land. Welcome to the central drama of deep learning.</p>`,
  ml:`This is THE training loop. Replace x with 70 billion weights and f with the loss over your dataset — the update rule is unchanged. Everything else (SGD, momentum, Adam) is decoration on this one line. The "loss curves" researchers stare at all day are pictures of this ball rolling.`,
  interactive:'graddesc',
  quiz:[
    {q:'At your position the slope f′(x) = +4 and lr = 0.1. Gradient descent moves you…', opts:['Right by 0.4','Left by 0.4','Right by 4','Nowhere'], a:1,
     why:'x ← x − 0.1·4 = x − 0.4. Positive slope means uphill-to-the-right, so step LEFT to descend.'},
    {q:'Your loss bounces higher and higher each step. Most likely cause?', opts:['Learning rate too small','Learning rate too large','The loss has no minimum','Too few parameters'], a:1,
     why:'Each oversized step leaps across the valley and lands higher on the far wall — the divergence you triggered in the lab.'},
    {q:'Two runs start at different points and settle in different valleys with different losses. This shows…', opts:['Gradient descent is broken','A bug in the code','Initialization matters: GD finds local minima, not guaranteed global ones','The learning rate changed itself'], a:2,
     why:'GD only follows the local slope. Different starting points can flow into different basins — why neural nets are sensitive to initialization (and why we train with random restarts, etc.).'},
  ]
});
...
LESSONS.push({
  ...
  <div class="formula">Area ≈ Σ f(xᵢ)·Δx     →     ∫ₐᵇ f(x) dx   (as Δx → 0)</div>
  ...
  <p>And the punchline of all calculus, the <strong>Fundamental Theorem</strong>: integration and differentiation are inverses —</p>
  <div class="formula">∫ₐᵇ f′(x) dx = f(b) − f(a)</div>
  <p>Slopes accumulated = total change. Two halves of one subject.</p>`,
  ml:`Integrals show up where probability lives: the area under a probability density must equal 1, expected values are integrals, and the "AUC" metric you see on every classifier eval is literally <b>A</b>rea <b>U</b>nder the <b>C</b>urve. Diffusion models (image generators) are trained around integrals of noise processes.`,
  interactive:'integrals',
  quiz:[
    {q:'∫₀³ 2x dx = ? (Hint: antiderivative of 2x is x².)', opts:['6','9','3','18'], a:1,
     why:'By the Fundamental Theorem: x² evaluated 0→3 gives 9 − 0 = 9. (It\'s also a triangle: ½·3·6 = 9.)'},
    {q:'As the number of rectangles n → ∞, the Riemann sum…', opts:['Diverges to infinity','Approaches the exact area — the integral','Approaches zero','Oscillates forever'], a:1,
     why:'Thinner slices hug the curve better; the error shrinks to 0. You watched this happen on the slider.'},
    {q:'A probability density function must satisfy…', opts:['f(x) ≤ 1 everywhere','Total area under it = 1','It must be a straight line','f(0) = 0'], a:1,
     why:'Probabilities over all outcomes sum to 1 — for continuous variables "sum" means integral. (Density values themselves CAN exceed 1.)'},
  ]
});
...
LESSONS.push({
  ...
  learn:`<p>Real loss functions take many inputs. With two, <code>f(x, y)</code>, ask the slope question one variable at a time — <strong>partial derivatives</strong>:</p>
  <div class="formula">∂f/∂x = slope if you step in x     ∂f/∂y = slope if you step in y</div>
  <p>Stack them into a vector — hello, World 1! — and you get the <strong>gradient</strong>:</p>
  <div class="formula">∇f = [ ∂f/∂x , ∂f/∂y ]</div>
  <p>The gradient is a vector that <strong>points in the steepest uphill direction</strong>, with magnitude = steepness. So <code>−∇f</code> is the fastest way down. Gradient descent in any dimension:</p>
  <div class="formula">position ← position − lr · ∇f</div>
  <p>The lab surface is <code>f(x,y) = ½(x−1)² + 0.8(y+0.8)²</code> — a tilted bowl. Read the gradient field, then ride it to the bottom.</p>`,
  ...
  quiz:[
    {q:'For <code>f(x,y) = x² + 3y</code>, ∇f = ?', opts:['<code>[2x, 3]</code>','<code>[x², 3y]</code>','<code>[2x + 3]</code>','<code>[2, 3]</code>'], a:0,
     why:'∂f/∂x treats y as constant → 2x. ∂f/∂y treats x as constant → 3. The gradient stacks them into a vector.'},
    {q:'The gradient at your location points northeast. To minimize f, step…', opts:['Northeast','Southwest','North','It doesn\'t matter'], a:1,
     why:'∇f points steepest UPHILL; descent goes exactly opposite: −∇f, i.e. southwest.'},
    {q:'Training a 70B-parameter LLM, the gradient ∇L is…', opts:['A single number','A 70-billion-dimensional vector, one slope per weight','A 70B × 70B matrix','Computed only once before training'], a:1,
     why:'One partial derivative per parameter, stacked into one colossal vector — recomputed every single training step via backprop.'},
  ]
});
...
/* ================================================================
   WRONG_WHY — misconception-specific feedback for incorrect answers.
   [lesson id][question index][wrong option index] → html string.
   Pattern: why it's wrong + the trap you likely fell into + the fix.
   ================================================================ */
Object.assign(WRONG_WHY, {
'la-vectors':[
 {0:'<b>Likely trap:</b> adding the components (−4 + 3 = −1). Magnitude isn\'t a sum — it\'s Pythagoras: <b>square</b> each component, add, then square-root: √(16+9) = √25 = 5.',
 ...
 {0:'Understandable — you can\'t <i>picture</i> a 1,536-D arrow. But the math never requires a picture: the definitions (components, magnitude, dot product) work identically in any number of dimensions. ML lives in this regime.',
 ...
 {0:'<b>Classic trap.</b> The components being 1 doesn\'t make the length 1: ‖[1,1]‖ = √(1+1) = √2 ≈ 1.41. The diagonal of a unit square is longer than its sides.',
...
'la-vecops':[
 {1:'<b>Likely trap:</b> dropping the minus signs and adding sizes. Addition is strictly component-wise, signs included: 2+(−2)=0, −1+4=3.',
 ...
 {0:'Scalars <b>cannot rotate</b> — they only stretch or flip a vector along the line it already lives on. Rotation requires a matrix (next lessons!).',
 ...
 {0:'a·b is the <b>dot product</b> — it outputs a single number. A linear combination outputs a <i>vector</i>: scalars times vectors, added: 3a − 2b.',
...
'la-dot':[
 {1:'<b>Sign slip:</b> 2·4 + 3·(−1) = 8 − 3 = 5. You computed 8 + 3 — the second product is negative. Signs ride along inside dot products.',
 ...
 {0:'Opposite meanings push cosine toward <b>−1</b> (vectors pointing against each other). Zero means <i>no alignment either way</i> — orthogonal, unrelated.',
 ...
 {0:'Exactly 0° would mean perfectly aligned (cos θ = 1) — that\'s the extreme case, not what a positive dot product implies. <i>Any</i> angle under 90° gives a·b &gt; 0.',
...
'la-matrix':[
 {1:'Reflection over the x-axis keeps î = [1,0] in place and flips ĵ to [0,−1] — matrix [[1,0],[0,−1]]. Here <i>both</i> basis vectors changed direction: î→[0,1] (right became up) and ĵ→[−1,0] (up became left). That\'s a quarter-turn CCW.',
 ...
 {1:'<b>Likely trap:</b> applying the 2 to the second coordinate. The 2 sits in the first row — it scales <b>x</b>: [2·3, 1·4] = [6, 4].',
 ...
 {0:'Entry sums have no geometric meaning. The readable structure is the <b>columns</b>: column 1 = where î lands, column 2 = where ĵ lands.',
...
'la-matmul':[
 {0:'Natural reading — left to right — but composition evaluates <b>inside-out</b>, like nested function calls: B·A·x = B(A(x)). A touches x first.',
 ...
 {0:'Numbers commute (3·5 = 5·3); matrices generally don\'t. You proved it in the lab: rotate-then-shear lands the F somewhere different than shear-then-rotate.',
 ...
 {1:'Backwards: rows come from the <b>left</b> factor, columns from the <b>right</b>. Entry (i,j) = (row i of B) · (column j of A).',
...
'la-det':[
 {1:'<b>Likely trap:</b> ad + bc = 6 + 2 = 8. The formula <b>subtracts</b>: ad − bc = 6 − 2 = 4. That minus is what detects flips.',
 ...
 {0:'The identity changes nothing and has det = <b>1</b> (areas preserved). det = 0 is the opposite extreme — total collapse.',
...
'la-eigen':[
 {0:'λ is a <b>stretch factor</b>, never an angle. Eigenvectors are precisely the vectors that <i>don\'t</i> rotate — M·v points exactly along v, 3× longer.',
 ...
 {1:'The zeros are <i>off</i> the diagonal — they just mean "no mixing between axes". The diagonal entries 2 and 5 are the per-axis stretch factors = the eigenvalues.',
 ...
 {0:'Random projections exist (and are surprisingly useful!), but PCA\'s entire point is choosing the <b>optimal</b> directions — those that preserve maximum variance: the top eigenvectors.',
...
'la-boss':[
 {0:'Compression is a side benefit sometimes, but the core reason is geometric: a boundary that\'s impossible in raw space becomes a simple straight cut in the right representation.',
 ...
 {0:'The data is the fixed ground truth. If you could edit the inputs to fit the model, you wouldn\'t be learning anything about the world.',
 ...
 {0:'You watched it matter: at det ≈ 0 both clusters fell onto one line and intermixed — unrecoverable.',
...
'c-limits':[
 {0:'Plugging in gives 0/0 — which is not 0! It\'s an "indeterminate form": a signal to do more work. Factor: (x−3)(x+3)/(x−3) → x+3 → 6.',
 ...
 {0:'Tempting, but limits never average mismatched sides — there\'s no "compromise value" the function actually approaches. Disagreement means no limit, full stop.',
 ...
 {0:'It\'s not just possible — it\'s the defining feature of limits. They examine the <i>neighborhood</i> of a point and completely ignore the point itself (even a missing one).',
...
'c-deriv':[
 {0:'Height is f(a) itself — the function\'s <i>value</i>. The derivative is about <b>change</b>: how steeply that value is rising or falling.',
 ...
 {1:'4.5 = f(3) = 9/2 — that\'s the <b>height</b> at x=3, not the slope. For f = x²/2, the slope function is f′(x) = x, so f′(3) = 3.',
 ...
 {0:'Vertical tangents are where the slope blows up to ±∞ (like x^⅓ at 0) — the opposite extreme of zero slope.',
...
'c-rules':[
 {1:'You reduced the exponent but forgot to bring the old one down front: xⁿ → <b>n</b>·xⁿ⁻¹, so x⁵ → 5x⁴.',
 ...
 {1:'Almost — but −2x differentiates to the constant −2 (power rule with n=1: x¹ → 1). The x disappears.',
 ...
 {0:'Sign is irrelevant — negative constants are just as flat as positive ones. The reason is geometric: no change, no slope.',
...
'c-chain':[
 {0:'<b>Likely trap:</b> adding the local slopes (2+3). The chain rule <b>multiplies</b> them: if u moves 3× as fast as w, and L moves 2× as fast as u, L moves 6× as fast as w.',
 ...
 {0:'Careful — that\'s the trap. The update is w ← w − lr·grad. With grad &lt; 0, subtracting a negative <b>increases</b> w. The slope says "L falls as w rises" — so rise.',
 ...
 {0:'5 = 0.5 × 10 — you multiplied by the layer count. The chain rule multiplies the slopes <i>together</i>: 0.5¹⁰ ≈ 0.001.',
...
'c-optim':[
 {0:'A local max is the reverse pattern: slope goes + → 0 → − (climb, crest, descend). Here we descend then climb: a valley floor.',
 ...
 {0:'f′(0) = 0² − 1 = −1 ≠ 0 — at x=0 the curve is mid-descent, not flat. Solve x² − 1 = 0 → x = ±1.',
 ...
 {0:'Precision isn\'t the issue — the geometry is: slope-following can\'t see past the nearest flat spot.',
...
'c-graddesc':[
 {0:'<b>Sign flip.</b> The update subtracts: x ← x − 0.1·(+4) = x − 0.4. Positive slope = uphill to the right = step LEFT.',
 ...
 {0:'Too-small lr looks like <i>crawling</i> — slow but steady descent. Bouncing ever higher is the signature of oversized steps leaping across the valley.',
 ...
 {0:'It\'s working exactly as designed — following the local slope to the nearest basin. Locality, not breakage.',
...
'c-integrals':[
 {0:'6 is f(3) — the curve\'s <b>height</b> at the endpoint. The integral accumulates area the whole way: x² from 0 to 3 = 9. (Picture the triangle: ½·3·6.)',
 ...
 {0:'More rectangles, but each one thinner — width shrinks as 1/n, so the total stays pinned near the true area. It converges, not explodes.',
 ...
 {0:'Surprisingly, no! A density can spike above 1 on a narrow interval (a tight Gaussian peaks well above 1) as long as total <b>area</b> stays 1. Heights aren\'t probabilities; areas are.',
...
'c-boss':[
 {1:'Those are the original <i>terms</i>, not their slopes. Differentiate each: ∂(x²)/∂x = 2x and ∂(3y)/∂y = 3 → ∇f = [2x, 3].',
 ...
 {0:'Northeast is the gradient\'s own direction — steepest <b>ascent</b>. Following it maximizes f. To minimize, go exactly opposite: −∇f.',
 ...
 {0:'A single number is the loss L itself. Its gradient has one slope <i>per parameter</i> — 70 billion partial derivatives stacked into one vector.',
...
// extra.js before any LESSONS.push executed — top-level await preserves order)
