...
import { SCENES, insetShapeProblems, entityFrameProblems } from './registry.js';
...
export async function mountScene(idOrSpec, container, opts){
  ...
  // INLINE-SPEC GUARD (v1.4 controls, v1.6 inset): registered scenes are
  // validated at registerScene + validateScenes, but engine.js passes INLINE
  // scene OBJECTS from a lesson's scenes:[...] straight to mountScene — those
  // bypass BOTH. Re-validate here for an object spec so the SAME mistakes fail
  // LOUD at mount (before any backend/space/driver is acquired) as they would for
  // a registered scene. Covers: control shape/bind (v1.4), inset shape (v1.6),
  // AND the entity-level frame checks — those live only in validateScenes, which
  // NEVER runs in production, so without this an inline scene's frame errors are
  // silent (the backend just falls back to `main` at draw). We eval the pure
  // entity list once at t=0 and run the shared entityFrameProblems on it.
  if(typeof idOrSpec !== 'string'){
    ...
    cp.push(...insetShapeProblems(spec.inset, spec.space));
    if(typeof spec.entities === 'function'){
      let list0;
      try{ list0 = spec.entities(pView, 0); }catch(e){ /* an eval throw surfaces through the render tick — not a frame concern */ }
      if(Array.isArray(list0)) list0.forEach((e, i) => entityFrameProblems(e, spec.inset != null).forEach(m => cp.push('entities[' + i + '] ' + m)));
    }
  ...
  // INSET SUB-SPACE (v1.6): a separate plane2 sub-space (own extent/yUp) the
  // backend renders into the declared rect. Read-only (no pointer picking), so
  // — unlike the main space — the backend owns its sizing/geometry; here we just
  // create it and hand it over. Only when a rendered main space exists.
  const insetSpace = (spec.inset && space)
    ? createSpace(spec.space, { extent: spec.inset.extent ?? 1.2, yUp: spec.inset.yUp ?? true })
    : null;
  ...
  if(insetSpace && backend.setInset) backend.setInset(insetSpace, spec.inset.rect);
  ...
  async function recover(){
    ...
    try{
      if(rebuild){
        ...
        if(space) backend.setSpace(space);
        if(insetSpace && backend.setInset) backend.setInset(insetSpace, spec.inset.rect);   // re-declare the inset on the fresh backend
        if(container) backend.mountCanvas(container);
