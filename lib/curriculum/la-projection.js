...
registerLesson({
  ...
  <div class="formula">b·â = (a·b)/‖a‖ &nbsp;&nbsp;(â = a/‖a‖, the unit direction)</div>
  <p>The <strong>vector projection</strong> puts that length back onto the line as an actual vector:</p>
  <div class="formula">proj<sub>a</sub>b = (a·b / a·a) · a</div>
  ...
  ml:`Projection is how models <b>measure</b> a vector against a direction. A single neuron computes <code>w·x</code> — an unnormalised scalar projection of the input onto its weight direction: "how much of x points my way?" Attention scores are dot products (projections of a query onto keys). And the residual — the orthogonal leftover — is the raw material of <b>least squares</b> and of the residual streams in transformers: the part the current direction couldn't explain, passed on for the next layer to handle.`,
  deeper:[
...
registerLesson({
  ...
  <div class="formula">Aᵀ(b − Ax̂) = 0 &nbsp;⟹&nbsp; AᵀA x̂ = Aᵀb &nbsp;(the normal equations)</div>
...
registerLesson({
  ...
  <div class="formula">u₁ = v₁<br>u₂ = v₂ − proj<sub>u₁</sub>v₂<br>u₃ = v₃ − proj<sub>u₁</sub>v₃ − proj<sub>u₂</sub>v₃</div>
