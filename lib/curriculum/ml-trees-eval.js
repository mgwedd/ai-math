...
registerLesson({
  ...
  <p>Which cut is best? The one that leaves each side as <strong>pure</strong> as possible — ideally all one class. We score impurity two common ways, where \\(p\\) is the fraction of the majority-vs-minority split in a region:</p>
...
registerLesson({
  ...
  <p>Why does averaging help? Each weak tree makes different, largely <em>independent</em> mistakes. Independent errors partly cancel when you vote, so the ensemble's variance shrinks roughly like \\(\\dfrac{1}{N}\\) for N truly independent learners — while the bias stays about the same. The forcing of feature-subsets is what keeps the trees from all making the <em>same</em> mistake (decorrelation).</p>
  ...
  deeper:[
   ...
   {title:'🚀 Go deeper: out-of-bag error, and the variance floor', body:'Each tree is trained on a <em>bootstrap</em> resample — drawn with replacement, same size as the original data. That leaves some rows unseen by any given tree: the chance a specific row is never picked in N draws is \\((1-1/N)^N \\to e^{-1} \\approx 37\\%\\) as N grows. So roughly a third of the data is "out-of-bag" for each tree — never touched during its training — and scoring each tree only on its own out-of-bag rows gives a <b>free validation estimate</b> (the <em>OOB error</em>), no held-out split required. But decorrelation has a floor. Breiman\'s 2001 analysis shows the ensemble\'s variance doesn\'t vanish as N → ∞; it converges to \\(\\rho\\sigma^2\\), where σ² is a single tree\'s variance and ρ is the average pairwise correlation between trees\' predictions. Averaging kills the <em>uncorrelated</em> part of the error but leaves the <em>shared</em> part standing — which is exactly why random feature subsets (not just bootstrap resampling alone) matter: they push ρ down, lowering the floor itself, not just approaching it faster. (Breiman, "Random Forests," <em>Machine Learning</em>, 2001.)'}],
