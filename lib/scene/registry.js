...
import { controlProblems } from './seams/controls.js';
...
export function validateScenes(){
  ...
  for(const spec of SCENES){
    ...
    // Control cross-check (v1.4): a slider must bind a DECLARED, numeric
    // (scalar) param — a slider on a vec or a missing param is nonsensical.
    if(Array.isArray(spec.controls)){
      spec.controls.forEach((c, i) => {
        if(!c || c.kind !== 'slider' || typeof c.param !== 'string') return;
        if(!atoms[c.param]) problems.push(tag + 'controls[' + i + '] slider param `' + c.param + '` is not a declared param');
        else if(typeof atoms[c.param].get() !== 'number') problems.push(tag + 'controls[' + i + '] slider param `' + c.param + '` must be a numeric (scalar) param');
      });
    }
