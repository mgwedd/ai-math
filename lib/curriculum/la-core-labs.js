...
INTERACTIVES.detArea = function(stage){
  ...
  const ctx = F.ctx, W = F.W, H = F.H;
  const P = plane(ctx, W, H, 46, W / 2, H / 2);
...
INTERACTIVES.detProductFig = function(stage){
  ...
  function panel(ox, M, title){
    const P = plane(ctx, W, H, 20, ox, 150);
    P.grid();
...
INTERACTIVES.detProduct = function(stage, api){
  ...
  const m = api.missions([
    {text:'Build an area-preserving chain: det(B·A) = 1 using two DIFFERENT transforms', xp:20,
      check:s => Math.abs(s.detBA - 1) < 0.05 && s.ai !== s.bi},
  ...
  function draw(){
    ...
    L.ctx.closePath(); L.ctx.fillStyle = col+'.22)'; L.ctx.fill();
    L.ctx.strokeStyle = col+'.9)'; L.ctx.lineWidth = 2.5; L.ctx.stroke();
    P.arrow(0,0,BA[0],BA[2],detBA<0?'#ff5c7a':'#2dd4a0',3.5,'î→');
    P.arrow(0,0,BA[1],BA[3],detBA<0?'#ff5c7a':'#2dd4a0',3.5,'ĵ→');
...
INTERACTIVES.detShear = function(stage, api){
  ...
  api.predict({
    ...
    answer: 0,
    reveal: 'Shearing slides one column along the other; base and height are unchanged, so the area — and \\(\\det\\) — stay the same. That is why adding a multiple of one row to another during elimination never changes \\(\\det\\).',
  });
  ...
  const m = api.missions([
    ...
    {text:'Collapse it: scale a column to zero so det = 0 (not invertible)', xp:25,
  ...
  function draw(){
    ...
    L.ctx.beginPath(); L.ctx.moveTo(P.sx(-6*s), P.sy(0)); L.ctx.lineTo(P.sx(6*s), P.sy(0)); L.ctx.stroke();
  ...
  slider(L.ctrl, 'shear k (slide column 2 along column 1)', -2, 2, 0.1, 0, x=>x.toFixed(1), x=>{ k=x; draw(); });
