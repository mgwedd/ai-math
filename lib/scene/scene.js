...
import { SCENES, insetShapeProblems } from './registry.js';
...
export async function mountScene(idOrSpec, container, opts){
  ...
  // bypass BOTH. Re-validate the SHAPE-level checks here for an object spec so a
  // min>max / out-of-range / duplicate slider, or a bad inset rect, fails LOUD at
  // mount (before any backend is acquired) instead of rendering a stuck control
  // or an off-canvas inset. (Entity-eval frame checks stay in validateScenes;
  // an inline scene's entity errors still surface through the render tick.)
