...
/* ================================================================
   EIGEN EXPOSITION FIGURES (headless-safe, animated) + two labs.
   ================================================================ */

/* eigSweep — sweep a vector around the circle; M·v turns green only when it
   stays parallel to v (an eigendirection). The "skeleton" of a transform. */
INTERACTIVES.eigSweep = function(stage){
  const F = figCanvas(stage, 640, 340);
  const ctx = F.ctx, W = F.W, H = F.H;
  const P = plane(ctx, W, H, 36, W / 2, H / 2);   // 36 keeps Mv (max ‖·‖≈4.5) on-canvas through the whole sweep
  const M = [2, 1, 1, 2];
  let t = 0, raf = 0;
  function draw(){
    clearBg(ctx, W, H); P.grid();
    // faint dashed gold eigenlines
    ctx.save();
    ctx.setLineDash([6, 6]); ctx.lineWidth = 1.5; ctx.strokeStyle = 'rgba(255,201,77,.25)';
    ctx.beginPath(); ctx.moveTo(P.sx(-3), P.sy(-3)); ctx.lineTo(P.sx(3), P.sy(3)); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(P.sx(-3), P.sy(3)); ctx.lineTo(P.sx(3), P.sy(-3)); ctx.stroke();
    ctx.restore();
    const phi = t * 0.5;
    const v = { x: 2 * Math.cos(phi), y: 2 * Math.sin(phi) };
    const Mv = { x: M[0] * v.x + M[1] * v.y, y: M[2] * v.x + M[3] * v.y };
    const nv = Math.hypot(v.x, v.y), nMv = Math.hypot(Mv.x, Mv.y);
    const cos = (v.x * Mv.x + v.y * Mv.y) / (nv * nMv);
    const ang = Math.acos(Math.max(-1, Math.min(1, Math.abs(cos)))) * 180 / Math.PI;
    const par = ang < 3;
    P.arrow(0, 0, v.x, v.y, '#7c5cff', 3, 'v');
    P.arrow(0, 0, Mv.x, Mv.y, par ? '#2dd4a0' : '#ff5c7a', 3, 'Mv');
    ctx.fillStyle = par ? '#2dd4a0' : '#8b93b8'; ctx.font = '700 13px ' + FONT(); ctx.textAlign = 'left';
    ctx.fillText(par ? 'parallel — eigendirection!' : 'angle(v, Mv) = ' + ang.toFixed(0) + '°', 16, 26);
  }
  draw();
  function loop(){ t += 0.02; draw(); raf = requestAnimationFrame(loop); }
  raf = requestAnimationFrame(loop);
...
/* eigPower — power iteration: apply M and renormalize; almost any start
   converges to the dominant eigenvector [1,1]. Cycles through fixed starts. */
INTERACTIVES.eigPower = function(stage){
  const F = figCanvas(stage, 640, 340);
  const ctx = F.ctx, W = F.W, H = F.H;
  const P = plane(ctx, W, H, 60, W / 2, H / 2);
  const M = [2, 1, 1, 2];
  const STARTS = [[-0.7, 0.95], [0.9, -0.5], [-1, -0.2]];
  function apply(M, v){ return { x: M[0] * v.x + M[1] * v.y, y: M[2] * v.x + M[3] * v.y }; }
  function norm(v){ const k = Math.hypot(v.x, v.y) || 1; return { x: v.x / k, y: v.y / k }; }
  let si = 0, u = norm({ x: STARTS[0][0], y: STARTS[0][1] }), trail = [], frame = 0, steps = 0, raf = 0;
  function draw(){
    clearBg(ctx, W, H); P.grid();
    // faint dashed gold [1,1] eigenline
    ctx.save();
    ctx.setLineDash([6, 6]); ctx.lineWidth = 1.5; ctx.strokeStyle = 'rgba(255,201,77,.25)';
    ctx.beginPath(); ctx.moveTo(P.sx(-3), P.sy(-3)); ctx.lineTo(P.sx(3), P.sy(3)); ctx.stroke();
    ctx.restore();
    // trail as fading cyan dots
    trail.forEach((p, i) => {
      ctx.globalAlpha = 0.15 + 0.6 * (i / Math.max(1, trail.length - 1));
      P.dot(p.x, p.y, 5, '#00d4ff');
    });
    ctx.globalAlpha = 1;
    P.arrow(0, 0, u.x, u.y, '#00d4ff', 3, 'u');
    ctx.fillStyle = '#8b93b8'; ctx.font = '700 13px ' + FONT(); ctx.textAlign = 'left';
    ctx.fillText('power iteration → dominant eigenvector [1,1]', 16, 26);
  }
  draw();
  function loop(){
    frame++;
    if(frame % 30 === 0){
      u = norm(apply(M, u)); steps++;
      trail.push({ ...u }); if(trail.length > 14) trail.shift();
      if(steps >= 10){ si = (si + 1) % 3; u = norm({ x: STARTS[si][0], y: STARTS[si][1] }); trail = []; steps = 0; }
    }
...
/* eigBasis — symmetric M morphs from I to [[2,1],[1,2]]; the unit circle
   becomes an ellipse aligned to the perpendicular eigen-axes [1,1],[1,-1]. */
INTERACTIVES.eigBasis = function(stage){
  const F = figCanvas(stage, 640, 340);
  const ctx = F.ctx, W = F.W, H = F.H;
  const P = plane(ctx, W, H, 50, W / 2, H / 2);
  const TGT = [2, 1, 1, 2], I = [1, 0, 0, 1];
  let t = 0, raf = 0;
  function draw(){
    clearBg(ctx, W, H); P.grid();
    // dashed gold eigenlines [1,1] and [1,-1]
    ctx.save();
    ctx.setLineDash([6, 6]); ctx.lineWidth = 1.5; ctx.strokeStyle = 'rgba(255,201,77,.25)';
    ctx.beginPath(); ctx.moveTo(P.sx(-3), P.sy(-3)); ctx.lineTo(P.sx(3), P.sy(3)); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(P.sx(-3), P.sy(3)); ctx.lineTo(P.sx(3), P.sy(-3)); ctx.stroke();
    ...
    const M = I.map((v, i) => v + (TGT[i] - v) * p);
    // faint original unit circle
    ctx.save();
    ctx.globalAlpha = 0.3; ctx.strokeStyle = '#5560a0'; ctx.lineWidth = 1.5;
    ctx.beginPath();
    for(let i = 0; i <= 64; i++){ const th = i / 64 * Math.PI * 2, x = Math.cos(th), y = Math.sin(th);
      const X = P.sx(x), Y = P.sy(y); i === 0 ? ctx.moveTo(X, Y) : ctx.lineTo(X, Y); }
    ctx.closePath(); ctx.stroke(); ctx.restore();
    // image of the unit circle (cyan ellipse)
    ctx.strokeStyle = '#00d4ff'; ctx.lineWidth = 2.5;
    ctx.beginPath();
    for(let i = 0; i <= 64; i++){ const th = i / 64 * Math.PI * 2, x = Math.cos(th), y = Math.sin(th);
      const tx = M[0] * x + M[1] * y, ty = M[2] * x + M[3] * y;
      const X = P.sx(tx), Y = P.sy(ty); i === 0 ? ctx.moveTo(X, Y) : ctx.lineTo(X, Y); }
    ctx.closePath(); ctx.stroke();
    // eigen-image points
    const s = 1 / Math.SQRT2;
    const g1 = { x: M[0] * s + M[1] * s, y: M[2] * s + M[3] * s };
    const g2 = { x: M[0] * s - M[1] * s, y: M[2] * s - M[3] * s };
    P.dot(g1.x, g1.y, 6, '#2dd4a0');
    P.dot(g2.x, g2.y, 6, '#ffc94d');
    ctx.fillStyle = '#8b93b8'; ctx.font = '700 13px ' + FONT(); ctx.textAlign = 'left';
    ctx.fillText('stretch ×' + (1 + 2 * p).toFixed(1) + ' along [1,1],  ×1 along [1,-1]', 16, 26);
  }
  draw();
  function loop(){ t += 0.02; draw(); raf = requestAnimationFrame(loop); }
  raf = requestAnimationFrame(loop);
...
/* Lab 3 — power iteration: converge an arbitrary start onto the dominant
   eigenvector, prove start-independence, then race a wide-gap matrix. */
INTERACTIVES.eigPowerLab = function(stage, api){
  const L = makeLab(stage);
  const MATS = [
    { name: 'M = [[2,1],[1,2]]', M: [2, 1, 1, 2] },
    { name: 'M = [[3,2],[2,3]]', M: [3, 2, 2, 3] },
  ];
  const STARTS = [[-0.7, 0.95], [0.9, -0.5], [-1, -0.2]];
  let mi = 0, si = 0, n = 0, trail = [], timer = null, counted = false, convergedRuns = 0, usedNewStart = false;
  function apply(M, v){ return { x: M[0] * v.x + M[1] * v.y, y: M[2] * v.x + M[3] * v.y }; }
  function norm(v){ const k = Math.hypot(v.x, v.y) || 1; return { x: v.x / k, y: v.y / k }; }
  function normStart(s){ return norm({ x: s[0], y: s[1] }); }
  let u = normStart(STARTS[0]);

  api.predict({
    prompt: 'You repeatedly apply \\(M\\) to a vector and renormalize. Where does it end up?',
    choices: ['Along the dominant eigenvector (largest λ)', 'Along the smallest-λ eigenvector', 'Spinning forever, never settling'],
    answer: 0,
    reveal: 'Each application amplifies the component along the largest-λ eigenvector most, so after several steps that direction dominates: the vector converges to the <b>dominant eigenvector</b>. This is power iteration — the core of PageRank.',
  });

  const m = api.missions([
    {text:'Converge to the dominant eigenvector: angle to [1,1] under 2° (either matrix)', xp:20, check:s=>s.angDom<2},
    {text:'Prove it is start-independent: press New start and converge a SECOND time', xp:25, check:s=>s.convergedRuns>=2 && s.usedNewStart},
    {text:'Converge FAST: on the wide-gap matrix M = [[3,2],[2,3]], reach it in ≤ 6 applies', xp:20, check:s=>s.mi===1 && s.angDom<2 && s.n<=6},
  ]);

  const P = plane(L.ctx, L.W, L.H, 60);
  function draw(){
    clearBg(L.ctx, L.W, L.H); P.grid();
    // faint dashed gold [1,1] line
    L.ctx.save();
    L.ctx.setLineDash([6, 6]); L.ctx.lineWidth = 1.5; L.ctx.strokeStyle = 'rgba(255,201,77,.25)';
    L.ctx.beginPath(); L.ctx.moveTo(P.sx(-4), P.sy(-4)); L.ctx.lineTo(P.sx(4), P.sy(4)); L.ctx.stroke();
    L.ctx.restore();
    trail.forEach((p, i) => {
      L.ctx.globalAlpha = 0.15 + 0.6 * (i / Math.max(1, trail.length - 1));
      P.dot(p.x, p.y, 5, '#00d4ff');
    });
    L.ctx.globalAlpha = 1;
    P.arrow(0, 0, u.x, u.y, '#00d4ff', 3.5, 'u');
    const c = Math.abs(u.x * 1 + u.y * 1) / (Math.hypot(u.x, u.y) * Math.SQRT2);
    const angDom = Math.acos(Math.min(1, c)) * 180 / Math.PI;
    if(angDom < 2 && !counted){ counted = true; convergedRuns++; }
    L.readout.innerHTML = MATS[mi].name + '<br>step ' + n + '<br>angle to [1,1] = <span style="color:' + (angDom < 2 ? '#2dd4a0' : '#cdd4f0') + '">' + angDom.toFixed(1) + '°</span>' + (angDom < 2 ? '  ✓ CONVERGED' : '') + '<br>runs converged: ' + convergedRuns;
    m.update({ angDom, n, mi, convergedRuns, usedNewStart });
  }
  function step(){
    u = norm(apply(MATS[mi].M, u)); n++;
    trail.push({ ...u }); if(trail.length > 16) trail.shift();
    draw();
  }
  function stopAuto(){ if(timer){ clearInterval(timer); timer = null; runBtn.textContent = '▶ Auto-run'; } }

  const matRow = chips(L.ctrl, 'matrix', MATS.map(x => x.name), (i, b, row) => {
    mi = i; u = normStart(STARTS[si]); n = 0; trail = []; counted = false;
    [...row.children].forEach(c => c.classList.remove('on')); b.classList.add('on');
    draw();
  });
  matRow.children[0].classList.add('on');

  const d = document.createElement('div'); d.className = 'ctrl'; const row = document.createElement('div'); row.className = 'chipbtns';
  const stepBtn = document.createElement('button'); stepBtn.className = 'chip'; stepBtn.textContent = 'Apply M ×1'; stepBtn.onclick = step;
  const runBtn = document.createElement('button'); runBtn.className = 'chip'; runBtn.textContent = '▶ Auto-run';
  runBtn.onclick = () => { if(timer){ stopAuto(); } else { timer = setInterval(step, 180); runBtn.textContent = '⏸ Pause'; } };
  const newBtn = document.createElement('button'); newBtn.className = 'chip'; newBtn.textContent = 'New start';
  newBtn.onclick = () => { si = (si + 1) % STARTS.length; u = normStart(STARTS[si]); n = 0; trail = []; counted = false; usedNewStart = true; draw(); };
  row.appendChild(stepBtn); row.appendChild(runBtn); row.appendChild(newBtn); d.appendChild(row); L.ctrl.appendChild(d);

  note(L.ctrl, 'Press <b>Apply M ×1</b> (or Auto-run) and watch the arrow swing onto the dominant eigenvector [1,1]. <b>New start</b> drops a fresh vector — it still lands in the same place. The wide-gap matrix converges in fewer steps.');
  registerCleanup(() => stopAuto());
  draw();
};

/* Lab 4 — trace & determinant steer the eigenvalues of a symmetric 2×2. */
INTERACTIVES.eigTraceDet = function(stage, api){
  const L = makeLab(stage);
  let a = 2, b = 0, d = 2;
  function eig(a, b, d){ const T = a + d, D = a * d - b * b, disc = T * T - 4 * D, r = Math.sqrt(Math.max(0, disc)); return { T, D, disc, l1: (T + r) / 2, l2: (T - r) / 2 }; }

  api.predict({
    prompt: 'For a 2×2, \\(\\lambda_1+\\lambda_2=\\operatorname{trace}\\) and \\(\\lambda_1\\lambda_2=\\det\\). A matrix has trace 7 and det 10. Predict its <b>larger</b> eigenvalue.',
    input: true, placeholder: 'larger λ', answer: 5, tol: 0.1, unit: '',
    reveal: 'The eigenvalues are the roots of \\(\\lambda^2 - 7\\lambda + 10 = (\\lambda-2)(\\lambda-5) = 0\\): λ = 5 and 2. The larger is <b>5</b>.',
  });

  const m = api.missions([
    {text:'Make the eigenvalues sum to 6: set the trace a + d = 6', xp:20, check:s=>Math.abs(s.T-6)<0.1},
    {text:'Make them multiply to 8 with a real off-diagonal: det = 8 and |b| ≥ 1', xp:25, check:s=>Math.abs(s.D-8)<0.15 && Math.abs(s.b)>=1 && s.disc>0.05},
    {text:'Hit λ = 4 and λ = 1 (trace 5, det 4)', xp:20, check:s=>Math.abs(s.l1-4)<0.15 && Math.abs(s.l2-1)<0.15},
  ]);

  const P = plane(L.ctx, L.W, L.H, 44);
  function draw(){
    clearBg(L.ctx, L.W, L.H); P.grid();
    const { T, D, disc, l1, l2 } = eig(a, b, d);
    // faint eigen-axis lines — the ACTUAL eigendirections of symmetric [[a,b],[b,d]]:
    // they lie at θ = ½·atan2(2b, a−d), which is the diagonals only when a=d.
    const th = 0.5 * Math.atan2(2 * b, a - d);
    const e1 = { x: Math.cos(th), y: Math.sin(th) }, e2 = { x: -Math.sin(th), y: Math.cos(th) };
    L.ctx.save();
    L.ctx.setLineDash([6, 6]); L.ctx.lineWidth = 1.2; L.ctx.strokeStyle = 'rgba(255,201,77,.18)';
    L.ctx.beginPath(); L.ctx.moveTo(P.sx(-5 * e1.x), P.sy(-5 * e1.y)); L.ctx.lineTo(P.sx(5 * e1.x), P.sy(5 * e1.y)); L.ctx.stroke();
    L.ctx.beginPath(); L.ctx.moveTo(P.sx(-5 * e2.x), P.sy(-5 * e2.y)); L.ctx.lineTo(P.sx(5 * e2.x), P.sy(5 * e2.y)); L.ctx.stroke();
    L.ctx.restore();
    // unit-square image parallelogram: columns [a,b] and [b,d]
    const col1 = { x: a, y: b }, col2 = { x: b, y: d };
    const fill = Math.abs(D) < 0.05 ? 'rgba(255,201,77,.18)' : (D < 0 ? 'rgba(255,92,122,.18)' : 'rgba(45,212,160,.18)');
    const edge = Math.abs(D) < 0.05 ? '#ffc94d' : (D < 0 ? '#ff5c7a' : '#2dd4a0');
    L.ctx.beginPath();
    L.ctx.moveTo(P.sx(0), P.sy(0));
    L.ctx.lineTo(P.sx(col1.x), P.sy(col1.y));
    L.ctx.lineTo(P.sx(col1.x + col2.x), P.sy(col1.y + col2.y));
    L.ctx.lineTo(P.sx(col2.x), P.sy(col2.y));
    L.ctx.closePath();
    L.ctx.fillStyle = fill; L.ctx.fill();
    L.ctx.strokeStyle = edge; L.ctx.lineWidth = 2; L.ctx.stroke();
    P.arrow(0, 0, col1.x, col1.y, '#7c5cff', 3);
    P.arrow(0, 0, col2.x, col2.y, '#b9a8ff', 3);
    L.readout.innerHTML = 'M = [' + fmt2(a) + ' ' + fmt2(b) + '; ' + fmt2(b) + ' ' + fmt2(d) + ']<br>trace = a + d = ' + T.toFixed(2) + '<br>det = ad − b² = ' + D.toFixed(2) + '<br>λ₁ = ' + l1.toFixed(2) + '   λ₂ = ' + l2.toFixed(2);
    m.update({ T, D, disc, l1, l2, b });
  }
  slider(L.ctrl, 'a', -1, 5, 0.25, 2, x => x.toFixed(2), x => { a = x; draw(); });
  slider(L.ctrl, 'd', -1, 5, 0.25, 2, x => x.toFixed(2), x => { d = x; draw(); });
  slider(L.ctrl, 'b (off-diagonal)', -3, 3, 0.25, 0, x => x.toFixed(2), x => { b = x; draw(); });
  note(L.ctrl, 'For any 2×2 the eigenvalues sum to the <b>trace</b> (a + d) and multiply to the <b>determinant</b> (ad − b²). A symmetric matrix always has real eigenvalues. Move the sliders and watch λ₁, λ₂ follow trace and det.');
  draw();
};
