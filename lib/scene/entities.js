...
//
// FRAME (v1.6): every constructor materializes `frame: p.frame || 'main'`
// (CONTRACT Amendment v1.6 §1). `'main'` draws in the scene's main space;
// `'inset'` routes the descriptor into the declared inset sub-space at apply
// time (the backend re-parents; the diff never partitions). `frame` is an
// ordinary value prop — entities stay PURE §3 descriptors, no `label.at`
// overload. validateScenes rejects `frame:'inset'` without a declared `inset`
// and rejects a `handle` on an inset entity (the inset is read-only in v1.6).

/** Reference grid, morphable under a 2x2 matrix [a,b,c,d]. */
export function grid(opts){ const p = o(opts);
  const step = p.step ?? 1, extent = p.extent ?? 6;
  // Loud-in-dev validation (VISUAL_FIRST §8): a non-positive/non-finite step or
  // a non-finite extent makes the draw-loop `for(i=-E;i<=E;i+=step)` never
  // terminate (reviewer-reproduced hang). Reject at construction, not at draw.
  if(!(step > 0) || !Number.isFinite(step))
    throw new Error('grid: step must be a positive finite number, got ' + step);
  if(!(extent >= 0) || !Number.isFinite(extent))
    throw new Error('grid: extent must be a non-negative finite number, got ' + extent);
  return { kind: 'grid', key: p.key, frame: p.frame || 'main', matrix: p.matrix || [1, 0, 0, 1],
           extent, step, color: p.color || 'grid',
           handle: p.handle }; }

/** Arrow from origin (or opts.from) to v. `handle` binds the tip. */
export function vector(v, opts){ const p = o(opts);
  return { kind: 'vector', key: p.key, frame: p.frame || 'main', v, from: p.from || { x: 0, y: 0 },
           color: p.color || 'accent', label: p.label, width: p.width,
           handle: p.handle }; }

export function point(v, opts){ const p = o(opts);
  return { kind: 'point', key: p.key, frame: p.frame || 'main', v, color: p.color || 'accent',
...
export function segment(a, b, opts){ const p = o(opts);
  return { kind: 'segment', key: p.key, frame: p.frame || 'main', a, b, color: p.color || 'muted',
...
export function curve(fn, opts){ const p = o(opts);
  return { kind: 'curve', key: p.key, frame: p.frame || 'main', fn, domain: p.domain,
...
export function area(fn, opts){ const p = o(opts);
  return { kind: 'area', key: p.key, frame: p.frame || 'main', fn, domain: p.domain, to: p.to ?? 0,
...
export function polygon(pts, opts){ const p = o(opts);
  return { kind: 'polygon', key: p.key, frame: p.frame || 'main', pts, color: p.color || 'accent',
           fill: p.fill, alpha: p.alpha, closed: p.closed !== false,
           handle: p.handle }; }

/**
 * Text label. `at` DEFAULTS to `'readout'` (a11y-first): a label with no `at`
 * renders into the DOM readout strip (an aria-live mirror of canvas state) and
 * NOT onto the canvas. Pass an explicit `at: vec(x,y)` to place it on-canvas.
 * CONTRACT §3 (`opts.at?: vec|'readout'`).
 */
export function label(textOrFn, opts){ const p = o(opts);
  return { kind: 'label', key: p.key, frame: p.frame || 'main', text: textOrFn, at: p.at ?? 'readout',
...
export function angleArc(u, w, opts){ const p = o(opts);
  return { kind: 'angleArc', key: p.key, frame: p.frame || 'main', u, w, at: p.at || { x: 0, y: 0 },
...
export function dropLine(v, opts){ const p = o(opts);
  return { kind: 'dropLine', key: p.key, frame: p.frame || 'main', v, to: p.to || 'axes',
...
export function cellGrid(values, opts){ const p = o(opts);
  return { kind: 'cellGrid', key: p.key, frame: p.frame || 'main', values, at: p.at || { x: 0, y: 0 },
...
export function bars(values, opts){ const p = o(opts);
  return { kind: 'bars', key: p.key, frame: p.frame || 'main', values, at: p.at || { x: 0, y: 0 },
