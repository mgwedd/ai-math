...
registerLesson({
  ...
  // P2 wave C: additive scene arc (CONTRACT §1; lib/curriculum/scenes/la-vectors.js).
  // `labs` above is untouched — this is the gentlest on-ramp lesson, so the
  // arc runs ALONGSIDE the existing labs rather than replacing them this wave.
  scenes:[
...
registerLesson({
  ...
  // Scene Kit (P1 migration wave 1): the six-scene arc
  // (lib/curriculum/scenes/la-vecops.js), ending in the randomized
  // vecops.capstone exam. ADDITIVE — the labs above stay (CONTRACT v1.4 §5).
  scenes:[
...
registerLesson({
  ...
  // v1.4 §2: la-dot is SCENES-FIRST. The seven-scene flagship arc
  // (lib/curriculum/scenes/la-dot.js), ending in the randomized dot.capstone
  // exam, IS the labs step now — the legacy `labs:[dot, dotTarget]` block was
  // removed so the scene arc is the real interactive content. INTERACTIVES.dot
  // / dotTarget component code stays in-tree (deletion deferred until the
  // scenes path is proven in prod). No other lesson loses labs this wave.
  scenes:[
...
registerLesson({
  ...
  // Scene Kit (P1 migration wave 1): the six-scene arc
  // (lib/curriculum/scenes/la-matrix.js), ending in the randomized
  // matrix.capstone exam. ADDITIVE — the labs above stay (CONTRACT v1.4 §5).
  scenes:[
...
registerLesson({
  ...
  // Scene Kit (P2 migration wave 2): the six-scene arc
  // (lib/curriculum/scenes/la-matmul.js), ending in the randomized
  // matmul.capstone exam. ADDITIVE — the labs above stay (CONTRACT v1.5 §5).
  scenes:[
...
registerLesson({
  ...
  // Scene Kit (P2 migration wave 2): the six-scene arc
  // (lib/curriculum/scenes/la-det.js), ending in the randomized
  // det.capstone exam. ADDITIVE — the labs above stay (CONTRACT v1.5 §5).
  scenes:[
...
registerLesson({
  ...
  expositions:[
    {key:'skeleton', title:'Most vectors rotate — eigendirections don\'t', figure:'eigSweep', caption:'Apply \\(M\\) to vectors all around the circle: almost every one gets <b>rotated</b> off its direction. Only along the two <b>eigendirections</b> does \\(Mv\\) stay parallel to \\(v\\) — the skeleton of the transformation.'},
    {key:'power', title:'Power iteration finds the dominant eigenvector', figure:'eigPower', caption:'Apply \\(M\\) and renormalize, over and over: almost any start converges to the <b>dominant eigenvector</b> (largest \\(\\lambda\\)) — here \\([1,1]\\). This is the engine behind PageRank.'},
    {key:'axes', title:'Symmetric ⇒ perpendicular eigen-axes', figure:'eigBasis', caption:'\\(M=\\begin{bmatrix}2&1\\\\1&2\\end{bmatrix}\\) is symmetric, so it acts as a pure <b>stretch along perpendicular axes</b>: ×3 along \\([1,1]\\), ×1 along \\([1,-1]\\). The unit circle becomes an ellipse aligned to those axes — why PCA\'s axes come out orthogonal.'},
  ],
  labs:[
    ...
    {key:'power', title:'Power iteration — find the dominant eigenvector', interactive:'eigPowerLab',
     intro:'<p>Repeatedly apply \\(M\\) and renormalize. Watch an arbitrary start swing onto the <strong>dominant eigenvector</strong> — the algorithm behind PageRank. Then prove it barely depends on where you started.</p>'},
    {key:'tracedet', title:'Trace & determinant → eigenvalues', interactive:'eigTraceDet',
     intro:'<p>For any 2×2, \\(\\lambda_1+\\lambda_2=\\operatorname{trace}\\) and \\(\\lambda_1\\lambda_2=\\det\\). Set a symmetric matrix and steer its eigenvalues by tuning the trace and determinant — the algebra behind the geometry.</p>'},
  ],
  // Scene Kit (P2 wave B migration): the six-scene arc
  // (lib/curriculum/scenes/la-eigen.js), ending in the randomized
  // eigen.capstone exam. ADDITIVE — the labs above stay (CONTRACT §5).
  scenes:[
...
// Scene Kit registry (CONTRACT §1): registers la-dot's seven scenes into
// SCENES so the `scenes:[...]` entry on the la-dot lesson above (and its
// CONTRACT §7.1 capstone-last check, below) actually resolves against real
// content instead of only synthetic test lessons.
await import('./scenes/index.js');
