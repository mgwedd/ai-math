...
registerScene({
  ...
  goals: [
    ...
    goal('Cancel the height: make the sum lie flat (its y ≈ 0) while still pointing right, to see how per-axis cancellation in vector addition is exactly why you can correct one coordinate of an error independently of the other',
...
registerScene({
  ...
  goals: [
    ...
    goal('Attend almost entirely to v3: its weight above 0.85, to see how an attention output can never leave the convex hull of its values — it can shift toward one vertex, but never invent something outside the set it’s attending over',
