...
registerScene({
  ...
  goals: [
    goal('Drag the probe onto the goal 🏁 to see how a single reward number fully defines "success" here',
    ...
    goal('Now find the pit ☠️ to see how a negative terminal reward defines "failure" just as completely',
...
registerScene({
  ...
  goals: [
    // ANTI-GAMING: excludes terminals (isTerminal guard) — V[GOAL]=1 is fixed
    // at init, never swept, so probing the goal itself would trivially clear
    // V₁ > 0.5 without ever exercising a backup (review-confirmed defect).
    goal('Probe a state right next to 🏁 (not the terminal itself) to see how one Bellman backup already lets a NEIGHBOR "feel" the goal',
      (s) => { const c = cellOf(s.probe); return !isTerminal(c.r, c.c) && valueIterate(GAMMA, 1)[c.r][c.c] > 0.5; },
    ...
    goal('Now probe a state 3–4 steps from 🏁 to see how a single sweep only reaches IMMEDIATE neighbors — value hasn’t traveled this far yet',
...
registerScene({
  ...
  entities: (p) => {
    ...
    return [
      ...
      ...wallEntities(),
      ...terminalMarkers(),
      label('γ = ' + f2(p.gamma) + '    V(start) = ' + f3(V[START.r][START.c]) + '    (start is the farthest cell from 🏁)', { at: 'readout' }),
    ];
  },
  goals: [
    goal('Raise γ until V(start) > 0.1 to see how a higher discount factor GROWS the agent’s effective horizon',
    ...
    goal('Now lower γ until V(start) < 0 to see how a low discount factor COLLAPSES that horizon back to just the immediate cost',
...
registerScene({
  ...
  goals: [
    goal('Advance k until V(start) > 0 to see how each sweep propagates value exactly one hop farther from a terminal',
    ...
    goal('Push k further until the residual hits (numerically) zero to see the Bellman backup actually reach its fixed point',
...
registerScene({
  ...
  goals: [
    goal('Find a state whose greedy arrow points E to see how the optimal action is read straight off V∗ by one argmax — no separate learning step',
    ...
    goal('Now find a state next to ☠️ to see how the optimal policy steers around a low-value terminal purely because argmax says so',
...
registerScene({
  ...
  goals: [
    goal('Hit your assigned value target for start to see how γ and k JOINTLY control how much terminal reward reaches you',
    ...
    goal('Push further — the same target, doubled — to see how a more extreme value demands a more extreme γ or a fully-converged k',
    ...
    goal('Finally, drive the sweep to the exact fixed point (residual < 1e-6) to see the Bellman contraction converge no matter which γ you chose',
