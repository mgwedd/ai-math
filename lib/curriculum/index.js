...
registerLesson({
  ...
  expositions:[
    {key:'regime', title:'One number, from aligned to opposed', figure:'dotRegime', caption:'As b swings from pointing with a to against it, \\(a\\cdot b = \\|a\\|\\,\\|b\\|\\cos\\theta\\) slides from most positive, through <b>zero at 90°</b> (orthogonal), to most negative. The sign is the alignment.'},
    {key:'shadow', title:'The dot product is a shadow', figure:'dotShadow', caption:'\\(a\\cdot b = \\|b\\|\\,(\\|a\\|\\cos\\theta)\\): b\'s length times the length of a\'s <b>shadow</b> (projection) onto b. When a leans past 90°, the shadow flips to the far side and the dot product turns negative.'},
    {key:'search', title:'Search &amp; attention rank by direction', figure:'dotSearchFig', caption:'Semantic search and attention score items by <b>cosine similarity</b> \\(\\cos\\theta = \\frac{a\\cdot b}{\\|a\\|\\,\\|b\\|}\\) to a query — length divided out, so the most-aligned key wins. Attention\'s \\(Q\\cdot K^\\top\\) is the unnormalized cousin — same alignment idea, lengths kept in.'},
  ],
  labs:[
    ...
    {key:'project', title:'Projection — the length of the shadow', interactive:'dotProject',
     intro:'<p>Drag a and read its <strong>scalar projection</strong> onto the fixed b: \\(\\frac{a\\cdot b}{\\|b\\|} = \\|a\\|\\cos\\theta\\). Hit an exact shadow length, then make the shadow vanish (orthogonal) and go negative (leaning away).</p>'},
    {key:'search', title:'Cosine search — be the query', interactive:'dotSearch',
     intro:'<p>One query against several fixed keys, ranked by <strong>cosine similarity</strong>. Steer the query\'s <em>direction</em> — not its length — to promote a chosen key to the top — the direction-only version of the \\(Q\\cdot K^\\top\\) score attention uses.</p>'},
  ],
  quiz:[
