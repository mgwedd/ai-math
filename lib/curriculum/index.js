...
registerLesson({
  ...
  expositions:[
    {key:'hole', title:'A hole the curve never reaches', figure:'limHole', caption:'\\(f(x)=\\frac{x^2-1}{x-1}\\) is 0/0 at \\(x=1\\) — a <b>hole</b>. But as \\(x\\) slides toward 1 from either side, \\(f(x)\\) heads straight for <b>2</b>, the value it approaches but never touches.'},
    {key:'table', title:'Left and right must agree', figure:'limTable', caption:'A limit exists only if both sides approach the same value. Watch \\(f(0.9),f(0.99),f(0.999)\\) climb to 2 and \\(f(1.1),f(1.01),f(1.001)\\) fall to 2: \\(\\lim_{x\\to 1} f(x)=2\\).'},
    {key:'fail', title:'Three ways a limit fails', figure:'limFail', caption:'A limit can fail to exist as a <b>jump</b> (left ≠ right), an <b>asymptote</b> (\\(f\\to\\pm\\infty\\)), or endless <b>oscillation</b> like \\(\\sin(1/x)\\).'},
  ],
  labs:[
    ...
    {key:'onesided', title:'One-sided limits: jump vs asymptote', interactive:'limitsOneSided',
     intro:'<p>Read the <strong>left</strong> and <strong>right</strong> limits separately and decide whether the two-sided limit exists — a jump where the sides disagree, an asymptote where both blow up, and a removable hole where they meet.</p>'},
    {key:'table', title:'Estimate a limit from a value table', interactive:'limitsTable',
     intro:'<p>Some limits can\'t be factored. Creep \\(x\\to 0\\) on \\(\\frac{\\sin x}{x}\\), fill a value table, and read the limit off the numbers — the way it\'s really done for transcendental functions.</p>'},
  ],
  quiz:[
