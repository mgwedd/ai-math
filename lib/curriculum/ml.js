...
registerLesson({
  ...
  learn:`<p>One <strong>ReLU neuron</strong> computes \\(w \\cdot \\text{relu}(x - b)\\). Its graph is dead flat at zero, then a straight ramp — like a door hinge lying flat, then opening. The corner where flat becomes ramp is the <strong>kink</strong>, and it sits at x = b. One neuron = one kink, slope w after it.</p>
...
INTERACTIVES.mlgpt = function(stage, api){
  ...
  let seq=['the'], T=1, hiT=0, loT=0, ended=false;
  const m=api.missions([
  ...
  slider(ctrl,'temperature',0.05,3,0.05,1,v=>v.toFixed(2),v=>{T=v;draw();});
  function dist(){
  ...
  box.querySelector('#gnext').onclick=()=>{
    ...
    if(T<=0.2) loT++; if(T>=2) hiT++; if(pick==='.') ended=true;
    draw();
  };
