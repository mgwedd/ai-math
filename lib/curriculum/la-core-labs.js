...
/* ================================================================
   EXPOSITION FIGURES for la-matrix ("see it first" gallery).
   matColumns / matLinearity are animated (rAF loop, cancel on return);
   matGallery is static (no return). No Math.random — deterministic.
   ================================================================ */

/* matColumns — an oscillating rotation angle sweeps 0°..40°, so the two
   column arrows (î-landing, ĵ-landing) visibly slide while labeled text
   pins down what each one means: "column 1 = where î lands", etc. */
INTERACTIVES.matColumns = function(stage){
  ...
  let t=0, raf=0;
  function draw(){
    const p=0.5+0.5*Math.sin(t); const phi=(40*Math.PI/180)*p;
    M.a=Math.cos(phi); M.b=-Math.sin(phi); M.c=Math.sin(phi); M.d=Math.cos(phi);
    clearBg(ctx,W,H);
    ctx.globalAlpha=.35; P.grid(); ctx.globalAlpha=1;
    ctx.lineWidth=1.4;
    for(let i=-6;i<=6;i++){
      ctx.strokeStyle=i===0?'rgba(0,212,255,.6)':'rgba(0,212,255,.18)';
      let pp=T(i,-6); ctx.beginPath(); ctx.moveTo(P.sx(pp[0]),P.sy(pp[1]));
      pp=T(i,6); ctx.lineTo(P.sx(pp[0]),P.sy(pp[1])); ctx.stroke();
      ctx.strokeStyle=i===0?'rgba(124,92,255,.6)':'rgba(124,92,255,.18)';
      pp=T(-6,i); ctx.beginPath(); ctx.moveTo(P.sx(pp[0]),P.sy(pp[1]));
      pp=T(6,i); ctx.lineTo(P.sx(pp[0]),P.sy(pp[1])); ctx.stroke();
    }
    const c0=T(0,0),c1=T(1,0),c2=T(1,1),c3=T(0,1);
    ctx.fillStyle='rgba(255,201,77,.18)'; ctx.strokeStyle='rgba(255,201,77,.7)'; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(P.sx(c0[0]),P.sy(c0[1]));
    [c1,c2,c3].forEach(pt=>ctx.lineTo(P.sx(pt[0]),P.sy(pt[1])));
    ctx.closePath(); ctx.fill(); ctx.stroke();
    P.arrow(0,0,M.a,M.c,'#2dd4a0',4,'î→'); P.dot(M.a,M.c,6,'#2dd4a0');
    P.arrow(0,0,M.b,M.d,'#ff5c7a',4,'ĵ→'); P.dot(M.b,M.d,6,'#ff5c7a');
    ctx.textAlign='left'; ctx.font='700 12px '+FONT();
    ctx.fillStyle='#2dd4a0'; ctx.fillText('column 1 = where î lands',16,24);
    ctx.fillStyle='#ff5c7a'; ctx.fillText('column 2 = where ĵ lands',16,44);
    ctx.fillStyle='#cdd4f0'; ctx.font='600 12px ui-monospace, Menlo, monospace';
    ctx.fillText('M = [ '+fmt2(M.a)+'  '+fmt2(M.b)+' ]',16,H-38);
    ctx.fillText('    [ '+fmt2(M.c)+'  '+fmt2(M.d)+' ]',16,H-20);
  }
  draw();
  function loop(){ t+=0.02; draw(); raf=requestAnimationFrame(loop); }
  raf=requestAnimationFrame(loop);
...
/* matLinearity — a fixed shear-ish matrix M warps the grid once; a vector v
   circles the origin at constant speed and Mv traces the ellipse that v's
   circle becomes, decomposed visibly into its x·col1 + y·col2 pieces. */
INTERACTIVES.matLinearity = function(stage){
  const F = figCanvas(stage, 640, 340);
  const ctx = F.ctx, W = F.W, H = F.H;
  const P = plane(ctx, W, H, 44, W/2, H/2);
  const M = {a:1,b:0.5,c:0.3,d:1};
  function T(x,y){ return [M.a*x+M.b*y, M.c*x+M.d*y]; }
  let t=0, raf=0;
  function draw(){
    clearBg(ctx,W,H);
    ...
    // the ellipse that v's radius-2 circle maps to under M
    ctx.strokeStyle='rgba(45,212,160,.35)'; ctx.lineWidth=1.5; ctx.beginPath();
    for(let k=0;k<=48;k++){ const a=k/48*2*Math.PI, cx=2*Math.cos(a), cy=2*Math.sin(a);
      const ex=M.a*cx+M.b*cy, ey=M.c*cx+M.d*cy; k===0?ctx.moveTo(P.sx(ex),P.sy(ey)):ctx.lineTo(P.sx(ex),P.sy(ey)); }
    ctx.closePath(); ctx.stroke();
    const vx=2*Math.cos(t*0.5), vy=2*Math.sin(t*0.5);
    const mvx=M.a*vx+M.b*vy, mvy=M.c*vx+M.d*vy;
    P.arrow(0,0,vx,vy,'#7c5cff',3,'v');
    P.arrow(0,0,mvx,mvy,'#2dd4a0',4,'Mv');
    P.dot(mvx,mvy,5,'#2dd4a0');
    P.arrow(0,0,vx*M.a,vx*M.c,'#7fe7ff',2);
    ctx.globalAlpha=.5; P.arrow(vx*M.a,vx*M.c,mvx,mvy,'#ffc94d',2); ctx.globalAlpha=1;
    // colour-coded legend: cyan matches x·col1 arrow, gold matches y·col2 arrow
    ctx.textAlign='left'; ctx.font='700 12px '+FONT(); let lx=16;
    ctx.fillStyle='#cdd4f0'; ctx.fillText('Mv = ',lx,24); lx+=ctx.measureText('Mv = ').width;
    ctx.fillStyle='#7fe7ff'; ctx.fillText('x·col1',lx,24); lx+=ctx.measureText('x·col1').width;
    ctx.fillStyle='#cdd4f0'; ctx.fillText(' + ',lx,24); lx+=ctx.measureText(' + ').width;
    ctx.fillStyle='#ffc94d'; ctx.fillText('y·col2',lx,24);
  }
  draw();
  function loop(){ t+=0.02; draw(); raf=requestAnimationFrame(loop); }
  raf=requestAnimationFrame(loop);
...
/* matGallery — four static panels, each a named transform (rotation, shear,
   non-uniform scale, reflection) shown as its warped unit square + column
   arrows, with the matrix printed below. Purely a "recognize the family"
   reference — no animation, no interaction. */
INTERACTIVES.matGallery = function(stage){
  const F = figCanvas(stage, 640, 360);
  const ctx = F.ctx, W = F.W, H = F.H;
  function panel(ox,oy,M,name){
    const P = plane(ctx, W, H, 20, ox, oy);
    function T(x,y){ return [M.a*x+M.b*y, M.c*x+M.d*y]; }
    // faint identity unit square
    ctx.globalAlpha=.4; ctx.strokeStyle='rgba(139,147,184,.5)'; ctx.lineWidth=1.5;
    ctx.strokeRect(P.sx(0), P.sy(1), P.sx(1)-P.sx(0), P.sy(0)-P.sy(1));
    ctx.globalAlpha=1;
    ...
    [c1,c2,c3].forEach(pt=>ctx.lineTo(P.sx(pt[0]),P.sy(pt[1])));
    ctx.closePath(); ctx.fill(); ctx.stroke();
    P.arrow(0,0,M.a,M.c,'#2dd4a0',3,'î');
    P.arrow(0,0,M.b,M.d,'#ff5c7a',3,'ĵ');
    ctx.fillStyle='#cdd4f0'; ctx.font='700 12px '+FONT(); ctx.textAlign='center';
    ctx.fillText(name, ox, oy-70);
    ctx.font='600 11px ui-monospace, Menlo, monospace'; ctx.fillStyle='#8b93b8';
    ctx.fillText('[ '+fmt2(M.a)+'  '+fmt2(M.b)+' ]', ox, oy+50);
    ctx.fillText('[ '+fmt2(M.c)+'  '+fmt2(M.d)+' ]', ox, oy+64);
  }
  clearBg(ctx,W,H);
  panel(165,100,{a:0.71,b:-0.71,c:0.71,d:0.71},'rotate 45°');
  panel(480,100,{a:1,b:0.8,c:0,d:1},'shear →');
  panel(165,270,{a:1.6,b:0,c:0,d:0.7},'scale (non-uniform)');
  panel(480,270,{a:-1,b:0,c:0,d:1},'reflect over y-axis');
};

/* ================== la-matrix — Lab 3: Build a named transform ================
   Concept: construct-to-spec. Place the columns (drag the four sliders) to hit
   named, recognizable transforms — reflection over x-axis, 45° rotation,
   reflection over y=x — reinforcing "columns = landing spots" as a skill,
   not just a fact to recognize. */
INTERACTIVES.matrixBuild = function(stage, api){
  const L = makeLab(stage);
  let M = {a:1,b:0,c:0,d:1};
  function near(x,t){return Math.abs(x-t)<.13;}

  api.predict({
    prompt: 'To <b>reflect over the x-axis</b>, \\(\\hat\\imath=[1,0]\\) stays put. Predict where \\(\\hat\\jmath=[0,1]\\) must land.',
    choices: ['\\([0,-1]\\)', '\\([0,1]\\)', '\\([-1,0]\\)'],
    answer: 0,
    reveal: 'Reflecting over the x-axis flips the vertical direction: \\(\\hat\\jmath=[0,1]\\to[0,-1]\\), while \\(\\hat\\imath\\) is unchanged. So \\(M=\\begin{bmatrix}1&0\\\\0&-1\\end{bmatrix}\\).',
  });

  const m = api.missions([
    {text:'Reflect over the x-axis: keep î at [1,0], send ĵ to [0,−1]', xp:20,
      check:s=>near(s.a,1)&&near(s.b,0)&&near(s.c,0)&&near(s.d,-1)},
    {text:'Rotate 45° CCW: î→[0.71, 0.71], ĵ→[−0.71, 0.71]', xp:25,
      check:s=>near(s.a,0.71)&&near(s.b,-0.71)&&near(s.c,0.71)&&near(s.d,0.71)},
    {text:'Reflect over the line y = x: swap the axes (î→[0,1], ĵ→[1,0])', xp:20,
      check:s=>near(s.a,0)&&near(s.b,1)&&near(s.c,1)&&near(s.d,0)},
  ]);
  ...
  function draw(){
    ...
    L.readout.innerHTML = 'M = [ '+fmt2(M.a)+'  '+fmt2(M.b)+' ]<br>      [ '+fmt2(M.c)+'  '+fmt2(M.d)+' ]<br>' +
      'î→['+fmt2(M.a)+','+fmt2(M.c)+']  ĵ→['+fmt2(M.b)+','+fmt2(M.d)+']';
    m.update(M);
  }
  slider(L.ctrl, 'a — î\'s x landing', -2, 2, 0.1, 1, null, v=>{M.a=v;draw();});
  slider(L.ctrl, 'b — ĵ\'s x landing', -2, 2, 0.1, 0, null, v=>{M.b=v;draw();});
  slider(L.ctrl, 'c — î\'s y landing', -2, 2, 0.1, 0, null, v=>{M.c=v;draw();});
  slider(L.ctrl, 'd — ĵ\'s y landing', -2, 2, 0.1, 1, null, v=>{M.d=v;draw();});
  note(L.ctrl, 'Place the columns: sliders a,c aim <b style="color:#2dd4a0">î</b>; b,d aim <b style="color:#ff5c7a">ĵ</b>. A reflection over <b>y = x</b> swaps the axes; a 45° rotation puts both columns at ±0.71.');
  draw();
};

/* ================== la-matrix — Lab 4: Read the matrix off the warp ===========
   Concept: the inverse of Lab 1. A target transform is shown as a faint gold
   ghost grid; the learner drags the î-landing and ĵ-landing dots (i.e. builds
   the columns by hand) until their own grid sits on top of it — reading the
   matrix off the geometry instead of off a formula. */
INTERACTIVES.matrixReadoff = function(stage, api){
  const L = makeLab(stage);
  const iLand = {x:1,y:0}, jLand = {x:0,y:1};
  let ti = 0, dragging = null;
  const TARGETS = [
    {name:'rotate 90° CCW',      col1:[0,1],  col2:[-1,0]},
    {name:'horizontal shear',    col1:[1,0],  col2:[1,1]},
    {name:'reflect over x-axis', col1:[1,0],  col2:[0,-1]},
  ];

  api.predict({
    prompt: 'A matrix sends \\(\\hat\\imath\\to[0,-1]\\) and \\(\\hat\\jmath\\to[1,0]\\). Which transformation is it?',
    choices: ['90° rotation clockwise', '90° rotation counterclockwise', 'reflection over the line y = x'],
    answer: 0,
    reveal: 'Columns are landing spots: \\(\\hat\\imath\\to[0,-1]\\) (right becomes down) and \\(\\hat\\jmath\\to[1,0]\\) (up becomes right) — a quarter-turn clockwise, \\(M=\\begin{bmatrix}0&1\\\\-1&0\\end{bmatrix}\\).',
  });

  const m = api.missions([
    {text:'Match the <b>90° rotation</b>: î→[0,1], ĵ→[−1,0]', xp:20,
      check:s=>s.ti===0 && s.di<0.2 && s.dj<0.2},
    {text:'Match the <b>horizontal shear</b>: î→[1,0], ĵ→[1,1]', xp:20,
      check:s=>s.ti===1 && s.di<0.2 && s.dj<0.2},
    {text:'Match the <b>reflection over the x-axis</b>: î→[1,0], ĵ→[0,−1]', xp:20,
      check:s=>s.ti===2 && s.di<0.2 && s.dj<0.2},
  ]);

  const P = plane(L.ctx, L.W, L.H, 52);
  function draw(){
    const tgt = TARGETS[ti];
    const Mt = {a:tgt.col1[0], c:tgt.col1[1], b:tgt.col2[0], d:tgt.col2[1]};
    function Tt(x,y){ return [Mt.a*x+Mt.b*y, Mt.c*x+Mt.d*y]; }
    clearBg(L.ctx, L.W, L.H);
    L.ctx.globalAlpha=.35; P.grid(); L.ctx.globalAlpha=1;
    // ghost target warped grid (faint gold)
    L.ctx.lineWidth=1.2; L.ctx.strokeStyle='rgba(255,201,77,.35)';
    for(let i=-6;i<=6;i++){
      let p=Tt(i,-6); L.ctx.beginPath(); L.ctx.moveTo(P.sx(p[0]),P.sy(p[1]));
      p=Tt(i,6); L.ctx.lineTo(P.sx(p[0]),P.sy(p[1])); L.ctx.stroke();
      p=Tt(-6,i); L.ctx.beginPath(); L.ctx.moveTo(P.sx(p[0]),P.sy(p[1]));
      p=Tt(6,i); L.ctx.lineTo(P.sx(p[0]),P.sy(p[1])); L.ctx.stroke();
    }
    const gc0=Tt(0,0),gc1=Tt(1,0),gc2=Tt(1,1),gc3=Tt(0,1);
    L.ctx.fillStyle='rgba(255,201,77,.12)'; L.ctx.strokeStyle='rgba(255,201,77,.5)'; L.ctx.lineWidth=1.5;
    L.ctx.beginPath(); L.ctx.moveTo(P.sx(gc0[0]),P.sy(gc0[1]));
    [gc1,gc2,gc3].forEach(p=>L.ctx.lineTo(P.sx(p[0]),P.sy(p[1])));
    L.ctx.closePath(); L.ctx.fill(); L.ctx.stroke();
    // current warp, built from the two draggable landing dots
    const M = {a:iLand.x, c:iLand.y, b:jLand.x, d:jLand.y};
    function T(x,y){ return [M.a*x+M.b*y, M.c*x+M.d*y]; }
    L.ctx.lineWidth=1.4;
    for(let i=-6;i<=6;i++){
      L.ctx.strokeStyle=i===0?'rgba(0,212,255,.6)':'rgba(0,212,255,.18)';
    ...
    P.dot(iLand.x, iLand.y, 8, '#2dd4a0'); P.dot(jLand.x, jLand.y, 8, '#ff5c7a');
    P.arrow(0,0,iLand.x,iLand.y,'#2dd4a0',3,'î→'); P.arrow(0,0,jLand.x,jLand.y,'#ff5c7a',3,'ĵ→');
    const di = Math.hypot(iLand.x-tgt.col1[0], iLand.y-tgt.col1[1]);
    const dj = Math.hypot(jLand.x-tgt.col2[0], jLand.y-tgt.col2[1]);
    const hit = di<0.2 && dj<0.2;
    L.readout.innerHTML = tgt.name + '<br>gap: î ' + di.toFixed(2) + '  ĵ ' + dj.toFixed(2) + (hit ? '  ✓ matched' : '');
    m.update({ti, di, dj});
  }
  L.canvas.addEventListener('pointerdown', e=>{
    const p = L.toCanvas(e), wx = P.wx(p.x), wy = P.wy(p.y);
    dragging = (Math.hypot(wx-iLand.x, wy-iLand.y) < Math.hypot(wx-jLand.x, wy-jLand.y)) ? iLand : jLand;
  ...
  row.children[0].classList.add('on');
  note(L.ctrl, 'The gold ghost is the target transformation. Drag the <b style="color:#2dd4a0">î-landing</b> and <b style="color:#ff5c7a">ĵ-landing</b> dots until your grid sits on top of it — those two dots are the matrix\'s columns.');
  draw();
};
