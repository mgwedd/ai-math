...
registerScene({
  ...
  goals: [
    ...
    goal('Now park a near x=2 (within 0.15) with h still small so the slope reads ≈2 — you have just computed a gradient value by hand, the same operation autodiff performs billions of times per training step',
...
registerScene({
  ...
  entities: (p) => {
    ...
    return [
      ...
      label('f\'(x)', { at: vec(-2.9, 3.0), frame: 'inset', color: 'muted' }),
    ];
  },
  goals: [
...
registerScene({
  ...
  goals: [
    goal('Slide a until the tangent goes flat (slope within 0.05 of 0) — you have found a critical point, exactly the kind of flat spot (zero gradient) gradient descent is searching for: no slope left means no more updates, training has settled',
    ...
    goal('Now find a spot where the slope reads about −3 (well past the peak) — confirming there is still a strong gradient pulling training toward the flat spot',
  ...
  goals: [
    goal('Shrink h below 0.1 anywhere on the curve so the secant slope converges to the true tangent slope — proving you can compute the exact gradient this exam\'s model would need, not just eyeball it',
      ...
      { xp: 40, hold: 700, tag: 'derivative meaning',
        focus: 'The secant-to-tangent limit works at ANY point on the curve — shrinking h always converges, exactly the guarantee autodiff relies on.' }),
