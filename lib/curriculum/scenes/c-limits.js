...
registerScene({
  ...
  caption: 'Slide x toward the hole at x=1 from both sides and read where f(x) settles — the curve heads for 2 even though the point itself is missing.',
});
...
registerScene({
  ...
  goals: [
    goal('Sneak toward x=3 from the LEFT (within 0.05) — to see that a 0/0-looking computation is heading somewhere very specific, not toward zero, the same skepticism worth applying to any surprising 0/0 that shows up mid-training',
    ...
    goal('Read the limit to two decimals: land close enough that f(x) is within 0.02 of 6 — proving 0/0 was a signal to factor, not a computed zero, the same fact-check habit worth running on any suspicious 0/0 you spot mid-training',
  ...
  caption: 'Slide x toward the marked hole at x=3. The curve heads for 6 (green), never the 0 a bare "0/0" instinct suggests (red) — factor first, the habit that catches a "fooled by 0/0" mistake wherever it shows up.',
});
...
registerScene({
  ...
  params: { fi: 0, eps: 1 },
  controls: [
    slider('fi', { min: 0, max: 2, step: 1, label: 'function (0, 1, 2)' }),
...
registerScene({
  ...
  goals: [
    goal('Creep to within 0.1–0.2 of the hole at x=3 and hold — landing where the SIMPLIFIED formula, not the raw 0/0 expression, governs the curve, the same trend-not-formula reasoning a normalized ratio in a model needs whenever its raw computation looks like 0/0',
    ...
    goal('Land on the flagged point right at x=3 and hold — noticing the function\'s ACTUAL recorded value sits nowhere near the limit you just found, the value/limit gap that vanishing- and exploding-gradient debugging depends on',
  ...
  caption: 'No hints now. Creep to the hole and read the limit, shrink ε to catch the jump disagreeing, then land on the flagged point and see the recorded value miss the limit — the same value/limit gap that trips up gradient debugging, holding each steady. This is the exam.',
});
