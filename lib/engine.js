...
function renderLesson(id){
  ...
  /* ---- Visual-first scenes path (docs/VISUAL_FIRST.md §2/§4) ----
     A lesson with `scenes:[...]` renders an ordered scene SEQUENCE instead of
     the Learn/Lab steps. Advance is gated on each scene's goals completing. A
     scene flagged `capstone:true` is the exam: completing its goals completes
     the lesson (the quiz becomes optional XP). registerCleanup runs on every
     swap. Lessons WITHOUT `scenes` never reach here — the legacy flow below is
     byte-identical. */
  function renderScenes(){
    ...
    function showScene(){
      ...
      const b=sceneShell(
        ...
        '<div class="btn-row">'+
          '<button class="btn ghost" id="back">← '+(si===0?'Map':'Previous scene')+'</button>'+retryBtn+
      ...
      const nextBtn=b.querySelector('#next');
      b.querySelector('#back').onclick = si===0 ? ()=>go('home') : ()=>{ si--; showScene(); };
  ...
  if(l.scenes && l.scenes.length){ renderScenes(); return; }
  showLearn();
}
