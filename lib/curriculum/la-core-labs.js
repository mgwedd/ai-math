...
INTERACTIVES.mmOrder = function(stage){
  ...
  let t=0,raf=0;
  function panel(P,C,title,color){
  ...
  function draw(){
    clearBg(ctx,W,H);
    panel(PL,CL,'B·A: shear after rotate','rgba(45,212,160,');
    panel(PR,CR,'A·B: rotate after shear','rgba(255,92,122,');
  }
  draw();
  function loop(){ t+=0.02; draw(); raf=requestAnimationFrame(loop); }
  raf=requestAnimationFrame(loop);
...
INTERACTIVES.matmulEntry = function(stage, api){
  ...
  function grid(gx,vals,greenMask){
    ...
    for(let i=0;i<2;i++) for(let j=0;j<2;j++){
      ...
      ctx.fillStyle = greenMask ? (greenMask[idx]?'#2dd4a0':'#ff9db0') : '#cdd4f0';
  ...
  note(L.ctrl,'Each product entry is a row·column dot product. Fill all four, then switch to \\(A\\cdot B\\) — the entries change, because order matters.');
  draw();
};
...
INTERACTIVES.matmulCommute = function(stage, api){
  ...
  const TS=[
    ...
    {name:'Rotate 180°', M:[-1,0,0,-1], rot:true},
  ...
  note(L.ctrl,'Green = \\(B\\cdot A\\), red = \\(A\\cdot B\\). When they land on the same F the pair commutes; when they split apart, order matters.');
  draw();
};
