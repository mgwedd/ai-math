/* ================================================================
   ...
   ================================================================ */
import { LESSONS, INTERACTIVES, WRONG_WHY, registerLesson, validateCurriculum } from './registry.js';
...
registerLesson({
  ...
  ml:`A deep network is literally a chain of these: <code>output = W₃·σ(W₂·σ(W₁·x))</code>. "Deep" = many compositions. Matrix multiply is <b>associative</b> (so the chain is well-defined) but <b>not commutative</b> (so layer order matters). The load-bearing catch: the <code>σ</code> nonlinearities are essential — without them a stack of matrices <em>collapses to a single matrix</em> W₃W₂W₁, and depth buys nothing.`,
  interactive:'matmul',
  quiz:[
...
registerLesson({
  ...
  <p>The lab uses <code>M = [[2,1],[1,2]]</code>. It has two eigendirections: along <code>[1,1]</code> vectors get stretched ×3 (λ=3), and along <code>[1,−1]</code> they're untouched (λ=1). <em>Repeatedly</em> applying M drives almost any vector toward the dominant (largest-λ) direction — that's <strong>power iteration</strong>, the engine behind PageRank.</p>
  <p>Eigenvectors reveal what a matrix <em>fundamentally does</em>, stripped of the rotating — but not every matrix has (real) ones: a pure <strong>rotation</strong> turns every vector, so it has <em>no</em> real eigenvectors (its eigenvalues are complex). This <code>M</code> is also <strong>symmetric</strong>, and symmetric matrices are special — the spectral theorem guarantees their eigenvectors are real and <em>perpendicular</em> (here [1,1]⊥[1,−1]), which is exactly why PCA's axes come out orthogonal.</p>`,
...
registerLesson({
  ...
  learn:`<p>Consider <code>f(x) = (x² − 1)/(x − 1)</code>. Plug in x = 1 and you get 0/0 — an <strong>indeterminate form</strong>. That's stronger than "undefined": unlike 1/0 (which blows up), 0/0 could approach <em>any</em> value, so you have to do more work to find out. There's a <strong>hole</strong> in the graph at x = 1.</p>
...
registerLesson({
  ...
  <div class="formula">Part 1:  d/dx ∫ₐˣ f(t) dt = f(x)      Part 2:  ∫ₐᵇ f′(x) dx = f(b) − f(a)</div>
