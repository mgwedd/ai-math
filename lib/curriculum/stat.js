...
registerLesson({
  ...
  <div class="formula">$$\\sigma^2_{\\text{MLE}} = \\dfrac{1}{n}\\sum (x_i - \\bar{x})^2 \\quad \\leftarrow \\text{biased LOW}$$</div>
  ...
  <div class="formula">$$s^2 = \\dfrac{1}{n-1}\\sum (x_i - \\bar{x})^2 \\quad \\leftarrow \\text{unbiased for } \\sigma^2$$</div>
...
registerLesson({
  ...
  <div class="formula">$$\\bar{x} \\pm 1.96 \\cdot \\dfrac{\\sigma}{\\sqrt{n}} \\quad \\leftarrow \\text{the } \\pm 1.96 \\cdot SE \\text{ band}$$</div>
  ...
  deeper:[
   ...
   {title:'🚀 Go deeper: the delta method — CIs for a transformed estimate', body:'You often don\'t want a CI for θ̂ itself but for some nonlinear function of it, g(θ̂) — say perplexity = e^(cross-entropy). You already have SE(θ̂); the <b>delta method</b> converts it without new formulas: for smooth g, SE(g(θ̂)) ≈ |g′(θ̂)| · SE(θ̂), and the CI becomes g(θ̂) ± 1.96 · |g′(θ̂)| · SE(θ̂). The intuition is just a local linear zoom: near θ̂, g looks like its tangent line with slope g′(θ̂), so an error bar of size SE on the input stretches (or shrinks) by that slope on the output. This is exactly why error bars on <b>log-perplexity</b> and on <b>perplexity</b> look so different from the same underlying run: g(x) = eˣ has g′(x) = eˣ, so the CI half-width scales by a factor of eˣ itself — a fixed additive uncertainty in log-space becomes a multiplicative, and much more dramatic-looking, uncertainty once you exponentiate. Always ask which scale an error bar was computed in before comparing two of them. (Casella &amp; Berger, <em>Statistical Inference</em>, §5.5.)'}],
  interactive:'statci',
  quiz:[
...
registerLesson({
  ...
  <div class="formula">$$p = P(\\text{statistic} \\geq \\text{observed} \\mid H_0)$$</div>
  ...
  deeper:[
   ...
   {title:'🚀 Go deeper: two ways to be wrong (Neyman–Pearson)', body:'Every test you run this lesson can fail in exactly two directions. A <b>Type I error</b> is rejecting H₀ when it was actually true — a false alarm; its rate is α = P(reject | H₀ true), and it is precisely the significance threshold you pick before looking (α = 0.05 means you accept a false-alarm rate of 5%). A <b>Type II error</b> is failing to reject H₀ when a real effect exists — a miss; its rate is β = P(fail to reject | effect real). <b>Power</b> is the complement, 1 − β: the probability you correctly detect a real effect when it is there. In the permutation test, α is the threshold you set on the p-value (how rare a shuffled result has to look before you call it "real"); β/power depends on things you control — sample size, effect size, how noisy the measurements are — with more shuffles narrowing your estimate of p, but more DATA (bigger nT, more subjects) is what actually buys you power against a real but modest effect. The two error rates trade off: shrinking α (demanding stronger evidence) makes Type II errors more likely unless you also collect more data. (Neyman &amp; Pearson, 1933; Casella &amp; Berger, <em>Statistical Inference</em>, ch. 8.)'}],
