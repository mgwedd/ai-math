...
registerLesson({
  ...
  quiz:[
    {type:'numeric', q:'Compute the magnitude of <code>[5, 12]</code>. (‖v‖ = √(x²+y²))', answer:13, tol:0.05,
  ...
  3:'Each of the 1,536 numbers is a <b>component</b> (one coordinate) of a single vector — like x and y are components of [3,2], not two separate vectors.'}},
    {q:'Which vector is a <b>unit</b> vector?', opts:['<code>[1, 1]</code>','<code>[0.6, 0.8]</code>','<code>[2, 0]</code>','<code>[0.5, 0.5]</code>'], a:1,
     tag:'unit vectors', focus:'Check several vectors: do the squared components sum to exactly 1? Normalize one by dividing by its length.', why:'√(0.36 + 0.64) = √1 = 1. Note <code>[1,1]</code> has magnitude √2 ≈ 1.41 — a common trap.', wrong:{0:'<b>Classic trap.</b> The components being 1 doesn\'t make the length 1: ‖[1,1]‖ = √(1+1) = √2 ≈ 1.41. The diagonal of a unit square is longer than its sides.',
...
registerLesson({
  ...
  quiz:[
    {type:'numeric', q:'For the triangular matrix <code>M = [[4, 7], [0, 3]]</code>, what is its <b>largest</b> eigenvalue? (Triangular ⇒ eigenvalues are the diagonal entries.)', answer:4, tol:0.05,
...
registerLesson({
  ...
  3:'The power rule handles it fine: 7 = 7x⁰, so 7·0·x⁻¹ = 0. No failure — just zero.'}},
    {type:'numeric', q:'For <code>f(x) = x³</code>, the power rule gives <code>f′(x) = 3x²</code>. What is <code>f′(2)</code>?',
     answer:12, tol:0.001,
...
registerLesson({
  ...
  3:'No learning rate fixes blindness: at a local minimum the gradient is genuinely zero, so no scaled step escapes it. (That\'s why momentum & restarts exist.)'}},
    {type:'order', q:'Arrange the derivation that finds and confirms the <b>local minimum</b> of <code>f(x) = x³/3 − x</code>:',
