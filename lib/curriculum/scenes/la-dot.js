/* ================================================================
   ...
   Mid-lesson scenes carry the same tags so a review stumble is
   attributable to the scene that teaches the concept.

   CAPSTONE RANDOMIZATION HOOK (for quality's harness): the capstone
   spec carries `capstone:true` and `rollParams(seed) -> paramsObject`.
   `params` is the seed-1 draw (a plain object, per CONTRACT §1);
   `rollParams` is the re-roll called on each fresh attempt. See
   flagship-content.md → Handoffs for the exact binding.
   ================================================================ */
import {
  registerScene, vec, handle,
  grid, vector, point, segment, dropLine, angleArc, label, bars,
  goal,
} from '../../scene/index.js';
import {
  dot, cos, mag, angleDeg, proj, projVec, scale, norm, softmax, mapRange,
  rayConstraint, circleConstraint, trackConstraint,
} from './vec-math.js';
...
registerScene({
  ...
  entities: (p) => {
    ...
    return [
      ...
      vector(p.a, { color: 'accent', label: 'a', handle: 'a' }),
  ...
  goals: [
    ...
    goal('Fold a down so its shadow shrinks to nothing (a ⟂ b)',
      (s) => Math.abs(proj(s.a, s.b)) < 0.1,
      { xp: 20, hold: 400, tag: 'dot product geometry',
        focus: 'Zero shadow means a·b = 0 — perpendicular. Watch the drop-line collapse to the origin.' }),
  ...
  goals: [
    goal('Make the vectors orthogonal (cos θ ≈ 0)',
      ...
      { xp: 20, hold: 500, tag: 'sign vs angle',
        focus: 'Positive dot ⇔ angle < 90°, zero ⇔ exactly 90°, negative ⇔ obtuse. Zero is the boundary.' }),
...
registerScene({
  ...
  goals: [
    ...
    goal('Flatten attention: keep every key under 0.4',
      (s) => Math.max(...attn(s.q, tempOf(s.tk.y))) < 0.4,
      { xp: 20, tag: 'cosine similarity',
        focus: 'A high temperature washes out the differences between dot products — attention spreads evenly.' }),
...
// Re-roll hook for the capstone. quality's harness binds to `rollParams`.
function rollParams(seed) {
  const rng = mulberry32((seed >>> 0) || (Math.floor(Math.random() * 0xffffffff) >>> 0) || 1);
...
registerScene({
  ...
  lesson: LESSON,
  capstone: true,
  rollParams,                                             // extra key (CONTRACT §1 tolerates)
  space: 'plane2',
  params: rollParams(1),                                  // seed-1 draw (plain object)
  entities: (p) => [
    grid(),
    angleArc(p.a, p.b, { color: 'warn', label: deg0(p.a, p.b) + '°' }),
