...
// Signed minor sweep from angle a0 to a1, normalized to (-π, π]. Guarantees the
// angleArc renders the SHORTER arc between the two directions rather than the
// reflex one (|sweep| <= π). Exported for unit reasoning.
function arcSweep(a0, a1){
  let d = a1 - a0;
...
const DRAW = {
  grid(g, e, sp){
    ...
    // Defense in depth: grid() already rejects these at construction, but a raw
    // descriptor built without the constructor could still hang the loop.
    if(!(step > 0) || !Number.isFinite(step) || !Number.isFinite(E)) return;
  ...
  angleArc(g, e, sp){
    ...
    // Draw the MINOR arc between u and w. Passing raw a0->a1 to arc() draws the
    // reflex (long-way) arc whenever the directions are >180° apart in the
    // default winding; sweep by the normalized signed delta and let the winding
    // flag follow its sign so the indicated angle is always the acute/obtuse one.
    const d = arcSweep(a0, a1);
    g.moveTo(c.px, c.py).arc(c.px, c.py, r, a0, a0 + d, d < 0).stroke({ width: 2, color: color(e.color) });
  },
  dropLine(g, e, sp){
...
export { color as resolveColor, arcSweep };
