...
INTERACTIVES.dotRegime = function(stage){
  ...
  function draw(){
    ...
    ctx.fillText('−', 20, 258); ctx.fillText('+', 200, 258);
...
INTERACTIVES.dotShadow = function(stage){
  ...
  function draw(){
    ...
    P.dot(foot.x, foot.y, 5, comp>=0 ? '#ffc94d' : '#ff5c7a');
    ctx.textAlign = 'left';
...
INTERACTIVES.dotSearchFig = function(stage){
  ...
  scored.forEach(k => {
    ...
    // top match's label drops below its tip to clear the 'query' label
    ctx.fillText(k.e + ' ' + k.cos.toFixed(2), P.sx(k.x) + 8, P.sy(k.y) + (k.n===topN ? 18 : -4));
  });
...
INTERACTIVES.dotProject = function(stage, api){
  ...
  note(L.ctrl, 'b is fixed; drag <b style="color:#b9a8ff">a</b>. The gold segment is a\'s <b>shadow</b> on b, length <b>a·b / ‖b‖</b>. Lean a past 90° from b and the shadow flips negative.');
  draw();
};

/* ================== la-dot — Lab 4: Cosine search (be the query) ============
   Concept: rank fixed keys against a query by cosine similarity — pure
   direction, length divided out — the direction-only cousin of attention's
   Q·Kᵀ score. Steer the
