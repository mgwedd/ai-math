...
export async function getAuthUser() {
  ...
  // Prefer the server-only SUPABASE_URL over the browser-baked
  // NEXT_PUBLIC_SUPABASE_URL. In the local self-host stack these differ: the
  // browser bundle bakes the host-reachable proxy URL (http://localhost:8000)
  // while server code must reach the proxy over the compose network
  // (http://proxy:8000). In prod only NEXT_PUBLIC_SUPABASE_URL is set, so this
  // precedence is harmless there.
