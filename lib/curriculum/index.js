...
registerLesson({
  ...
  expositions:[
    ...
    {key:'search', title:'Search &amp; attention rank by direction', figure:'dotSearchFig', caption:'Semantic search and attention score items by <b>cosine similarity</b> \\(\\cos\\theta = \\frac{a\\cdot b}{\\|a\\|\\,\\|b\\|}\\) to a query — length divided out, so the most-aligned key wins. That is exactly what \\(Q\\cdot K^\\top\\) computes.'},
  ],
  labs:[
    ...
    {key:'search', title:'Cosine search — be the query', interactive:'dotSearch',
     intro:'<p>One query against several fixed keys, ranked by <strong>cosine similarity</strong>. Steer the query\'s <em>direction</em> — not its length — to promote a chosen key to the top, exactly as attention scores \\(Q\\cdot K^\\top\\).</p>'},
  ],
  quiz:[
