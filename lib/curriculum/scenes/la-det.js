...
registerScene({
  ...
  goals: [
    goal('Build a matrix with det = ad − bc exactly 4, to see how the determinant is computed directly from a matrix’s four entries',
...
registerScene({
  ...
  goals: [
    ...
    goal('Collapse it again, but point the columns in OPPOSITE directions along the line, to see how it’s sharing a line — not which way each column points along it — that zeroes the determinant',
      (s) => mag(s.col1) > MIN_MAG && mag(s.col2) > MIN_MAG && Math.abs(det2(s.col1, s.col2)) < 0.2 && dot(s.col1, s.col2) < 0,
      { xp: 20, hold: 400, tag: 'det = 0 collapse',
...
registerScene({
  ...
  goals: [
    ...
    goal('Now scale column 1 instead: double the area to det = 4, to see how, unlike shearing, scaling a column directly moves the determinant',
