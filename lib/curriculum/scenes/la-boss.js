  ...
  entities: (p) => {
    ...
    return [
      ...
      cellGrid([[Math.abs(p.a), Math.abs(p.b)], [Math.abs(p.c), Math.abs(p.d)]],
        { at: vec(-5, 5), cell: 1, min: 0, max: 2, key: 'wheat' }),
  ...
  goals: [
    goal('Send x′ = W·x onto ★₁ by tuning a, b, c, d — x itself never moves',
      ...
      { xp: 20, hold: 400, tag: 'what training changes',
        focus: 'x′ = W·x. This IS training a linear layer: the input is fixed forever, and the only thing that ever changes is the matrix — exactly these four sliders.' }),
    goal('Now send x′ to a completely different spot, ★₂ — same x, a different W',
      ...
      { xp: 25, hold: 400, tag: 'what training changes',
        focus: 'Any point can land almost anywhere under SOME W — a linear layer’s entire expressive power lives in these four numbers, never in touching the data.' }),
  ],
...
registerScene({
  ...
  controls: wSliders(),
  entities: (p) => {
    const d = Math.abs(detW(p.a, p.b, p.c, p.d));
    // honest per-band framing: only det≈0 has actually destroyed information;
    // the whole 0 < |det| < DET_MIN band is still invertible, just close to
    // the singular boundary — saying "information is being lost" there would
    // be false (any nonzero det is lossless).
    const note = d < COLLAPSE_EPS ? '  — collapsed: information is being lost'
      : d < DET_MIN ? '  — dangerously close to singular' : '';
    return [
      ...clusterEntities(p),
      label(wReadout(p.a, p.b, p.c, p.d) + note, { at: 'readout' }),
    ];
  },
  goals: [
  ...
  caption: 'Separating is easy if you are allowed to destroy information; this time watch the readout and stay well clear of the singular boundary while you do it. That trade-off — re-represent without erasing — is what a linear layer must respect.',
});
  ...
  goals: [
    goal('Sabotage it: line row [a, b] up with row [c, d], SAME direction, so det ≈ 0 and both classes collapse onto one line',
      ...
      { xp: 25, hold: 400, tag: 'information loss',
        focus: 'det ≈ 0 is not a harmless flip — it is the layer throwing away a whole dimension. Once both classes land on the same line, no amount of downstream mixing can ever pull them back apart: the information is simply gone.' }),
...
registerScene({
  ...
  caption: 'Push past the bare minimum this time — watch the readout for both margin and |det| as you separate, in both directions. A robust classifier does not sit on the edge of the decision line — it stays well clear of it.',
});
...
// Exported so the test suite imports these instead of hand-copying them —
// a future tier edit then can't silently break the exhaustive baseline check.
// CAP_DET/CAP_DET2 are disjoint (max 0.5 < min 0.55) so goal 3's det bar is
// always strictly higher than goal 2's, for every independent draw pairing.
export const CAP_SIDES = [1, -1];
export const CAP_MARGIN = [0.2, 0.25, 0.3];
export const CAP_MARGIN2 = [0.4, 0.5, 0.6];
export const CAP_DET = [0.3, 0.4, 0.5];
export const CAP_DET2 = [0.55, 0.6, 0.7];
  ...
  goals: [
    ...
    goal('Finally, push both bars further — wider margin, higher invertibility — the same four knobs, refined',
      ...
      { xp: 40, hold: 700, tag: 'what training changes',
        focus: 'Nothing about the clusters ever changed — only a, b, c, d did. That IS what training a linear layer means: the same four dials, tuned until the fit is both separable and safe.' }),
  ],
