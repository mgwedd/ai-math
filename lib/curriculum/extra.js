...
deepen('c-chain',[
 ...
 {title:'🚀 Go deeper: when paths fan out (the real rule)', body:'A network isn\'t a single chain — a weight often affects the loss through <b>several</b> downstream paths. The multivariable chain rule says: <b>sum the contributions of every path</b>, ∂L/∂x = Σᵢ (∂L/∂uᵢ)(∂uᵢ/∂x). In autodiff code this is the <code>+=</code> that accumulates gradient into a node from each of its consumers. "Multiply along a path, add across paths" is the whole of backprop in seven words.'}]);
deepen('c-optim',[
...
deepen('c-boss',[
 ...
 {title:'🚀 Go deeper: why the gradient is steepest-ascent (and ⟂ contours)', body:'It\'s not a definition, it\'s a theorem. The slope you feel walking in a unit direction u is the directional derivative ∇f·u = ‖∇f‖cos θ, which is largest when θ = 0 — i.e. when you walk <em>along</em> ∇f. And walking <em>along</em> a contour (constant f) the directional derivative is 0, so ∇f·u = 0 there: the gradient is always <b>perpendicular to the level curves</b>. Watch the lab arrows cross the ellipses at right angles.'},
 {title:'🚀 Go deeper: the loss landscape view of training', body:'Training a network IS this lab in 70-billion-D: loss = f(weights), backprop computes ∇f, the optimizer steps along −∇f. Learning-rate schedules, warmup, gradient clipping — all are tweaks to the step you just took on the contour map. The gaps from here to real training: backprop computes ∇f through a <em>computational graph</em> (not by hand), and in high-D the landscape is ruled by <b>saddle points</b>, not the tidy bowl shown here.'}]);

/* ---------------- expanded question pools (flagship lessons) ---------------- */
function addQ(id, q, traps){
  const l = byId(id); if(!l) return;
  if(traps) q.wrong = traps;
  l.quiz.push(q);
}
addQ('la-vectors',
 {q:'Vectors \\(u=[1,2]\\) and \\(v=[3,1]\\). What is the distance between them (as points)?',
...
addQ('la-vectors',
 {q:'Normalizing \\([6, 8]\\) (making it length 1) gives…',
  opts:['\\([0.6, 0.8]\\)','\\([1, 1]\\)','\\([3, 4]\\)','\\([\\dfrac{6}{14}, \\dfrac{8}{14}]\\)'], a:0,
...
addQ('la-matrix',
 {q:'Which matrix reflects the plane over the <b>x-axis</b>?',
  opts:['\\(\\begin{bmatrix} 1 & 0 \\\\ 0 & -1 \\end{bmatrix}\\)','\\(\\begin{bmatrix} -1 & 0 \\\\ 0 & 1 \\end{bmatrix}\\)','\\(\\begin{bmatrix} 0 & 1 \\\\ 1 & 0 \\end{bmatrix}\\)','\\(\\begin{bmatrix} -1 & 0 \\\\ 0 & -1 \\end{bmatrix}\\)'], a:0,
...
addQ('la-matrix',
 {q:'M sends î→[2,0] and ĵ→[1,1]. Where does \\([1, 2]\\) land?',
  opts:['\\([4, 2]\\)','\\([2, 2]\\)','\\([3, 1]\\)','\\([2, 1]\\)'], a:0,
...
registerLesson({
  ...
  learn:`<p>A <strong>function</strong> is a machine with one ironclad rule: feed in a number x, and it returns <em>exactly one</em> number f(x). Notation like \\(f(x) = x^2 - 2\\) is both a recipe — "square the input, then subtract 2" — and a name (\\(f\\)) so we can talk about the machine without re-describing it every time.</p>
  ...
  deeper:[
   ...
   {title:'🚀 Go deeper: codomain vs. range', body:'The <b>range</b> from the card above is really shorthand for two different things. The <b>codomain</b> is the set you *declare* outputs live in — a bookkeeping choice, part of how the function is defined. The <b>range</b> (or <em>image</em>) is the set of outputs the machine *actually* hits — which can be a strict subset of the codomain. Softmax is the textbook case: its codomain is all of \\(\\mathbb{R}^k\\) (that\'s where the vectors live), but its range is only the open probability simplex — every coordinate strictly between 0 and 1, summing to 1. A model can declare "outputs are k-vectors of reals" while the function itself only ever reaches a much smaller, curved slice of that space. (Goodfellow, Bengio &amp; Courville, <em>Deep Learning</em>, §6.2.2.)'}],
  labs:[
   ...
   {key:'compose', title:'Chaining machines', interactive:'prefn_compose',
    intro:'<p>Feed one machine\'s output straight into the next: \\(g(f(x))\\). Order rarely commutes — square-then-add almost never equals add-then-square. This is exactly how layers stack inside a network.</p>'},
   {key:'solve', title:'Solve f(x) = target', interactive:'prefnSolve',
    intro:'<p>The inverse question: given an <em>output</em>, find the input(s). Predict a value first, then hunt down both roots of f(x) = 0 — and discover a target the parabola can never reach.</p>'},
  ],
  quiz:[
   {q:'f(x) = x² − 2. What is f(3)?', opts:['7','4','1','9'], a:0,
    ...
    why:'f(3) = 3² − 2 = 9 − 2 = 7.',
    wrong:{1:'4 = 3+1? Substitute fully: square FIRST (9), then subtract 2.',2:'1 = 3−2 skips the squaring. The machine squares before subtracting.',3:'9 is x² — one more step: subtract 2.'}},
   {q:'A "root" of f is a point where…', opts:['f(x) = 0 — the graph crosses the x-axis','x = 0 — the y-axis','The graph is steepest','f is undefined'], a:0,
    ...
    wrong:{1:'x = 0 is the y-axis crossing (the value f(0)), a different landmark.',2:'Steepness is about derivatives, not roots.',3:'Undefined points are holes/gaps, not roots.'}},
   {q:'Which is NOT a function of x?', opts:['A circle: x² + y² = 1','y = |x|','y = x³','y = 7'], a:0,
    ...
    wrong:{1:'|x| gives one output per input (V shape) — a fine function.',2:'x³ is one-to-one — certainly a function.',3:'y = 7 ignores x but still gives exactly one output: a constant function.'}},
   {q:'If the graph of f is higher at x=2 than at x=5, then…', opts:['f(2) &gt; f(5)','f(2) &lt; f(5)','2 &gt; 5','f is decreasing everywhere'], a:0,
    ...
    wrong:{1:'Height read backwards — higher curve means bigger value.',2:'2 < 5 always; inputs aren\'t being compared.',3:'One comparison at two points says nothing about everywhere.'}},
   {q:'With f(x) = x + 1 and g(x) = x², what is g(f(2))?', opts:['9','5','6','4'], a:0,
    ...
    wrong:{1:'5 is f(g(2)) — you applied g first. Read g(f(2)) inside-out: f acts first (2→3), then g squares (→9).',2:'6 looks like 2·3 — but g squares its input, it doesn\'t multiply by 2. g(3) = 9.',3:'4 = g(2) alone; you skipped f. Apply f first: 2→3, then square → 9.'}},
   {q:'A vertical line crosses a graph at two points. This tells you…', opts:['It is NOT a function — one input has two outputs','It has a root there','The function is steep there','That input is in the domain twice'], a:0,
    ...
    wrong:{1:'A root is where the curve crosses the X-axis (height 0) — unrelated to how many times a VERTICAL line crosses.',2:'Steepness is about slope, not the count of crossings.',3:'An input is either in the domain or not — it can\'t be there "twice". Two outputs is the actual problem.'}},
  ],
});
...
registerLesson({
  ...
  learn:`<p>A line is the simplest non-trivial function: \\(y = mx + b\\). Just two numbers pin it down completely:</p>
  ...
  <div class="formula">$$m = \\dfrac{\\text{rise}}{\\text{run}} = \\dfrac{y_2 - y_1}{x_2 - x_1}$$</div>
  ...
  deeper:[
   ...
   {title:'🚀 Go deeper: the closed-form answer', body:'You never actually need to slide the knobs — for a single input variable the bowl\'s exact bottom has a one-line formula: <b>m = cov(x,y) / var(x)</b>, then <b>b = ȳ − m·x̄</b> (average y minus slope times average x). No search, no iterations — plug in the data and read off the best line. One consequence falls straight out of that second formula: rearranged, it says ȳ = m·x̄ + b, so the fitted line always passes exactly through the point (x̄, ȳ) — the "center of mass" of the data. That <b>through-the-mean property</b> is a good sanity check on any fit: if your line misses the average x, average y point, something is wrong. (Weisberg, <em>Applied Linear Regression</em>, ch. 2.)'}],
  labs:[
   ...
   {key:'twopoint', title:'Two-point slope & intercept', interactive:'preslopeTwoPoint',
    intro:'<p>Work y = mx + b backward: drag two points to build a line with an exact slope, a perfectly horizontal one, and a chosen y-intercept. Slope is rise over run, computed on demand.</p>'},
  ],
  quiz:[
   {q:'The line through (1, 2) and (3, 8) has slope…', opts:['3','6','2','1/3'], a:0,
    ...
    why:'m = (8−2)/(3−1) = 6/2 = 3.',
    wrong:{1:'6 is just Δy — divide by Δx = 2.',2:'2 is Δx. Slope is rise OVER run: 6/2.',3:'1/3 is run over rise — inverted.'}},
   {q:'In y = mx + b, what does b control?', opts:['Height where the line crosses the y-axis','The steepness','The x-axis crossing','The line\'s length'], a:0,
    ...
    wrong:{1:'Steepness is m\'s job.',2:'The x-crossing depends on BOTH m and b (it\'s −b/m).',3:'Lines are infinite; they don\'t have lengths.'}},
   {q:'Fitting a line by minimizing squared error is called…', opts:['Linear regression','Clustering','Classification','Integration'], a:0,
    ...
    wrong:{1:'Clustering groups unlabeled points — no line, no target.',2:'Classification predicts categories, not continuous values.',3:'Integration accumulates areas — different world.'}},
   {q:'A line with m = −2 means…', opts:['y drops 2 for every 1 step right','y rises 2 per step right','The line crosses y at −2','The line is vertical'], a:0,
    ...
    wrong:{1:'Sign flipped: negative slope falls as x grows.',2:'That\'s b\'s meaning, not m = −2.',3:'Vertical lines have UNDEFINED slope, not −2.'}},
...
registerLesson({
  ...
  learn:`<p>The central question of linear algebra: given the transform A and a target \\(b\\), <strong>which input x lands on b?</strong></p>
  <div class="formula">$$A \\cdot x = b \\quad\\Rightarrow\\quad x = A^{-1} \\cdot b$$</div>
  ...
  // P2 wave K: additive scene arc (lib/curriculum/scenes/la-inverse.js).
  // `interactive`/`quiz` above are untouched — mirrors every prior migration.
  scenes:[
  ...
  quiz:[
   {q:'A⁻¹ exists exactly when…', opts:['det A ≠ 0','A is square with positive entries','det A &gt; 0','A is symmetric'], a:0,
    ...
    wrong:{1:'Positivity is irrelevant — [[−1,0],[0,1]] (a flip) inverts itself fine.',2:'det = −2 is perfectly invertible: flips are reversible. Only ZERO kills the inverse.',3:'Symmetry is unrelated; plenty of symmetric matrices are singular (e.g. all-ones).'}},
   {q:'A is singular (det = 0) and b is OFF the line A\'s outputs cover. Then Ax = b has…', opts:['No solution','Exactly one solution','Infinitely many solutions','x = 0 as the solution'], a:0,
    ...
    wrong:{1:'Exactly one solution is the healthy det ≠ 0 case.',2:'Infinitely many happens when b IS on the output line (the misses collapse together).',3:'x = 0 maps to 0 — it only solves b = 0.'}},
   {q:'Applying A then A⁻¹ to any vector v gives…', opts:['v itself','The zero vector','2v','A·v'], a:0,
    ...
    wrong:{1:'Zero would mean A⁻¹ destroyed v — inverses destroy nothing.',2:'2v would mean the round trip stretched space; undo means NO net change.',3:'A·v is the forward trip only — the inverse brings it back.'}},
   {q:'For A = [[2,0],[0,4]], A⁻¹ = ?', opts:['\\(\\begin{bmatrix} 0.5 & 0 \\\\ 0 & 0.25 \\end{bmatrix}\\)','\\(\\begin{bmatrix} -2 & 0 \\\\ 0 & -4 \\end{bmatrix}\\)','\\(\\begin{bmatrix} 4 & 0 \\\\ 0 & 2 \\end{bmatrix}\\)','\\(\\begin{bmatrix} 2 & 0 \\\\ 0 & 4 \\end{bmatrix}\\)'], a:0,
    ...
    wrong:{1:'Negating doesn\'t undo scaling: ×2 then ×(−2) gives −4x, not x.',2:'Swapping the factors composes to ×8 per axis, worse.',3:'Same matrix twice = ×4, ×16 — squared, not undone.'}},
  ],
});
...
registerLesson({
  ...
  learn:`<p>Exponentials \\(f(x) = a^x\\) grow by <em>multiplication</em>: each unit step multiplies by a. Their defining magic: the derivative is <strong>proportional to the function itself</strong>:</p>
  <div class="formula">$$\\dfrac{d}{dx} a^x = a^x \\cdot \\ln(a)$$</div>
  <p>One special base makes that constant exactly 1 — growth rate equals current value. That base is <strong>e ≈ 2.718</strong>:</p>
  <div class="formula">$$\\dfrac{d}{dx} e^x = e^x$$</div>
  ...
  quiz:[
   {q:'What is special about f(x) = eˣ?', opts:['It equals its own derivative','It grows faster than any function','Its derivative is 1','It\'s the inverse of x²'], a:0,
    ...
    wrong:{1:'Factorials and x^x grow faster — e\'s magic is the self-derivative, not raw speed.',2:'Derivative 1 belongs to f(x) = x. eˣ\'s derivative is eˣ itself.',3:'The inverse of x² is √x; eˣ\'s inverse is ln x.'}},
   {q:'ln(a·b) = ?', opts:['ln a + ln b','ln a · ln b','ln(a + b)','aᵇ'], a:0,
    ...
    wrong:{1:'Multiplying logs ≈ nothing useful; the product rule lives INSIDE the log: ln(ab) = ln a + ln b.',2:'ln(a+b) doesn\'t simplify at all — a classic trap.',3:'aᵇ is what you\'d exponentiate, not a log identity.'}},
   {q:'d/dx 2ˣ = ?', opts:['2ˣ · ln 2','x · 2ˣ⁻¹','2ˣ','ln 2'], a:0,
    ...
    wrong:{1:'x·2ˣ⁻¹ is the power rule — wrong tool: that\'s for xⁿ (variable base, fixed exponent).',2:'Plain 2ˣ misses the ln 2 factor — only base e has factor 1.',3:'ln 2 alone is the constant; the function 2ˣ stays in the derivative.'}},
   {q:'Softmax exponentiates scores before normalizing. A key consequence:', opts:['Bigger score gaps become much bigger probability gaps','All outputs become equal','Negative scores are discarded','Probabilities can exceed 1'], a:0,
    ...
    wrong:{1:'Softmax outputs always sum to 1 but are rarely equal — equality needs equal scores.',2:'Negative scores are fine: e^negative is small but positive.',3:'Normalization caps every probability at 1.'}},
  ],
});
...
registerLesson({
  ...
  learn:`<p>An <strong>exponent</strong> just counts repeated multiplication: \\(2^5 = 2\\cdot 2\\cdot 2\\cdot 2\\cdot 2 = 32\\). Everything else follows from that one idea — and these are the rules you'll reuse forever:</p>
  ...
  <div class="formula">$$a^m \\cdot a^n = a^{m+n} \\qquad a^{-n} = \\dfrac{1}{a^n} \\qquad a^{1/2} = \\sqrt{a}$$</div>
  ...
  labs:[
   ...
   {key:'laws', title:'Apply the exponent laws', interactive:'prepowersLaws',
    intro:'<p>Predict 2³·2⁴ first, then wield the two laws — multiplying <em>adds</em> exponents, a power of a power <em>multiplies</em> them — to hit exact targets, including the a⁰ = 1 case.</p>'},
  ],
  quiz:[
   {q:'What is 2⁻³?', opts:['1/8','−8','−6','6'], a:0, tag:'negative exponents', focus:'Negative exponent = reciprocal: a⁻ⁿ = 1/aⁿ. The sign never makes the value itself negative.',
    ...
    wrong:{1:'The sign of the exponent never flips the number\'s sign — 2⁻³ is a small POSITIVE fraction, 1/8.',2:'−6 = 2·(−3): that\'s multiplication, not exponentiation.',3:'6 = 2·3 — also multiplication. Exponent means repeated multiply, then reciprocate for the minus sign.'}},
   {q:'Simplify 10³ · 10⁴.', opts:['10⁷','10¹²','100⁷','10⁰·⁷⁵'], a:0, tag:'multiplying powers', focus:'Same base, multiplying ⇒ ADD the exponents: aᵐ·aⁿ = aᵐ⁺ⁿ.',
    ...
    wrong:{1:'10¹² multiplies the exponents — but multiplying powers ADDS them: 3 + 4 = 7.',2:'The base stays 10, not 100 — you don\'t multiply the bases together.',3:'Averaging or dividing the exponents isn\'t a rule at all.'}},
   {q:'What is 9^(1/2)?', opts:['3','4.5','81','18'], a:0, tag:'fractional exponents', focus:'The ½ power is the square root: a^(1/2) = √a.',
    ...
    wrong:{1:'4.5 = 9/2 — you halved the base instead of taking its ½ power (the root).',2:'81 = 9² — that\'s the SECOND power, the opposite direction from one-half.',3:'18 = 9·2 — multiplication again, not a root.'}},
   {q:'Written in scientific notation, 0.0004 is…', opts:['4 × 10⁻⁴','4 × 10⁴','4 × 10⁻³','0.4 × 10⁻³'], a:0, tag:'scientific notation', focus:'Shift the decimal to sit just after the first nonzero digit; a left-start (leading zeros) ⇒ negative power.',
    ...
    wrong:{1:'10⁴ = 10,000 — far too big. Leading zeros mean a NEGATIVE power of ten.',2:'10⁻³ = 0.004, one place off — count all the way to the first nonzero digit (4 places).',3:'Scientific notation keeps exactly one nonzero digit before the point: 4, not 0.4.'}},
   {q:'A model with 10⁹ parameters is how many orders of magnitude bigger than one with 10⁶?', opts:['3','1000','103','9'], a:0, tag:'orders of magnitude', focus:'Orders of magnitude apart = the difference of the exponents.',
    ...
    wrong:{1:'1000 is the RATIO (10³), not the count of orders. Orders of magnitude apart = the gap in exponents, which is 3.',2:'103 just mashes the digits together — subtract the exponents: 9 − 6 = 3.',3:'9 is only the larger exponent; the DIFFERENCE 9 − 6 = 3 is what "orders of magnitude apart" means.'}},
  ],
});
...
registerLesson({
  ...
  <div class="formula">$$\\log_b(x) = y \\iff b^y = x$$</div>
  <p>So \\(\\log_{10}(1000) = 3\\) (ten to the 3rd is 1000) and \\(\\log_2(8) = 3\\) (two cubed is 8). A log is literally the <strong>undo button</strong> for an exponential — they cancel: log<sub>b</sub>(bˣ) = x. Two quick consequences worth memorizing: <strong>log(1) = 0</strong> (any base⁰ = 1) and the log of a fraction is <strong>negative</strong> (log₂(½) = −1).</p>
  ...
  <div class="formula">$$\\log(a \\cdot b) = \\log a + \\log b \\qquad \\log(a^n) = n \\cdot \\log a$$</div>
  ...
  labs:[
   ...
   {key:'mul', title:'logs turn × into +', interactive:'logmul',
    intro:'<p>Pick two numbers. Watch \\(\\log a\\) and \\(\\log b\\) lay end-to-end and land exactly on \\(\\log(a\\cdot b)\\). Multiplication on the inputs becomes plain <b>addition</b> on the logs — the trick behind log-likelihood.</p>'},
   {key:'solve', title:'Solve exponential equations', interactive:'prelogsSolve',
    intro:'<p>A logarithm just solves base<sup>x</sup> = target for the exponent. Predict log₂(32), then drag x to solve 2<sup>x</sup> = 8, 2<sup>x</sup> = 100 (a fractional answer!), and 10<sup>x</sup> = 50.</p>'},
  ],
  quiz:[
   {q:'What is log₁₀(1000)?', opts:['3','100','30','10'], a:0, tag:'evaluating a log', focus:'log₁₀(x) asks "10 to the what equals x?" — for round numbers, count the zeros.',
    ...
    wrong:{1:'100 = 10² is a value, not an exponent — the log returns the EXPONENT, which is 3.',2:'30 mixes things up; log₁₀(1000) is the power of ten that makes 1000, namely 3.',3:'10 is the base, not the answer. Ask "10 to the WHAT is 1000?" → 3.'}},
   {q:'log<sub>b</sub>(x) = y is exactly the same statement as…', opts:['bʸ = x','xʸ = b','b·y = x','y = b/x'], a:0, tag:'definition of log', focus:'A log IS an exponent: log_b(x)=y means the base raised to y gives x.',
    ...
    wrong:{1:'xʸ = b swaps the roles — the BASE is b and it\'s raised to y, giving x.',2:'b·y = x is multiplication; a log is about exponentiation, not a product.',3:'y = b/x is division — unrelated to the exponent relationship.'}},
   {q:'log(a·b) equals…', opts:['log a + log b','log a · log b','log a − log b','log(a + b)'], a:0, tag:'product rule', focus:'The headline identity: logs turn multiplication into addition.',
    ...
    wrong:{1:'Multiplying the logs isn\'t a rule — the product INSIDE becomes a SUM outside: log a + log b.',2:'log a − log b is the rule for a QUOTIENT, log(a/b). For a product you add.',3:'log(a+b) doesn\'t simplify at all — a classic trap. Only products and quotients split.'}},
   {q:'What is log₂(½)?', opts:['−1','1','0','½'], a:0, tag:'logs of fractions', focus:'2 to the what gives ½? A reciprocal needs a negative exponent.',
    ...
    wrong:{1:'+1 would be log₂(2). For ½ = 2⁻¹ the exponent is −1, so the log is negative.',2:'0 is log of 1 (b⁰ = 1) — but ½ is below 1, giving a negative log.',3:'½ is the number itself, not its exponent. 2 to the −1 gives ½, so the log is −1.'}},
...
registerLesson({
  ...
  <div class="formula">$$\\sum_{i=1}^{n} a_i = a_1 + a_2 + \\cdots + a_n$$</div>
  <p>Read it in three parts. The <strong>index</strong> i starts at the bottom value, climbs to the top value n, and for each i you evaluate the expression on the right and add it to a running total. The subscript \\(a_i\\) simply means "the i-th item in the list a".</p>
  ...
  deeper:[
   {title:'😵 Stuck? It\'s literally a for-loop', body:'Σ is a <code>for</code> loop you\'ve written a thousand times:<pre><code class="language-python">total = 0\nfor i in range(1, n + 1):\n    total += a[i]</code></pre>The bottom of the Σ is the loop start, the top is the end, the right-hand expression is the loop body. Coders have an unfair advantage at reading this.'},
  ...
  labs:[
   ...
   {key:'target', title:'Build a sum to a target', interactive:'presigmaSolve',
    intro:'<p>Predict Σᵢ₌₁⁵ i, then choose the term rule <em>and</em> both limits to hit exact totals — the lower limit matters as much as the upper one. Real index manipulation, the skill papers demand.</p>'},
  ],
  quiz:[
   {q:'What does Σ&thinsp;<sub>i=1</sub><sup>4</sup> i equal?', opts:['10','24','4','1+2+3+4+5'], a:0, tag:'evaluating a sum', focus:'Run the index from the bottom value up to the top, adding the expression each time.',
    ...
    wrong:{1:'24 = 1·2·3·4 — that\'s the PRODUCT (Π), not the sum. Σ adds: 1+2+3+4.',2:'4 is just the top limit, not the total. Add every term from i=1 up to 4.',3:'The index stops AT 4, so there is no +5 term. It\'s 1+2+3+4 = 10.'}},
   {q:'In Σ&thinsp;<sub>i=1</sub><sup>n</sup> aᵢ, what is n?', opts:['Where the index stops — it sets how many terms are added','The size of each term','The running total','The starting index'], a:0, tag:'reading the limits', focus:'Bottom = where i starts, top (n) = where it stops. Together they fix the number of terms.',
    ...
    wrong:{1:'That describes a term\'s size — but n is the upper LIMIT: it says where the index stops, i.e. how many terms.',2:'The running total is the RESULT of the Σ, not n.',3:'The starting index is the bottom number (here 1), not the top value n.'}},
   {q:'The mean of a dataset is written…', opts:['(1/n) Σ xᵢ','Σ xᵢ²','Π xᵢ','Σ (1/xᵢ)'], a:0, tag:'mean formula', focus:'Mean = sum of the values divided by how many there are: (1/n) Σ xᵢ.',
    ...
    wrong:{1:'Σ xᵢ² sums squares — that\'s variance territory, not the plain mean.',2:'Π xᵢ multiplies the values; the mean adds them.',3:'Σ (1/xᵢ) leads to the harmonic mean — a different beast.'}},
   {q:'A capital Π (pi) instead of Σ tells you to…', opts:['Multiply the terms','Add the terms','Take the average','Differentiate'], a:0, tag:'product notation', focus:'Σ sums, Π multiplies — same loop, different operation in the body.',
    ...
    wrong:{1:'Adding is Σ\'s job; Π is specifically the MULTIPLY operator.',2:'Averaging needs a Σ plus a division; Π alone just multiplies.',3:'Π is a sum-like accumulation operator — nothing to do with derivatives.'}},
   {q:'Why sum LOG-probabilities instead of multiplying the probabilities directly?', opts:['Products of tiny numbers underflow to 0; logs turn the product into a safe sum','Logs are faster to compute','Summing is more accurate than the true product','It changes which model comes out best'], a:0, tag:'log-sum trick', focus:'ln(ab) = ln a + ln b converts an underflowing product (Π) into a stable sum (Σ).',
    ...
    wrong:{1:'Speed isn\'t the issue — correctness is: the raw product underflows to exactly 0 and destroys the information entirely.',2:'The log-sum equals the log of the product exactly (ln ab = ln a + ln b) — it is not an approximation.',3:'Because ln is monotonic, the SAME parameters maximize both forms — the best model is identical.'}},
  ],
});
...
INTERACTIVES.sigmamean = function(stage, api){
  ...
  const note=document.createElement('div'); note.className='ctrl';
  note.innerHTML='<div style="font-size:13px;line-height:1.6;color:#cdd4f0">The <b>mean</b> is the balance point of the data: \\(\\dfrac{1}{n} \\sum x_i\\). Drag the blue points — the gold fulcrum always settles where they balance. Push one point far out and the mean follows (means are sensitive to outliers).</div>';
  L.ctrl.appendChild(note);
  draw();
};
