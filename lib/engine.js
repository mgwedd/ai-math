...
// Scene Kit (visual-first path, docs/VISUAL_FIRST.md §4, CONTRACT.md §7/§8).
// goals.js = pure goal runtime (engine wires persistence + XP in mountGoals);
// interaction.js = handle/goal wiring over a mounted scene's controller;
// mountScene/SCENES = kit-core's runtime + registry. Pixi stays out of the
// auth-gate chunk: engine.js itself is post-auth dynamic-imported, and pixi
// loads only via createRenderer()'s dynamic import inside renderer/pixi.js.
import { makeGoals } from './scene/goals.js';
import { attachInteraction } from './scene/interaction.js';
import { mountScene } from './scene/scene.js';
...
/* ---------- scene kit: goals panel + scenes path (visual-first) ----------
   The scenes path (renderScenes below) is ADDITIVE: it only runs for a lesson
   that declares `scenes:[...]`. Lessons without scenes keep the legacy
   Learn/Lab/Quiz flow byte-identically. Rendering goes through kit-core's
   `await mountScene(idOrSpec, container, opts)` (CONTRACT §8) and interaction
   wiring through attachInteraction; the engine owns only the goal-checklist
   DOM + XP + advance gating — the same split makeMissions uses.

   Test/DI seam: mountScene resolves its backend from opts.backend; a headless
   run injects createNullBackend via setSceneBackend so no pixi import happens
   in node (mirrors kit-core's own test seam). Prod never calls this. */
let sceneBackendFactory = null;
function setSceneBackend(f){ sceneBackendFactory = f; }

// Build a goals checklist into `stage` and return the wired makeGoals runtime.
// Persists into S.missions[key] (key = lessonId+'::'+sceneId) in the SAME
// index→bool shape as missions — no storage migration. onAllDone gates advance.
function mountGoals(stage, key, defs, onAllDone){
  const box=document.createElement('div'); box.className='missions';
  box.innerHTML='<h3>🎯 GOALS — complete all to continue</h3>';
  const saved = S.missions[key] || {};
  const rows = (defs||[]).map((d,i)=>{
    const row=document.createElement('div'); row.className='mission';
    row.innerHTML='<div class="mcheck"></div><div>'+d.text+'</div><div class="mxp">+'+(d.xp||0)+'</div>';
    if(saved[i]){ row.classList.add('done'); row.querySelector('.mcheck').textContent='✓'; }
    box.appendChild(row); return row;
  });
  stage.appendChild(box);
  enhanceContent(box);
  const store = {
    load(){ return S.missions[key] || {}; },
    save(map){ S.missions[key] = map; save(); },
  };
  const g = makeGoals(defs||[], {
    store,
    onComplete:(def,i)=>{
      rows[i].classList.add('done'); rows[i].querySelector('.mcheck').textContent='✓';
      grantXP(def.xp||0, rows[i]); toast('🎯','Goal complete!',def.text);
    },
    onAllDone,
  });
  return { g, box, rows };
...
const WORLD_META = {
  ...
  rl:   {name:'🕹️ World 5 — Reinforcement Learning', desc:''},
};
const WORLD_ORDER = ['pre','la','calc','prob','ml','rl'];
...
function resetLessonProgress(id){
  ...
  // Namespaced sub-keys ride under `id + '::'`: multi-lab missions
  // (id::labKey), predict commits (id::predict), and scene goals
  // (id::sceneId). Without this sweep, a "reset" left them all complete.
  const prefix = id + '::';
  for(const k of Object.keys(S.missions)) if(k.startsWith(prefix)) delete S.missions[k];
  ...
  for(const w of WORLD_ORDER){
    ...
    for(const l of list){
      ...
      h+='<div class="lcard'+(unlocked?'':' locked')+(done_?' done-card':'')+'" '+
         ...
         '<div class="lemoji">'+l.emoji+'</div><h3>'+l.title+
         ((l.scenes&&l.scenes.length)?' <span class="scene-star" title="Visual scene-based lesson">✦</span>':'')+
         '</h3><div class="lsub">'+l.sub+'</div>'+
...
/* ================================================================
   KB ENRICHMENT UI (docs/KNOWLEDGE-BASE-PLAN.md §7, §10, PR 4).
   ----------------------------------------------------------------
   The engine only ever talks to its OWN /api/kb/* routes (keys are
   server-side); everything here is ADDITIVE and degrades to nothing.
   "Log loudly / degrade silently": a failed/keyless/empty response leaves
   the view pixel-identical to today — no card, no button, no error.
   ================================================================ */
function escHtml(s){
  return String(s==null?'':s).replace(/[&<>"']/g, c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
}
// One concept-bundle fetch per slug per session (memoized). Resolves to the
// bundle or null; never rejects.
const kbBundleCache = new Map();
function fetchConceptBundle(slug){
  if(!API || !slug) return Promise.resolve(null);
  if(kbBundleCache.has(slug)) return kbBundleCache.get(slug);
  const p = fetch(API+'/kb/concept/'+encodeURIComponent(slug))
    .then(r => r.ok ? r.json() : null)
    .catch(() => null);
  kbBundleCache.set(slug, p);
  return p;
}
// Capability probe (once per session): does the server have Wolfram configured?
// Keyless (or offline) => false => the "Show the steps" button never appears,
// so the quiz stays pixel-identical. Returns a boolean; never rejects.
let kbStepsCap = null;
function stepsAvailable(){
  if(!API) return Promise.resolve(false);
  if(kbStepsCap !== null) return Promise.resolve(kbStepsCap);
  return fetch(API+'/kb/steps')
    .then(r => r.ok ? r.json() : {})
    .then(d => { kbStepsCap = !!(d && d.available); return kbStepsCap; })
    .catch(() => { kbStepsCap = false; return false; });
}
// A lesson's primary concept slug: the first quiz tag (preferring weak tags)
// that resolves through the concept registry. null when nothing resolves.
function lessonConceptSlug(l, weak){
  const tags = [];
  if(weak && weak.length) tags.push(...weak);
  for(const q of (l.quiz||[])) if(q && q.tag) tags.push(q.tag);
  for(const t of tags){ const c = resolveTag(t); if(c) return c.id; }
  return null;
}
// Append KB cards into `container`, matching the existing <details class="deep">
// idiom. Renders NOTHING unless the bundle carries a Wikipedia summary or
// Wolfram pods — a bundle with only registry `related` (or {}) leaves the view
// unchanged, so keyless/offline is pixel-identical.
function renderKBEnrichment(container, bundle){
  if(!container || !bundle) return;
  const hasSummary = typeof bundle.summary === 'string' && bundle.summary;
  const hasPods = Array.isArray(bundle.pods) && bundle.pods.length;
  if(!hasSummary && !hasPods) return;
  let html = '';
  if(hasSummary){
    const a = (bundle.attribution && bundle.attribution.wikipedia) || {};
    const url = bundle.url || a.url || 'https://en.wikipedia.org/';
    const related = (Array.isArray(bundle.related) && bundle.related.length)
      ? '<div class="kb-related">Related: '+bundle.related.slice(0,5)
          .map(r=>'<span class="kb-reltag">'+escHtml(r.title)+'</span>').join(' ')+'</div>'
      : '';
    // CC BY-SA attribution + link back is a Wikipedia licensing requirement (§10).
    html += '<details class="deep kb-card" open><summary>🌐 '+escHtml(bundle.title||'Concept summary')+'</summary>'+
      '<div class="deep-body"><p>'+escHtml(bundle.summary)+'</p>'+related+
      '<div class="kb-attrib">'+escHtml(a.text||'From Wikipedia')+
      ' · <a href="'+escHtml(url)+'" target="_blank" rel="noopener noreferrer">'+
      escHtml(a.license||'source')+'</a></div></div></details>';
  }
  if(hasPods){
    const a = (bundle.attribution && bundle.attribution.wolfram) || {};
    html += '<details class="deep kb-card"><summary>🧮 Computed with Wolfram|Alpha</summary>'+
      '<div class="deep-body">'+kbPodsHtml(bundle.pods)+kbWolframAttrib(a)+'</div></details>';
  }
  container.innerHTML = html;
  enhanceContent(container);
}
function kbPodsHtml(pods){
  return (pods||[]).map(p=>
    '<div class="kb-pod"><div class="kb-pod-title">'+escHtml(p.title||'')+'</div>'+
    (p.subpods||[]).map(s=>'<pre class="kb-subpod">'+escHtml(s.plaintext||'')+'</pre>').join('')+
    '</div>').join('');
}
function kbWolframAttrib(a){
  a = a || {};
  return '<div class="kb-attrib"><a href="'+escHtml(a.url||'https://www.wolframalpha.com/')+
    '" target="_blank" rel="noopener noreferrer">'+escHtml(a.text||'Powered by Wolfram|Alpha')+'</a></div>';
}
// Render Show-Steps output into `out` (the /api/kb/steps { steps } shape).
// Empty/degraded leaves `out` untouched so the caller can drop the button.
function renderKBSteps(out, data){
  if(!out) return;
  const steps = data && Array.isArray(data.steps) ? data.steps : [];
  if(!steps.length) return;
  out.innerHTML = '<div class="kb-steps deep-body">'+kbPodsHtml(steps)+
    kbWolframAttrib(data.attribution && data.attribution.wolfram)+'</div>';
  enhanceContent(out);
}
...
function renderLesson(id){
  ...
  function showLearn(){
    ...
    const weakNote = weak.length
      ...
      : '';
    // Optional "see it first" gallery: static/auto figures that illustrate the
    // idea before the learner does it. Each figure is a headless-safe INTERACTIVE
    // taking ONLY a stage; it never touches missions/predict. runCleanups() (fired
    // on step-change/navigation) cancels any figure rAF loop it registered.
    const exps = Array.isArray(l.expositions) ? l.expositions : [];
    const expId = (e,i)=>'expo-'+(e.key||i);   // stable stage id (uses key when present)
    const expHTML = exps.length
      ? '<div class="panel expo-panel"><h2>🎬 See it first</h2><div class="expo-set">'+
        exps.map((e,i)=>'<figure class="expo">'+
          (e.title?'<div class="expo-title">'+e.title+'</div>':'')+
          '<div class="expo-stage" id="'+expId(e,i)+'"></div>'+
          (e.caption?'<figcaption class="expo-cap">'+e.caption+'</figcaption>':'')+
        '</figure>').join('')+'</div></div>'
      : '';
    const b=shell('<div class="panel"><h2>📖 Learn</h2><div class="learn-body">'+l.learn+
      ...
      weakNote + deeper + '<div id="kb-enrich" class="kb-enrich"></div></div></div>'+
      expHTML +
      '<div class="btn-row">'+
      ((S.done[l.id]&&l.quiz&&l.quiz.length)?'<button class="btn ghost" id="rq">Retake quiz (fresh draw) →</button>':'')+
      '<button class="btn" id="next">Try it yourself →</button></div>');
    exps.forEach((e,i)=>{
      const stage=b.querySelector('#'+expId(e,i));
      if(!stage) return;
      const fn=INTERACTIVES[e.figure];
      if(typeof fn!=='function'){ stage.innerHTML='<div class="lab-intro">Missing figure: '+e.figure+'</div>'; return; }
      // A throwing figure must never block the lesson entrance (the #next wiring
      // below), so isolate it — one broken figure degrades to a placeholder.
      try{ const cleanup=fn(stage); if(typeof cleanup==='function') registerCleanup(cleanup); }
      catch(err){ stage.innerHTML='<div class="lab-intro">Figure failed to render.</div>'; }
    });
    // v1.4 §1 sequencing: a scenes lesson keeps the standard shell — Learn is
    // step 0, then the scene ARC replaces the labs step. "Try it yourself"
    // enters renderScenes instead of showLab. Legacy lessons are unchanged.
    b.querySelector('#next').onclick = (l.scenes && l.scenes.length) ? renderScenes : showLab;
    const rq=b.querySelector('#rq'); if(rq) rq.onclick=showQuiz;
    // KB enrichment: lazily fetch the concept bundle when a Go-deeper card
    // expands — or immediately when the weak-tag auto-expand has opened one —
    // and append KB cards. Once per lesson visit; keyless/offline => nothing.
    const kbBox = b.querySelector('#kb-enrich');
    const kbSlug = lessonConceptSlug(l, weak);
    let kbDone = false;
    const enrich = () => { if(kbDone || !kbBox || !kbSlug) return; kbDone = true;
      fetchConceptBundle(kbSlug).then(bundle => renderKBEnrichment(kbBox, bundle)); };
    if(weak.length) enrich(); // first Go-deeper card is auto-open on a weak return
    b.querySelectorAll('details.deep').forEach(d =>
      d.addEventListener('toggle', () => { if(d.open) enrich(); }));
  }
  ...
  function showQuiz(){
    ...
    function renderQ(){
      ...
      let tried=false;
      // "Show the steps" remediation on a MISSED numeric question that declares
      // a `wolfram` hint (docs §7). The button appears only when the server has
      // Wolfram configured (capability probe) — keyless => no button, so the
      // quiz stays pixel-identical. Clicking POSTs only the question_key (the
      // route resolves the query from the registry: no free-form proxying).
      function maybeStepsButton(afterEl){
        if(!API) return;
        if((q.type ?? 'mc') !== 'numeric' || typeof q.wolfram !== 'string' || !q.wolfram) return;
        if(qp.querySelector('.kb-steps-wrap')) return; // once per question
        stepsAvailable().then(ok => {
          if(!ok || qp.querySelector('.kb-steps-wrap')) return;
          const wrap = document.createElement('div'); wrap.className='kb-steps-wrap';
          const btn = document.createElement('button'); btn.className='btn ghost'; btn.textContent='Show the steps';
          const out = document.createElement('div'); out.className='kb-steps-out';
          wrap.appendChild(btn); wrap.appendChild(out);
          if(afterEl && afterEl.insertAdjacentElement) afterEl.insertAdjacentElement('afterend', wrap);
          else qp.querySelector('#ex').appendChild(wrap);
          btn.onclick = () => {
            btn.disabled = true; btn.textContent = 'Loading steps…';
            fetch(API+'/kb/steps', {method:'POST', headers:{'Content-Type':'application/json'},
              body: JSON.stringify({ question_key: l.id+':'+qi })})
              .then(r => r.ok ? r.json() : {})
              .catch(() => ({}))
              .then(data => {
                renderKBSteps(out, data);
                if(out.innerHTML) btn.remove(); // steps shown: retire the button
                else wrap.remove();             // nothing to show: degrade silently
              });
          };
        });
      }
  ...
  /* ---- Visual-first scenes path (docs/VISUAL_FIRST.md §2/§4, CONTRACT v1.4 §1) ----
     A lesson with `scenes:[...]` keeps the standard shell: the Learn step runs
     FIRST (showLearn, unchanged), then this ordered scene ARC replaces the LABS
     step — entered from Learn's "Try it yourself" button. Advance is gated on
     each scene's goals completing. A scene flagged `capstone:true` is the exam:
     completing its goals completes the lesson (the quiz still EXISTS per v1.3
     §5 — reachable via the Learn screen's "Retake quiz", and the completion
     gate for a scenes lesson that has NO capstone). registerCleanup runs on
     every swap. Lessons WITHOUT `scenes` take the legacy Learn→Lab→Quiz flow. */
  function renderScenes(){
    // A lesson's scenes[] entries are registered scene ids (strings) or inline
    // spec objects (flagship owns the lesson<->scene-list resolution rule).
    const scenes = l.scenes.map((sc,i)=>{
      const spec = typeof sc==='string' ? SCENES.find(s=>s.id===sc) : sc;
      if(!spec) console.error('[scenes] lesson '+l.id+' references unknown scene `'+sc+'`');
      // Goal progress persists under lessonId::sceneId — an id-less scene falls
      // back to its INDEX, which silently remaps progress if scenes reorder.
      // Loud in dev, tolerated at runtime (reviewer finding #5).
      if(spec && !spec.id) console.error('[scenes] lesson '+l.id+' scene '+i+' has no id — goal progress will be keyed by index (reorder-fragile)');
      return spec || {id:'missing-'+i, goals:[], caption:'', _missing:true};
    });
    // CAPSTONE POSITION GUARD (reviewer finding #2 / quality's hole): the exam
    // must be the LAST scene. A mid-sequence capstone would end the lesson and
    // silently skip the scenes after it — demote it to a normal scene, loudly.
    const lastIdx = scenes.length-1;
    scenes.forEach((sc,i)=>{ if(sc.capstone && i!==lastIdx)
      console.error('[scenes] lesson '+l.id+': capstone `'+(sc.id||i)+'` is not the last scene — treated as a NORMAL scene. Move it to the end.'); });
    const hasCapstone = !!scenes[lastIdx] && !!scenes[lastIdx].capstone;
    function sceneShell(inner, si){
      view.innerHTML =
        '<div class="lesson-top"><button class="backbtn" onclick="go(\'home\')">← Map</button>'+
        '<div class="lesson-title">'+l.emoji+' '+l.title+'</div></div>'+
        '<div class="steps">'+scenes.map((_,i)=>'<div class="step-dot'+(i<si?' done':i===si?' active':'')+'"></div>').join('')+'</div>'+
        '<div id="stepbody"></div>';
      const b=document.getElementById('stepbody'); b.innerHTML=inner; enhanceContent(b); return b;
    }
    function completeLesson(){
      const firstComplete = !S.done[l.id];
      if(firstComplete){
        logEvent({type:'lesson_complete', lessonId:l.id});
        S.done[l.id]=true; S.firstTry[l.id]=true;
        if(S.weak) delete S.weak[l.id];
        save(); grantXP(SCORING.lessonBonus);
      }
      checkAchievements(); confetti(120);
    ...
    function showScene(){
      ...
      const isCapstone=!!sc.capstone && last;   // position guard: only a LAST capstone is the exam
      const cap = sc.caption ? '<div class="lab-intro">'+sc.caption+'</div>' : '';
      const badge = isCapstone ? '<div class="lab-prog">🏁 CAPSTONE — the exam</div>' : '';
      // Capstone scenes carry a retry affordance (CONTRACT v1.3 §3): reroll the
      // exam params for a fresh attempt. Wired to ix.newAttempt() once mounted.
      const retryBtn = isCapstone ? '<button class="btn ghost" id="reattempt">🎲 New attempt</button>' : '';
      const b=sceneShell(
        '<div class="panel"><h2>'+(sc.emoji||'🎬')+' '+(sc.title||'Scene '+(si+1))+'</h2>'+
          badge+cap+'<div id="scene-canvas"></div><div id="scene-goals"></div></div>'+
        '<div class="btn-row">'+
          '<button class="btn ghost" id="back">← '+(si===0?'Re-read':'Previous scene')+'</button>'+retryBtn+
          '<button class="btn" id="next" disabled>Complete goals to continue</button></div>', si);
      const canvas=b.querySelector('#scene-canvas');
      const goalsHost=b.querySelector('#scene-goals');
      const nextBtn=b.querySelector('#next');
      // Back from the FIRST scene returns to the Learn step (the scene arc is
      // step 1, mirroring showLab's `li===0 ? showLearn`), not straight to Map.
      b.querySelector('#back').onclick = si===0 ? showLearn : ()=>{ si--; showScene(); };
      const goalKey = l.id+'::'+(sc.id||si);
      // Advance targets: capstone completing the lesson vs. moving to the next
      // scene vs. (no capstone) handing off to the legacy quiz gate at the end.
      function advanceToDone(){ const fc=completeLesson(); runCleanups(); showDone(fc, []); }
      function armAdvance(){
        nextBtn.disabled=false;
        if(isCapstone){
          // v1.4 §1 (architect ruling): the capstone GATES completion but is
          // NOT sufficient — the quiz remains required (quiz retirement is a
          // deferred ruling). A credited capstone routes into the standard quiz
          // step; quiz pass → completeLesson via showQuiz.finish(), exactly as
          // the labs path did. Only a lesson with NO quiz finishes directly here.
          if(l.quiz && l.quiz.length){ nextBtn.textContent='Take the quiz →'; nextBtn.onclick=showQuiz; }
          else { nextBtn.textContent='Finish 🏁'; nextBtn.onclick=advanceToDone; }
        }
        else nextBtn.textContent = last ? (hasCapstone||!(l.quiz&&l.quiz.length)?'Finish 🏁':'Take the quiz →') : 'Next scene →';
      }
      // Small goal-complete celebration only — the BIG one (confetti 120) is
      // completeLesson()'s, fired on Finish. A capstone firing 120 here too
      // double-confettied the finish (v1.3 low-severity finding).
      function onAllGoals(){ armAdvance(); confetti(35); }
      if(!isCapstone){
        nextBtn.onclick = last
          ? ((l.quiz && l.quiz.length && !hasCapstone) ? showQuiz : advanceToDone)
          : ()=>{ si++; showScene(); };
      }
      // Mission entry + goals checklist ONLY for scenes with >=1 goal
      // (CONTRACT v1.3 §4): makeGoals([]) is vacuously all-done — a decorative
      // scene gates nothing and must not mint an empty S.missions entry.
      // validateCurriculum (flagship) rejects goal-less CAPSTONES at build.
      const goalDefs = sc.goals||[];
      let g = null;
      if(goalDefs.length){
        ({ g } = mountGoals(goalsHost, goalKey, goalDefs, onAllGoals));
        registerCleanup(()=>g.dispose());
      } else {
        armAdvance();   // nothing to gate; no celebration for an empty checklist
      }
      // Mount through kit-core's async runtime (CONTRACT §8). The cleanup is
      // registered BEFORE the await resolves: a scene swap mid-mount flips
      // `dead`, and the late-resolving mount tears itself down instead of
      // leaking a live controller (driver/canvas/listeners) under the next
      // screen. attachInteraction wires handles + goal evaluation + the
      // learner-input gate off the controller.
      let dead=false, ctl=null, ix=null;
      registerCleanup(()=>{ dead=true;
        if(ix){ try{ ix.dispose(); }catch(e){} ix=null; }
        if(ctl){ try{ ctl.destroy(); }catch(e){} ctl=null; }
      });
      // Capstone retry (CONTRACT v1.3 §2–3): the mandated trio lives in
      // ix.newAttempt(): controller.newAttempt() (reroll + gate close) →
      // goals.resetAttempt() (drop in-flight hold/visited progress; earned
      // credit stays) → baseline re-eval. No-op until the mount resolves.
      if(isCapstone){
        b.querySelector('#reattempt').onclick = ()=>{ if(ix) ix.newAttempt(); };
      }
      if(sc._missing){ canvas.innerHTML='<div class="lab-intro">Scene failed to render.</div>'; }
      else (async ()=>{
        try{
          const c = await mountScene(sc, canvas, {
            // Pass a backend INSTANCE, not the factory: mountScene's
            // instance-vs-factory check is `typeof backend.apply==='function'`,
            // which a factory FUNCTION also satisfies (Function.prototype.apply)
            // — so a factory would be mis-taken for an instance. Resolve it here.
            backend: sceneBackendFactory ? sceneBackendFactory() : undefined,
            hooks:{ onError:(e)=>{ console.error('[scenes] render error in '+goalKey, e); } },
          });
          if(dead){ try{ c.destroy(); }catch(e){} return; }   // swapped away mid-mount
          ctl = c;
          const x = attachInteraction(c, { goals:g, container:canvas });
          if(dead){ try{ x.dispose(); }catch(e){} try{ c.destroy(); }catch(e){} ctl=null; return; }
          ix = x;
          // Size the space/backend to the stage (controller.resize is the
          // caller's job per CONTRACT §8 — without it toWorld has no scale).
          const r = canvas.getBoundingClientRect ? canvas.getBoundingClientRect() : null;
          c.resize((r&&r.width)||640, (r&&r.height)||440,
            typeof devicePixelRatio!=='undefined'?devicePixelRatio:1);
        }catch(e){
          if(!dead) canvas.innerHTML='<div class="lab-intro">Scene failed to render.</div>';
          console.error('[scenes] mount failed for '+goalKey, e);
        }
      })();
    }
    showScene();
  ...
  // v1.4 §1: EVERY lesson starts at the Learn step. A scenes lesson's Learn
  // #next routes into renderScenes (above); a legacy lesson's routes to showLab.
  showLearn();
}
...
// Scene Kit: goals panel builder (unit-tested) + the headless backend seam
// (tests inject createNullBackend so the scenes path runs without pixi/GPU).
// resetLessonProgress exported for the ::-prefix sweep test.
export { setSceneBackend, mountGoals, resetLessonProgress };
