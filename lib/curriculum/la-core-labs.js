...
INTERACTIVES.vecopsAddAnim = function(stage){
  const F = figCanvas(stage, 640, 300);
  const ctx = F.ctx, W = F.W, H = F.H;
  const P = plane(ctx, W, H, 46, 200, H / 2);
...
INTERACTIVES.vecopsSpanFig = function(stage){
  ...
  const PL = plane(ctx, W, H, 24, 165, H / 2);
  PL.grid();
  lattice(PL, { x: 1.4, y: 0.5 }, { x: 0.4, y: 1.4 });
  ctx.fillStyle = '#cdd4f0'; ctx.font = '700 13px ' + FONT(); ctx.textAlign = 'center';
  ctx.fillText('independent → whole plane', 165, 28);
  ...
  const PR = plane(ctx, W, H, 24, 480, H / 2);
  lattice(PR, { x: 1, y: 0.6 }, { x: 2, y: 1.2 });
  ctx.fillStyle = '#cdd4f0'; ctx.font = '700 13px ' + FONT(); ctx.textAlign = 'center';
  ctx.fillText('dependent (b = 2a) → a line', 480, 28);
};
...
INTERACTIVES.vecopsSpan = function(stage, api){
  ...
  note(L.ctrl, 'Drag <b style="color:#b9a8ff">a</b> and <b style="color:#7fe7ff">b</b>. The dots are every reachable combination c₁a + c₂b. Slide b onto a\'s own line and the whole lattice flattens into one line — that collapse is <b>linear dependence</b>, and it happens exactly when det → 0.');
  draw();
};
