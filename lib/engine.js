...
// Scene Kit goals runtime (visual-first path, docs/VISUAL_FIRST.md §4). Pure,
// headless; the engine wires it to persistence + XP/toast in mountGoals().
...
/* ---------- scene kit: goals panel + runtime seam (visual-first) ----------
   ...
   Learn/Lab/Quiz flow byte-identically. The actual scene rendering (Pixi/three
   entities) is owned by the Scene Kit core; the engine holds only a runtime
   SEAM that the kit registers into, plus the goal-checklist DOM + XP wiring —
   the same split makeMissions uses, but goals.js carries the pure logic.

   Scene runtime contract (see coordination Handoffs / CONTRACT.md):
     SCENE_RUNTIME.mount(sceneSpec, canvasEl, { goals }) -> cleanupFn
   where `goals` is a makeGoals() instance the kit drives:
     - goals.evaluate(paramsSnapshot) once at mount (BASELINE) + on every change
     - goals.reportEpisode(outcome) when a sim/game run ends
   The kit uses lib/scene/interact.js for handles. Returns a cleanup fn. */
let SCENE_RUNTIME = null;
function setSceneRuntime(rt){ SCENE_RUNTIME = rt; }
...
function renderLesson(id){
  ...
  function renderScenes(){
    ...
    const hasCapstone = scenes.some(s => s.capstone);
    ...
    function showScene(){
      runCleanups();
      const sc=scenes[si], last=si===scenes.length-1, isCapstone=!!sc.capstone;
      const wasDone = !!S.done[l.id];
      ...
      // Mount the real scene via the kit runtime; degrade gracefully if the kit
      // hasn't registered a runtime yet (e.g. before the P0 kit lands).
      if(SCENE_RUNTIME && typeof SCENE_RUNTIME.mount==='function'){
        try{ const cl=SCENE_RUNTIME.mount(sc, canvas, { goals:g });
          if(typeof cl==='function') registerCleanup(cl); }
        catch(e){ canvas.innerHTML='<div class="lab-intro">Scene failed to render.</div>'; }
      } else {
        canvas.innerHTML='<div class="lab-intro">Scene renderer loading…</div>';
      }
    }
    showScene();
  }
...
// Scene Kit seam: Kit Core registers its renderer via setSceneRuntime(); the
// scenes path (renderLesson) + goals panel are unit-tested via these.
export { setSceneRuntime, mountGoals };
