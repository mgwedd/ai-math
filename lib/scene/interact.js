...
export function makeInteraction(surface, cfg = {}) {
  ...
  function write(h, worldPoint) {
    ...
    // atom.set fans out to subscribers (goal evaluation, frame scheduling) —
    // a throwing subscriber must not break the pointer rig mid-drag.
    try { h.atom.set(p); } catch (e) { /* isolate */ }
    if (h.onChange) { try { h.onChange(p); } catch (e) { /* isolate */ } }
    if (surface.requestDraw) surface.requestDraw();
    onAny(h, p);
  }

  // End the active drag cleanly: release the pointer capture and fire
  // onRelease. ONE exit path shared by pointerup/cancel, handle.remove() on
  // the active handle (v1.3 item 3 — refresh() after newAttempt removes every
  // handle, possibly mid-drag), and dispose().
  function endDrag() {
    if (!active) return;
    const h = active; active = null;
    const pid = activePointer; activePointer = null;
    if (pid != null && el.releasePointerCapture) {
      try { el.releasePointerCapture(pid); } catch (e) { /* stub */ }
    }
  ...
  function onKey(ev) {
    if (active) return;   // mid-drag: arrow keys must not write (or clobber _lock)
  ...
  return {
    handle(atom, opts = {}) {
      const h = {
        ...
        remove() {
          ...
          if (active === h) endDrag();   // mid-drag removal: release capture + onRelease (v1.3)
    ...
    handles,
    dispose() {
      // Mid-drag teardown ends the drag cleanly — capture released AND
      // onRelease fired (real once a persistent Pixi canvas is reused across
      // scenes) — then restores the element's focusability state.
      endDrag();
